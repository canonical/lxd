...
Even though we will use the same image name to launch the instance as we used when creating a container, LXD downloads a slightly different image that is compatible with VMs. Thus, this image is not cached.
...
## Optional: Hold updates

Snaps automatically update when a new release is published to a snap channel. While it is not required, we strongly recommend that for production use, you disable automatic updates for the LXD snap, then apply updates manually. This gives you control over when updates are applied. To hold updates for the LXD snap indefinitely, run:

```bash
sudo snap refresh --hold lxd
```

To learn more about how to manage the LXD snap, including handling updates, see: {ref}`howto-snap`.
