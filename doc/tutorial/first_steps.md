...
### Snap installation requirements
...
If you see an error message that the LXD snap is already installed, run the following command to refresh it and ensure that you are running an up-to-date version:
...
If there is no result, enter the following commands:
...
The first command adds your user to the `lxd` group.
The second command starts a new shell where the group membership takes effect immediately.

(tutorial-initialize)=
## Initialize LXD

We will initialize LXD using a minimal setup with default options. Run:

```bash
lxd init --minimal
```

Once you have learned more about LXD, you might want to tune the {ref}`initialization options <initialize>` according to your own preferences, or learn how to {ref}`use a preseed file <initialize-preseed>` to initialize LXD. For now, the minimal configuration is sufficient.
...
### Confirm virtualization support
 ...
 For LXD virtual machines, your host system must be capable of KVM virtualization. Test for this as shown below.
...
If you see `virtual-machine` in the list of `instance_types`, this confirms that your host system is capable of virtualization.
...
The `lxc launch` command creates an instance then immediately starts it. By default, it creates a container instead of a virtual machine. Using this command, launch a container named `first`, using the Ubuntu 24.04 LTS image:
...
Next, we will launch a VM using the Ubuntu 24.04 LTS image.

Even though we will use the same image name to launch the instance as we used when creating a container, LXD downloads a slightly different image that is compatible with VMs.

`````{tabs}
````{group-tab} CLI

We will use the same `lxc launch` command, this time to create an instance named `ubuntu-vm`. To create it as a VM instead of a container, add the `--vm` flag:
...
Open the form to create an instance, and set the name to `ubuntu-vm`. Browse for an image. You'll notice that the cached Ubuntu 24.04 image is at the top, with a {guilabel}`Type` set to `container`. Select instead the Ubuntu 24.04 LTS image with the {guilabel}`Type` of `all`.
...
For the desktop VM to run smoothly, we must allocate a higher memory limit than the default 1 GiB for virtual machines.
...
Another command, `lxc config show`, displays configuration details for the instance. Run:
...
This list tells you the name, instance type, description, IPv4 address, and status for each instance. When you hover over an instance row, icons appear
to start, restart, freeze (pause), or stop the instance.
...
First, run these commands on your host system:
...
These are standard Linux commands; the first displays memory information in megabytes, and the second displays the number of available CPUs.
...
Enter an interactive shell into the `second` container and run the same commands:
...
Go to the {guilabel}`Terminal` tab for the `first` container. This terminal provides an interactive shell into the instance.
...
These are standard Linux commands; the first displays memory information in megabytes, and the second displays the number of available CPUs. Take note of the output from these commands.

Go to the {guilabel}`Terminal` tab for the `second` container and enter the same commands. You should see that the total memory is limited to 192 MiB, and the number of available CPUs is 1.

You should see that these values differ from those of the `first` container. The exact values are inherited from the host system. Thus for example, if your host system has only one CPU, you might see one CPU for the `first` container as well.
...
Now if you again view the content of the file on the container, you should see the line that you added:
...
(tutorial-snapshots)=
## Back up instances by creating snapshots
...
`lxc list <instance>` shows the number of available snapshots for an instance, and `lxc info <instance>` displays information about each snapshot.
...
Confirm that you can now enter the bash shell, then exit the shell:
...
Click the checkbox to the left of each instance you want to delete. From the buttons that appear at the top of the page, click {guilabel}`Stop`, then {guilabel}`Delete`.
````
`````
