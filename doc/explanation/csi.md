...
| Block volumes             | &#x2713;        | {ref}`storage-drivers-local`, {ref}`storage-drivers-remote`, and {ref}`storage-drivers-shared` | Supported when the driver provides block volumes.                                                                                                                                               |
...
| Shared volumes            | - (coming-soon) | {ref}`storage-drivers-shared`                                                                  | Allows attaching storage volume to multiple nodes simultaneously (through the use of volume access modes `ReadWriteMany` and `ReadOnlyMany`).                                                   |
...
1. The request is authorized by the DevLXD API, which verifies that desired operation is allowed to be executed on a particular LXD entity.
...
1. Kubelet invokes the CSI node service requesting attached volume to be mounted into Pod.
