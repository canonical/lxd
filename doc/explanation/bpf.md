...
`security.delegate_bpf` option enables BPF functionality delegation mechanism (using BPF Token). If this setting is enabled then LXD mounts BPF filesystem inside a container instance with delegation configuration (depending on `security.delegate_bpf.*` settings).
For example:
...
Then, applications inside the container can create BPF Token file descriptor using that BPFFS mount and `bpf(BPF_TOKEN_CREATE)` syscall. Later, this token file descriptor can be passed to `bpf(BPF_PROG_LOAD)`, `bpf(BPF_MAP_CREATE)`, or another `bpf()`-command syscall and kernel will perform a permission check against the token instead of current user capabilities. To be more precise, current user caps are also checked for `CAP_BPF` but in a current user namespace when `bpf(BPF_TOKEN_CREATE)` is called.

As it follows, user space applications inside the container must be aware of BPF Token kernel feature (appeared in Linux kernel v6.9) and make use of it. In contrast to `security.syscalls.intercept.*` features this one is not fully transparent and may require update or modification of the software inside the container. Fortunately, libbpf supports BPF tokens and if application uses libbpf then making use of this feature could be as simple
as updating libbpf.

```{note}
`security.delegate_bpf.cmd_types`, `security.delegate_bpf.map_types`, `security.delegate_bpf.prog_types`, `security.delegate_bpf.attach_types` need to be configured depending on BPF workload in the container.
```

Also see the [BPF Token documentation page](https://docs.ebpf.io/linux/concepts/token/) on `docs.ebpf.io` for details.

## Example (socket filter)

Let's consider an example with socket filter program from [libbpf-bootstrap](https://github.com/libbpf/libbpf-bootstrap).

Create an unprivileged container instance and set all the necessary configuration options to enable BPF delegation.
...
Clone and build an example code
...
Now, everything is ready for an experiment
...
If everything works like intended then you'll see something like:
...
As we can see from this output our ICMP packets were captured by eBPF program and logged.

## Finding the right configuration

To figure out the right values for `security.delegate_bpf.cmd_types`, `security.delegate_bpf.map_types`, `security.delegate_bpf.prog_types`, `security.delegate_bpf.attach_types` options you need to know how your application inside the container uses eBPF (which program types, map types, etc.) You can always consult with the source code or use [`strace`](https://github.com/strace/strace) tool to trace `bpf` syscall and see how it is being used.

Invocation can be like `strace -e bpf ./sockfilter`:
...
From this log we can see that our `sockfilter` is using:
