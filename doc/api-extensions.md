...
(extension-storage-lvm-use-thinpool)=
## `storage_lvm_use_thinpool`

Adds the {config:option}`storage-lvm-pool-conf:lvm.use_thinpool` configuration key, a Boolean that defaults to `true`.
...
(extension-snapshot-schedule-aliases)=
...
(extension-container-backup-override-pool)=
...
(extension-projects-limits)=
## `projects_limits`

Adds support for project-level resource limits, through the {config:option}`project-limits:limits.cpu`,
{config:option}`project-limits:limits.memory`, {config:option}`project-limits:limits.processes`,
{config:option}`project-limits:limits.containers`, and {config:option}`project-limits:limits.virtual-machines`
...
(extension-operation-metadata-entity-url)=
...
(extension-storage-driver-powerstore)=
...
(extension-oidc-device-client-id)=
...
(extension-storage-nvme-tcp)=
## `storage_nvme_tcp`

Renames storage pool NVMe/TCP mode from `nvme` to `nvme/tcp`.

(extension-project-replica-mode)=
...
(extension-cluster-links-used-by)=
...
1. A `POST` request with a name and `remote_address` (no `fingerprint`) creates a pending link. Cluster A fetches Cluster B's TLS certificate and confirms Cluster B is serving the LXD API, then returns the certificate's fingerprint for user verification. The certificate is held in `volatile.pending_certificate` and the verified address in `volatile.pending_address`. Neither is pinned yet, so the link remains inert (unreachable) until confirmed. Repeating this request for a link that is still pending refreshes it instead of returning a conflict, so a caller can retry an interrupted creation.
2. A second `POST` request with the same name, this time including the `fingerprint` returned by the first request, pins the certificate and activates the link. The caller acknowledges the certificate they verified by echoing the fingerprint. Cluster A rejects a fingerprint that does not match the one it fetched in the first request. Cluster A never asks the caller to resubmit the certificate itself: it pins the copy already held in `volatile.pending_certificate`. This request must not set `remote_address`; Cluster A rejects the request if it does. Cluster A always pins the canonical address (with an explicit port) recorded in `volatile.pending_address` into `volatile.addresses`. This ensures the link points at the address that was actually verified, rather than at anything supplied in the confirm request.
