...
As part of the recovery process it might be necessary to recover already existing remote storage pools or physical drives when creating
a new storage pool in LXD.

Previously this was only possible for some of the drivers (e.g. `ceph.osd.force_reuse`).
The new pool level `source.recover` configuration key can be set per node to allow reuse of an existing `source`.
What it does not allow is reusing the same source for multiple storage pools.
Some storage drivers like LVM have a specific configuration key for this purpose.
