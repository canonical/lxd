...
The LXD and DevLXD operation wait endpoints (`GET /1.0/operations/{id}/wait`) now return HTTP status `200` when the wait itself succeeds, even if the operation it waited on failed.
When the server supports this extension, the DevLXD operation also has a new `err_code` field that carries the HTTP status code associated with the failed operation.

(extension-vm-volatile-maxcpus)=
## `vm_volatile_maxcpus`
