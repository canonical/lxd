...
This introduces two new configuration keys {config:option}`server-miscellaneous:storage.project_*.images_volume` and
{config:option}`server-miscellaneous:storage.project_*.backups_volume` per each project to allow for a storage volume on an existing
