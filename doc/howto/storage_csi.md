...
This document provides practical examples of configuring StorageClass and PersistentVolumeClaim (PVC) resources when using the LXD CSI driver.
...
Setting volume binding mode to `Immediate` instructs Kubernetes to provision the volume as soon as the PVC is created.
...
A PVC requests storage volume from a StorageClass.
