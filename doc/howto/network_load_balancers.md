...
Network load balancers are similar to forwards in that they allow specific ports on an external or internal IP address to be forwarded to specific ports on internal IP addresses in the network that the load balancer belongs to.
...
- A single external or internal listen IP address.
...
In case of an external IP address:
...
In case of an internal IP address:

- Allowed listen addresses must not be used by the associated network's gateway, other existing load balancers and network forwards, and instance NICs.
