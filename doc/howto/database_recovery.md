...
myst:
  html_meta:
    description: How to recover LXD database records through the interactive recovery tool, which scans storage pools to identify missing volumes and other resources.
---

(disaster-recovery)=
# How to recover LXD database records for instances and custom volumes
...
Only use this tool for disaster recovery.
...
Back in the original terminal, press the `ENTER` key to resume the recovery process.
The tool now lists both the `default` and `backup` pools.
Press the `ENTER` key again to begin a scan of these pools.
...
Then return to the original terminal and press the `ENTER` key to resume the recovery process.
