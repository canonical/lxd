...
Both clusters need a project with the same name. Set the {config:option}`project-replica:replica.cluster` configuration key on both projects, to identify the cluster link that is allowed to replicate instances between them. It is required on the standby project, and is also needed on the leader project so that the leader can later be promoted back to leader mode after a failover, when it has itself become a standby.
...
1. On the leader cluster, create a project and point it at the standby cluster:
   ...
   ````{group-tab} CLI
   ```bash
   lxc project create <project_name> --config replica.cluster=<standby_cluster_link_name>
   ```
   ````
   ````{group-tab} UI
...
1. On the standby cluster, create a project with the same name and configure it to accept replication from the leader cluster:
   ...
   ````{group-tab} CLI
   ```bash
   lxc project create <project_name> --config replica.cluster=<leader_cluster_link_name>
   ```
   ````
   ````{group-tab} UI
   Expand the {guilabel}`Project` drop-down and select {guilabel}`+ Create project` at the bottom.
...
1. On the leader cluster, create a replicator that targets the standby cluster. The replicator must exist before the project can be promoted, because it is what identifies the project as the source of a replication topology. Each cluster link can be targeted by at most one replicator per project; creating or updating a replicator to target a cluster link already used by another replicator in the same project fails with a conflict error.
