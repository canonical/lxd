...
Create a storage pool named `my-cephrdb-pool` using the {ref}`Ceph RBD driver <storage-ceph>` and the on-disk name `my-osd` on three cluster members.
...
```{terminal}
:input: lxc storage create my-cephrdb-pool ceph --target=vm01
Storage pool my-cephrdb-pool pending on member vm01
:input: lxc storage create my-cephrdb-pool ceph --target=vm02
Storage pool my-cephrdb-pool pending on member vm02
:input: lxc storage create my-cephrdb-pool ceph --target=vm03
Storage pool my-cephrdb-pool pending on member vm03
:input: lxc storage create my-cephrdb-pool ceph ceph.osd.pool_name=my-osd
Storage pool my-cephrdb-pool created
```
