...
Before recovering a storage pool, you need to know its original configuration: the driver type, source, and configuration keys. Use the [`lxc storage show`](lxc_storage_show.md) command:
...
See {ref}`howto-cluster-storage` to learn how to view both cluster-wide and member-specific storage configuration.
