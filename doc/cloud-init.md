...
* All images from the `ubuntu` and `ubuntu-daily` {ref}`image registries <remote-image-servers>` have `cloud-init` support.
...
* Images from the [`images` registry](https://images.lxd.canonical.com/) have `cloud-init`-enabled variants, which are usually bigger in size than the default variant.
