...
All created volumes are thin-provisioned block volumes, that LXD formats with a desired file system if required (for containers and custom file system volumes).

LXD expects the PureStorage to be pre-configured with a specific service (e.g. iSCSI) on network interfaces whose address is provided during storage pool configuration.
...
On the other hand, using remote storage has significant advantages in a cluster setup, because all cluster members have access to the same storage pools with the exact same contents, without the need to synchronize them.
...
Volume snapshots are also supported by the PureStorage. However, each snapshot is associated with a parent volume, and cannot be directly attached to the host.
Therefore, when a snapshot is being exported, LXD creates a temporary volume behind the scenes, which is attached to the LXD host and remove once the operation is completed.
...
Therefore, the driver uses volume's {config:option}`storage-pure-volume-conf:volatile.uuid` to generate a shorter volume name.
