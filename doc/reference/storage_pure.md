...
When creating a new storage pool using the `pure` driver in either `iscsi` or `nvme` mode, LXD tries to discover one of the array's qualified names on the address `pure.array.address`.
