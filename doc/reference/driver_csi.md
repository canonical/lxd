...
This document contains reference for the LXD CSI driver CLI, available Helm chart values and its versioning information.
...
The LXD CSI driver Helm chart is available as [OCI image](https://ghcr.io/canonical/charts/lxd-csi-driver).
The source of the Helm chart can be found in [LXD CSI driver repository](https://github.com/canonical/lxd-csi-driver/tree/main/charts).
...
| `controller.replicas`                              | int    | `1`                                                     | Controller Deployment replicas. When deployed in multiple replicas, preferred affinity is configured in an attempt to distribute pods across multiple nodes.                              |
...
| `controller.serviceAccount.name`                   | string | `""` (Equals to Controller server name if empty)        | Custom service account name                                                                                                                                                               |
...
| `controller.csiProvisioner.image.imagePullPolicy`  | string | `IfNotPresent`                                          | CSI provisioner pull policy                                                                                                                                                               |
...
| `controller.csiAttacher.pullPolicy`                | string | `IfNotPresent`                                          | CSI attacher image pull policy                                                                                                                                                            |
...
| `node.serviceAccount.name`                         | string | `""` (Equals to Node server name if empty)              | Custom service account name                                                                                                                                                               |
