...
ifeq "$(GOMIN)" "$(NEW_GOMIN)"
	...
	@$(call check_and_commit,Makefile go.mod doc/requirements.md .github/copilot-instructions.md,"go: Update Go minimum version to $(NEW_GOMIN)")
...
ifneq "$(LXD_OFFLINE)" ""
	...
	@$(call check_and_commit,go.mod go.sum,"go: Update dependencies")
...
check-schema: update-schema
	...
	if [ -n "$$FILES" ]; then \
		$(call check_and_commit,$$FILES,"lxd: Update generated code"); \
	fi
...
check-api: update-api
	@$(call check_and_commit,doc/rest-api.yaml,"doc/rest-api: Refresh swagger YAML")

.PHONY: update-metadata
update-metadata: lxd-metadata
	@echo "Generating golang documentation metadata"
	$(shell [ -n "$(GOCOVERDIR)" ] && mkdir -p "$(GOCOVERDIR)" && chmod 0777 "$(GOCOVERDIR)")
	$(GOPATH)/bin/lxd-metadata . --json ./lxd/metadata/configuration.json --txt ./doc/metadata.txt --substitution-db ./doc/substitutions.yaml 2>&1 | grep -vF ' Found lxddoc at ' || true

.PHONY: check-metadata
check-metadata: update-metadata
	@$(call check_and_commit,lxd/metadata/configuration.json doc/metadata.txt,"doc: Update metadata")
...
check-auth: update-auth
	@$(call check_and_commit,lxd/auth/entitlements_generated.go lxd/auth/drivers/openfga_model.openfga,"lxd/auth: Update auth")
