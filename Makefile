...
deps-cache: deps
	...
	# Copy headers (part of the source tree) so the deps checkout stays intact for
	# subsequent rebuilds; only move the large build outputs to save disk.
	cp -r "$(DQLITE_PATH)/include" "$(DEPS_CACHE_PATH)/dqlite/include"
	mv "$(DQLITE_PATH)/.libs" "$(DEPS_CACHE_PATH)/dqlite/libs"
	mkdir -p "$(DEPS_CACHE_PATH)/liblxc"
	cp -r "$(LIBLXC_PATH)/include" "$(DEPS_CACHE_PATH)/liblxc/include"
