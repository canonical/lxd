...
func (c *cmdClusterRestore) command() *cobra.Command {
	...
	cmd.Flags().BoolVar(&c.action.flagMemberOnly, "member-only", false, i18n.G(`Restore only the cluster member status without starting local instances or migrating back evacuated instances`)+"``")
