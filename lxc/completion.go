...
// topLevelResourceNameFuncs is a map of functions that can return LXD API resource names without any arguments.
// This is used when returning completions for arguments like `<remote>:<name>`.
var topLevelResourceNameFuncs = map[string]func(server lxd.InstanceServer) ([]string, error){
	...
	"image": func(server lxd.InstanceServer) ([]string, error) {
		return server.GetImageFingerprints()
	},
	"image_alias": func(server lxd.InstanceServer) ([]string, error) {
...
func (g *cmdGlobal) cmpTopLevelResourceInRemote(remote string, entityType string, toComplete string) ([]string, cobra.ShellCompDirective) {
	nameFunc, ok := topLevelResourceNameFuncs[entityType]
	if !ok {
