...
running state, including process memory state, TCP connections, ...`))
	...
	cmd.Flags().StringVar(&c.flagDisks, "disks", "", i18n.G(`Which disks should be included in the snapshot; can be "root" or "volumes"`))
	cmd.Flags().StringVar(&c.flagExcludeDisks, "exclude-disks", "", i18n.G(`Comma-separated list of names for which disks should be ignored when performing a multi-volume snapshot`))
