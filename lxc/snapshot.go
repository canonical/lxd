...
running state, including process memory state, TCP connections, ...`))
	...
	cmd.Flags().StringVar(&c.flagDiskVolumes, "disk-volumes", "", i18n.G(`Disk volumes mode. Possible values are "root" (default) and "all-exclusive". "root" only snapshots the instance's root disk. "all-exclusive" snapshots the instance's root disk and any exclusively attached volumes (non-shared).`))
