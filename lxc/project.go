...
func (c *cmdProjectDelete) command() *cobra.Command {
	...
	cmd.Flags().BoolVarP(&c.flagForce, "force", "f", false, i18n.G("Force delete project and its entities (instances, profiles, images, networks, network ACLs, network zones, storage volumes, and storage buckets)")+"``")
...
func (c *cmdProjectDelete) promptConfirmation(name string) error {
	...
	fmt.Printf(i18n.G(`Forcefully removing a project will delete the project and its entities (instances, profiles, images, networks, network ACLs, network zones, storage volumes, and storage buckets)
