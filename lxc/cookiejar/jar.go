...
func (j *Jar) Save() error {
	...
	// Stat the file. We don't want to use os.Create immediately because we don't want to truncate the file before
	// obtaining a lock.
	info, err := os.Stat(j.filepath)
	if err != nil {
		if !errors.Is(err, os.ErrNotExist) {
		...
		// Else open it.
		f, err = os.OpenFile(j.filepath, os.O_RDWR, info.Mode())
		if err != nil {
			return fmt.Errorf("Failed to open cookie jar file: %w", err)
		}
	}
