...
func (c *cmdDelete) command() *cobra.Command {
	...
	cmd.Flags().StringVar(&c.flagDiskVolumes, "disk-volumes", "", i18n.G(`Disk volumes mode for snapshot deletion. Possible values are "root" (default) and "all-exclusive". "root" only deletes the instance's root disk snapshot. "all-exclusive" deletes the instance's root disk snapshot and any exclusively attached volumes (non-shared) snapshots.`))
...
func (c *cmdDelete) doDelete(d lxd.InstanceServer, name string) error {
	...
	if shared.IsSnapshot(name) {
		...
		if c.flagDiskVolumes != "" {
			op, err = d.DeleteInstanceSnapshotWithDiskVolumesMode(fields[0], fields[1], c.flagDiskVolumes)
		} else {
