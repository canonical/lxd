...
If the image remote is a public remote (e.g. simplestreams) then this project is ignored.
If the image remote is another LXD server, specify the source project for the image remote 
with --project and the instance remote with --target-project.
`))
	...
	cmd.Flags().StringVar(&c.flagTargetProject, "target-project", "", i18n.G("The project to create the instance in")+"``")
