...
func (c *cmdImageRegistry) command() *cobra.Command {
	...
	// Unset
	imageRegistryUnsetCmd := cmdImageRegistryUnset{global: c.global, imageRegistry: c, imageRegistrySet: &imageRegistrySetCmd}
...
The registry protocol is inferred from its configuration: a "url" key configures a
SimpleStreams registry, and a "cluster" key configures a LXD registry.
...
func (c *cmdImageRegistryCreate) run(cmd *cobra.Command, args []string) error {
	...
	// Create the image registry.
	op, err := client.CreateImageRegistry(imageRegistry)
	if err == nil {
		err = op.Wait()
	}
...
func (c *cmdImageRegistryRename) run(cmd *cobra.Command, args []string) error {
	...
	op, err := resource.server.RenameImageRegistry(resource.name, api.ImageRegistryPost{Name: args[1]})
	if err == nil {
		err = op.Wait()
	}
...
func (c *cmdImageRegistryDelete) run(cmd *cobra.Command, args []string) error {
	...
	op, err := resource.server.DeleteImageRegistry(resource.name)
	if err == nil {
		err = op.Wait()
	}
...
func (c *cmdImageRegistryEdit) run(cmd *cobra.Command, args []string) error {
	...
	if !termios.IsTerminal(getStdinFd()) {
		...
		op, err := resource.server.UpdateImageRegistry(resource.name, newData, "")
		if err == nil {
			err = op.Wait()
		}
	...
	for {
		...
		if err == nil {
			...
			op, err = resource.server.UpdateImageRegistry(resource.name, newData, etag)
			if err == nil {
				err = op.Wait()
			}
		}
...
func (c *cmdImageRegistrySet) run(cmd *cobra.Command, args []string) error {
	...
	op, err := resource.server.UpdateImageRegistry(resource.name, writable, etag)
	if err == nil {
		err = op.Wait()
	}
