...
func (c *cmdList) run(cmd *cobra.Command, args []string) error {
	...
	if needsData && d.HasExtension("container_full") {
		...
		if d.HasExtension("instances_state_selective_fields") {
			...
			// Construct recursion value with bracket notation (e.g., "[state.disk,state.network]" or "[]").
			recursion := "[" + strings.Join(recursionFields, ",") + "]"

			// Use selective recursion API.
			if c.flagAllProjects {
				instances, err = d.GetInstancesFullAllProjectsWithFilterAndRecursion(api.InstanceTypeAny, serverFilters, recursion)
			} else {
				instances, err = d.GetInstancesFullWithFilterAndRecursion(api.InstanceTypeAny, serverFilters, recursion)
			}
		} else {
			// Fallback to old API for servers without selective fields support.
			if c.flagAllProjects {
