...
func (c *cmdOIDCSessionList) run(cmd *cobra.Command, args []string) error {
	...
	for _, session := range sessions {
		data = append(data, []string{session.Email, session.Username, session.UUID, session.IP, session.UserAgent, session.CreationDate.String(), session.ExpiryDate.String()})
	}
