...
func (r *ProtocolLXD) GetClusterLinks() ([]api.ClusterLink, error) {
	...
	_, err = r.queryStruct(http.MethodGet, api.NewURL().Path("clusters").WithQuery("recursion", "1").String(), nil, "", &clusterLinks)
	if err != nil {
		return nil, err
	}
...
func (r *ProtocolLXD) GetClusterLink(name string) (*api.ClusterLink, string, error) {
	...
	etag, err := r.queryStruct(http.MethodGet, api.NewURL().Path("clusters", name).String(), nil, "", &clusterLink)
	if err != nil {
		return nil, "", err
	}
...
func (r *ProtocolLXD) CreateClusterLink(clusterLink api.ClusterLinkPost) error {
	...
	_, _, err = r.query(http.MethodPost, api.NewURL().Path("clusters").String(), clusterLink, "")
	if err != nil {
		return err
	}
...
func (r *ProtocolLXD) CreateIdentityClusterLinkToken(clusterLink api.ClusterLinkPost) (*api.CertificateAddToken, error) {
	...
	_, err = r.queryStruct(http.MethodPost, api.NewURL().Path("clusters").String(), clusterLink, "", &token)
	if err != nil {
		return nil, err
	}
...
func (r *ProtocolLXD) UpdateClusterLink(name string, clusterLink api.ClusterLinkPut, ETag string) error {
	...
	_, _, err = r.query(http.MethodPut, api.NewURL().Path("clusters", name).String(), clusterLink, ETag)
	if err != nil {
		return err
	}
...
func (r *ProtocolLXD) DeleteClusterLink(name string) error {
	...
	_, _, err = r.query(http.MethodDelete, api.NewURL().Path("clusters", name).String(), nil, "")
	if err != nil {
		return err
	}
...
// CheckClusterLinkAddress checks if a cluster link address is reachable and returns its metadata.
func (r *ProtocolLXD) CheckClusterLinkAddress(name string, address string) (*api.Response, error) {
	err := r.CheckExtension("cluster_links")
	if err != nil {
		return nil, err
	}

	reqURL := "http://" + address + "/1.0"
	resp, _, err := r.rawQuery(http.MethodGet, reqURL, nil, "")
	if err != nil {
		return nil, err
	}
