...
// CheckClusterLink checks if a cluster link's addresses are reachable.
func (r *ProtocolLXD) CheckClusterLink(name string, addresses []string) (map[string]bool, error) {
	err := r.CheckExtension("cluster_links")
	if err != nil {
		return nil, err
	}

	addressesReachable := make(map[string]bool)
	for _, address := range addresses {
		reqURL := "http://" + address + "/1.0"
		logger.Errorf("%s", reqURL)
		resp, _, err := r.rawQuery(http.MethodGet, reqURL, nil, "")
		if err != nil {
			return nil, err
		}

		addressesReachable[address] = resp.StatusCode == http.StatusOK
	}

	return addressesReachable, nil
}
