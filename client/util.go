	...
	if !insecureSkipVerify && tlsServerCert == "" && serverCertFingerprint != "" {
		...
		tlsConfig.VerifyPeerCertificate = func(rawCerts [][]byte, _ [][]*x509.Certificate) error {
			...
			if now.Before(serverCert.NotBefore) {
				return fmt.Errorf("Server certificate is not valid at %s (valid after %s)", now.Format(time.RFC3339), serverCert.NotBefore.Format(time.RFC3339))
			}

			if now.After(serverCert.NotAfter) {
				return fmt.Errorf("Server certificate is not valid at %s (expired at %s)", now.Format(time.RFC3339), serverCert.NotAfter.Format(time.RFC3339))
			}

			// Verify fingerprint.
			fingerprint := shared.CertFingerprint(serverCert)
			if !strings.EqualFold(fingerprint, serverCertFingerprint) {
				return fmt.Errorf("Server certificate fingerprint mismatch: got %s, want %s", fingerprint, serverCertFingerprint)
			}
