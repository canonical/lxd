...
func (r *ProtocolDevLXD) queryOperation(method string, path string, data any, ETag string) (op DevLXDOperation, err error) {
	var apiOp api.DevLXDOperation

	// Send the query.
	_, err = r.queryStruct(method, path, data, ETag, &apiOp)
	if err != nil {
		return nil, err
	}
