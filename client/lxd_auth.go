...
func (r *ProtocolLXD) CreateIdentityTLS(tlsIdentitiesPost api.IdentitiesPostTLS) error {
	err := r.CheckExtension("access_management_tls")
	if err != nil {
		return err
	}
...
// CreateIdentityTLSPending creates a pending TLS identity and returns an identity token that can be used by an untrusted client to set up authentication with LXD.
func (r *ProtocolLXD) CreateIdentityTLSPending(tlsIdentitiesPost api.IdentitiesPostTLS) (*api.IdentityTokenTLS, error) {
	err := r.CheckExtension("access_management_tls")
	if err != nil {
		return nil, err
	}
	...
	var token api.IdentityTokenTLS
