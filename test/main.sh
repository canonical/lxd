...
if [ "${1:-"all"}" != "snap" ] && [ "${1:-"all"}" != "cluster" ]; then
    ...
    run_test test_image_with_exec_output_symlink "image with exec-output symlink"
    ...
    run_test test_filemanip_concurrent_copy "concurrent file manipulations after instance copy"
    ...
    run_test test_backup_nullable_fields "backup nullable BackupConfig fields"
