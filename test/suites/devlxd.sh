...
test_devlxd_volume_management() {
  ...
  for instType in $instTypes; do
    ...
    [ "$(lxc exec "${inst}" --project "${project}" --env DEVLXD_BEARER_TOKEN="${token}" -- devlxd-client storage get-volume "${pool}" image vol)" = "Only custom storage volumes can be retrieved" ]
    [ "$(lxc exec "${inst}" --project "${project}" --env DEVLXD_BEARER_TOKEN="${token}" -- devlxd-client storage get-volume "${pool}" container vol)" = "Only custom storage volumes can be retrieved" ]
    [ "$(lxc exec "${inst}" --project "${project}" --env DEVLXD_BEARER_TOKEN="${token}" -- devlxd-client storage get-volume "${pool}" virtual-machine vol)" = "Only custom storage volumes can be retrieved" ]
