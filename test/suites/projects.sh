...
test_projects_images_volume() {
  ...
  ! lxc config set storage.project_foo.images_volume="${pool}/vol" || false
  ...
  lxc config set storage.project_foo.images_volume="${pool}/vol"

  # It should be possible to change the setting on empty projects
  lxc config unset storage.project_foo.images_volume
  lxc config set storage.project_foo.images_volume="${pool}/vol"
  ...
  ! lxc config unset storage.project_foo.images_volume || false

  # It should be possible to share the volume among multiple projects
  lxc project create foo2
  lxc config set storage.project_foo2.images_volume="${pool}/vol"
  lxc config show | grep -F "storage.project_foo2.images_volume: ${pool}/vol"
  ...
  ! lxc config show | grep -F "storage.project_foo2.images_volume: ${pool}/vol" || false
...
test_projects_backups_volume() {
  ...
  # Can't be set on non-existing projects
  ! lxc config set storage.project_foo.backups_volume="${pool}/vol" || false
  ...
  lxc config set storage.project_foo.images_volume="${pool}/vol"
  lxc config set storage.project_foo.backups_volume="${pool}/vol"

  # It should be possible to change the setting on empty projects
  lxc config unset storage.project_foo.backups_volume
  lxc config set storage.project_foo.backups_volume="${pool}/vol"
