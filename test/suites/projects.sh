...
test_certificate_project_restrictions() {
  ...
  LXD_CONF_TEST=$(mktemp -d -p "${TEST_DIR}" XXX)
  LXD_CONF="${LXD_CONF_TEST}" gen_cert_and_key "client"
  LXD_CONF="${LXD_CONF_TEST}" lxc remote add test-restricted "${trust_token1}"

  # The restricted client should only see the test-cert-project project.
  fingerprint1="$(cert_fingerprint "${LXD_CONF_TEST}/client.crt")"
  cert_info="$(lxc config trust show "${fingerprint1}")"

  # Verify restricted is true.
  echo "${cert_info}" | grep -F "restricted: true"

  # Verify projects list contains test-cert-project.
  echo "${cert_info}" | grep -E "^- test-cert-project"
  ...
  trust_token2="$(lxc config trust add --name test-no-projects --restricted 2>&1)"
  echo "${trust_token2}" | grep -i "Certificate is restricted but no projects specified."

  LXD_CONF_TEST2=$(mktemp -d -p "${TEST_DIR}" XXX)
  LXD_CONF="${LXD_CONF_TEST2}" gen_cert_and_key "client"

  # Extract just the token part if there's a warning.
  token_line="$(echo "${trust_token2}" | tail -n 1)"
  LXD_CONF="${LXD_CONF_TEST2}" lxc remote add test-restricted-empty "${token_line}"

  fingerprint2="$(cert_fingerprint "${LXD_CONF_TEST2}/client.crt")"
  cert_info2="$(lxc config trust show "${fingerprint2}")"

  # Verify restricted is true.
  echo "${cert_info2}" | grep -F "restricted: true"

  # Verify projects list is empty.
  echo "${cert_info2}" | grep -F "projects: []"
  ...
  LXD_CONF="${LXD_CONF_TEST3}" gen_cert_and_key "client"
  ...
  echo "${cert_info3}" | grep -F "restricted: true"
  echo "${cert_info3}" | grep -E "^- test-cert-project"
  ...
  LXD_CONF="${LXD_CONF_TEST4}" gen_cert_and_key "client5"
  ...
  rm -rf "${LXD_CONF_TEST}" "${LXD_CONF_TEST2}" "${LXD_CONF_TEST3}"
