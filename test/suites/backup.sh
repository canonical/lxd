...
test_backup_nullable_fields() {
  ...
  if [ "${lxd_backend}" != "dir" ]; then
    export TEST_UNMET_REQUIREMENT="backup nullable fields test only runs on dir backend, not for ${lxd_backend}"
    return
  fi
...
_backup_nullable_fields_run() {
  ...
  case "${kind}" in
    instance)
      ...
      required=".config .name .snapshots .config.container .config.pool .config.volume .config.volume_snapshots .config.snapshots"
      crossPath=1
      ;;
    vm)
      ...
      nullableList=".config.pools .config.profiles"
      required=".config .name .snapshots .type .config.container .config.pool .config.volume .config.volume_snapshots .config.snapshots"
      crossPath=1
      ;;
    volume)
      ...
      # the name as a CLI argument. .config.container / .config.pool /
      # .config.profiles / .config.snapshots do not apply to volumes;
      # volume snapshots live under .config.volume_snapshots instead.
      nullableScalar=".backend .name .optimized .optimized_header .pool .type .config.version"
      nullableList=".config.pools"
      required=".config .snapshots .config.volume .config.volume_snapshots"
      crossPath=0
      ;;
    *)
  ...
  while IFS=$'\t' read -r outcome mutation; do
    ...
    if [ "${crossPath}" = "1" ]; then
      ...
      printf 'fail\t%s\n' \
        ...
        '.snapshots = ["snap0"] | .config.volume_snapshots = []'
    fi
  )
