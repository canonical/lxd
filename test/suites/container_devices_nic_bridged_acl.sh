test_container_devices_nic_bridged_acl() {
  ...
  # Check egress TCP.
  lxc exec "${ctPrefix}A" -- nc -v -w2 192.0.2.1 53
  lxc exec "${ctPrefix}A" -- nc -v -w2 2001:db8::1 53

  nc -l -p 8080 -q0 -s 192.0.2.1 </dev/null >/dev/null &
  nc -l -p 8080 -q0 -s 2001:db8::1 </dev/null >/dev/null &

  ! lxc exec "${ctPrefix}A" -- nc -v -w2 192.0.2.1 8080 || false
  ! lxc exec "${ctPrefix}A" -- nc -v -w2 2001:db8::1 8080 || false
  ...
  lxc exec "${ctPrefix}A" -- nc -v -w2 192.0.2.1 8080
  lxc exec "${ctPrefix}A" -- nc -v -w2 2001:db8::1 8080
