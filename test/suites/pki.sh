...
EOF
  ...
  (
    ...
    lxc_remote config trust ls pki-lxd: | grep unrestricted
    ...
    lxc_remote config trust ls pki-lxd: | grep restricted
    ...
    token="$(LXD_DIR=${LXD5_DIR} lxc config trust add --name foo -q)"
    ! lxc_remote remote add pki-lxd "${LXD5_ADDR}" --accept-certificate --password "${token}" || false
  ...
  # mode is enabled.
  token="$(LXD_DIR=${LXD5_DIR} lxc config trust add --name foo -q)"
  ! lxc_remote remote add pki-lxd2 "${LXD5_ADDR}" --accept-certificate --password "${token}" || false
