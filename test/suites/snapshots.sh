...
test_snap_multi_volume() {
  ...
  echo "Check restoring fails if an attached volume is missing a snapshot."
  lxc storage volume delete "${poolName}" non-shared/snap0
  ! lxc restore c1 c1/c1-snap1 --disk-volumes=all-exclusive || false

  echo "Recreate missing attached snapshot and take a new multi-volume snapshot."
  lxc storage volume snapshot "${poolName}" non-shared
  ...
  [ "$(lxc storage volume list -f csv | grep -cF "Created alongside container/c1/c1-snap3")" = "0" ]
