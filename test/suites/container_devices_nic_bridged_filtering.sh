test_container_devices_nic_bridged_filtering() {
  ...
  lxc exec "${ctPrefix}A" -- ping -n -c2 -W5 192.0.2.1
  lxc exec "${ctPrefix}A" -- ping -n -c2 -W5 192.0.2.3
  ...
  if lxc exec "${ctPrefix}A" -- ping -n -c2 -W5 192.0.2.1; then
      echo "MAC filter not working to host"
      false
  fi

  # Check that ping is no longer working (i.e its filtered after fake MAC setup).
  if lxc exec "${ctPrefix}A" -- ping -n -c2 -W5 192.0.2.3; then
  ...
  # Check basic connectivity without any filtering.
  lxc exec "${ctPrefix}A" -- ping -n -c2 -W5 192.0.2.1
  lxc exec "${ctPrefix}A" -- ping -n -c2 -W5 2001:db8::1
  ...
  ! lxc exec "${ctPrefix}A" -- ping -n -c2 -W5 192.0.2.1 || false
  ! lxc exec "${ctPrefix}A" -- ping -n -c2 -W5 2001:db8::1 || false
