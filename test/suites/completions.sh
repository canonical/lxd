...
test_completions() {
    ...
    [ "$(complete config device add c1 devname disk '')" = 'boot.,ceph.,initial.,io.,limits.,path.,path=,pool=,propagation=,raw.,readonly=,recursive=,required=,shift=,size.,size=,source.,source=' ]
    ...
    [ "$(complete config device override c1 devname disk '')" = 'boot.,ceph.,initial.,io.,limits.,path.,path=,pool=,propagation=,raw.,readonly=,recursive=,required=,shift=,size.,size=,source.,source=' ]
