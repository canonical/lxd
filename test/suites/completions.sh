...
test_completions() {
    ...
    [ "$(complete config set '')" = 'acme.,backups.,c1,c2,cluster.,core.,images.,instances.,localhost:,loki.,network.,oidc.,storage.,user.' ]
    [ "$(complete config set n)" = 'network.' ]
    [ "$(complete config set c)" = 'c1,c2,cluster.,core.' ]
    [ "$(complete config set l)" = 'localhost:,loki.' ]
    [ "$(complete config set c1 '')" = 'boot.,cloud-init.,cluster.,environment.,limits.,linux.,migration.,placement.,raw.,security.,snapshots.,ubuntu_pro.,user.' ]
    [ "$(complete config set c1 l)" = 'limits.,linux.' ]
    [ "$(complete config set localhost:c1 '')" = 'boot.,cloud-init.,cluster.,environment.,limits.,linux.,migration.,placement.,raw.,security.,snapshots.,ubuntu_pro.,user.' ]
    ...
    [ "$(complete config get '')" = 'acme.,backups.,c1,c2,cluster.,core.,images.,instances.,localhost:,loki.,network.,oidc.,storage.,user.' ]
    [ "$(complete config get n)" = 'network.' ]
    [ "$(complete config get c)" = 'c1,c2,cluster.,core.' ]
    [ "$(complete config get l)" = 'localhost:,loki.' ]
    [ "$(complete config get c1 '')" = 'boot.,cloud-init.,cluster.,environment.,limits.,linux.,migration.,placement.,raw.,security.,snapshots.,ubuntu_pro.,user.' ]
    [ "$(complete config get c1 l)" = 'limits.,linux.' ]
    [ "$(complete config get localhost:c1 '')" = 'boot.,cloud-init.,cluster.,environment.,limits.,linux.,migration.,placement.,raw.,security.,snapshots.,ubuntu_pro.,user.' ]
    ...
    [ "$(complete config device add c1 devname nic network=)" = "$(lxc query /1.0/networks?recursion=1 | jq --join-output --exit-status '[.[].name] | sort | "network=" + join(",network=")')" ]
    ...
    [ "$(complete config device add c1 devname nic nictype=bridged '')" = 'boot.,host_name=,hwaddr=,ipv4.,ipv6.,limits.,mtu=,name=,network=,parent=,queue.,security.,vlan.,vlan=' ]
    [ "$(complete config device add c1 devname nic nictype=ipvlan '')" = 'gvrp=,hwaddr=,ipv4.,ipv6.,mode=,mtu=,name=,parent=,vlan=' ]
    [ "$(complete config device add c1 devname nic nictype=macvlan '')" = 'boot.,gvrp=,hwaddr=,mtu=,name=,network=,parent=,vlan=' ]
    ...
    [ "$(complete config device add c1 devname nic nictype=physical '')" = 'boot.,gvrp=,hwaddr=,mtu=,name=,network=,parent=,vlan=' ]
    ...
    [ "$(complete config device add c1 devname nic nictype=sriov '')" = 'boot.,hwaddr=,mtu=,name=,network=,parent=,security.,vlan=' ]
    ...
    [ "$(complete config device override c1 devname nic network=)" = "$(lxc query /1.0/networks?recursion=1 | jq --join-output --exit-status '[.[].name] | sort | "network=" + join(",network=")')" ]
    ...
    [ "$(complete config device override c1 devname nic nictype=bridged '')" = 'boot.,host_name=,hwaddr=,ipv4.,ipv6.,limits.,mtu=,name=,network=,parent=,queue.,security.,vlan.,vlan=' ]
    [ "$(complete config device override c1 devname nic nictype=ipvlan '')" = 'gvrp=,hwaddr=,ipv4.,ipv6.,mode=,mtu=,name=,parent=,vlan=' ]
    [ "$(complete config device override c1 devname nic nictype=macvlan '')" = 'boot.,gvrp=,hwaddr=,mtu=,name=,network=,parent=,vlan=' ]
    ...
    [ "$(complete config device override c1 devname nic nictype=physical '')" = 'boot.,gvrp=,hwaddr=,mtu=,name=,network=,parent=,vlan=' ]
    ...
    [ "$(complete config device override c1 devname nic nictype=sriov '')" = 'boot.,hwaddr=,mtu=,name=,network=,parent=,security.,vlan=' ]
