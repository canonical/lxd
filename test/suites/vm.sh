test_vm_empty() {
  ...
  # Check that the NIC devices are sorted when restarted.
  lxc restart -f v1
  [ "$(lxc config get v1 volatile.aaa0.bus)" = "4" ]
  [ "$(lxc config get v1 volatile.eth0.bus)" = "5" ]
  ...
  if [ "$(lxc config get --expanded v1 migration.stateful)" = "" ] || [ "${poolDriver}" = "ceph" ]; then
