...
test_vm_pcie_bus() {
  ...
  sub_test "Check shift=true directory shares are not remapped by virtiofsd in VMs"
  # As with security.shifted volumes, a directory share using shift=true must keep its real
  # on-disk ownership inside the VM even when raw.idmap is set, otherwise the file below would
  # appear owned by nobody instead of 123:456.
  ...
  lxc config set v1 raw.idmap="both 1000000 0"
  lxc config device add v1 v1shiftdir disk source="${TEST_DIR}/vm-shift-source" path=/mnt shift=true
  lxc start v1
  waitInstanceReady v1
  lxc exec v1 -- findmnt /mnt -t virtiofs
  [ "$(lxc exec v1 -- stat /mnt/shifted-file -c '%u:%g')" = "123:456" ]
