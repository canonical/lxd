test_waitready() {
  ...
  echo "==> Networks will appear ready after the 60s (+10s) retry window"
  [ "$(lxd waitready --network --storage --timeout 70 2>&1)" = "Error: Storage pools not ready yet after 70s timeout" ]
