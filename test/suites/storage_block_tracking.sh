...
# _nbd_export serves a single storage volume over NBD, see _nbd_serve.
_nbd_export() {
  _nbd_serve storage volume nbd "$@"
}
...
test_storage_block_tracking_vm() {
  ...
  local pool orig_volume_size count root_dev root_size root_copy checksum address first_pid holder_pid operation_uuid blk_dev blk_size blk_checksum root_all_copy blk_copy
  ...
  lxc storage volume bitmap list "${pool}" virtual-machine/v1 --format csv | grep -wF "bm1"
  ...
  sub_test "A second NBD export of the same volume is refused"
  # A raw connection keeps the first session and its NBD lock alive while a second export is attempted.
  _nbd_export "${pool}" virtual-machine/v1
  first_pid="${NBD_PID}"
  address="${NBD_URI#nbd://}"
  ...
  ! wait "${NBD_PID}" || false
  [[ "$(cat "${NBD_STDERR}")" == 'Error: Operation "'*'" (Exporting storage volume over NBD) is already running for instance "v1"'* ]]
  ...
  sleep 60 | nc "${address%:*}" "${address##*:}" &
  holder_pid=$!
  [[ "$(! "${_LXC}" storage volume set "${pool}" virtual-machine/v1 security.shared=true 2>&1 1>/dev/null)" == 'Error: Operation "'*'" (Exporting storage volume over NBD) is already running for instance "v1"' ]]
  kill "${NBD_PID}" 2>/dev/null || true
  wait "${NBD_PID}" || true
  kill "${holder_pid}"
  wait "${holder_pid}" || true
  ...
  lxc storage volume bitmap list "${pool}" cbt-blk --format csv | grep -wF "bm1"
  ...
  lxc bitmap v1 bm2
  lxc storage volume bitmap list "${pool}" virtual-machine/v1 --format csv | grep -wF "bm2"
  lxc storage volume bitmap list "${pool}" cbt-blk --format csv | grep -wF "bm2"
  ...
  sub_test "Instance-wide NBD export serves every disk under its device name"
  ...
  # Every non-shared block disk is listed under an export named after its LXD device name, with its own size.
  _nbd_serve nbd v1
  nbdinfo --list --json "${NBD_URI}" | jq --exit-status --argjson root "${root_size}" --argjson blk "${blk_size}" '
    ([.exports[] | select(."export-name" == "root")][0]."export-size" == $root)
    and ([.exports[] | select(."export-name" == "cbt-blk")][0]."export-size" == $blk)'
  wait "${NBD_PID}"

  # Selecting the root export reads every block through its overlay and yields the size of the root disk.
  root_all_copy="$(mktemp -p "${TEST_DIR}" root_all_copy.XXX)"
  _nbd_serve nbd v1
  nbdcopy --connections=1 "${NBD_URI}/root" "${root_all_copy}"
  wait "${NBD_PID}"
  [ "$(stat -c %s "${root_all_copy}")" = "${root_size}" ]

  # Selecting the custom disk export yields the guest's own view of the volume, so the transactional overlay
  # served its real data.
