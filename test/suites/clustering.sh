...
test_clustering_image_refresh() {
  ...
  old_fingerprint="$(LXD_DIR="${LXD_REMOTE_DIR}" lxc image info testimage | grep -F 'Fingerprint:' | awk '{print $2}')"
  ...
  new_fingerprint="$(LXD_DIR="${LXD_REMOTE_DIR}" lxc image info testimage | grep -F 'Fingerprint:' | awk '{print $2}')"
