...
test_clustering_storage() {
  ...
  if [ "${poolDriver}" = "dir" ]; then
    ...
    LXD_DIR="${LXD_ONE_DIR}" lxc config set storage.project_foo.images_volume=pool1/vol1
    LXD_DIR="${LXD_TWO_DIR}" lxc config set storage.project_foo.images_volume=pool1/vol1
    LXD_DIR="${LXD_ONE_DIR}" lxc config set storage.project_foo.backups_volume=pool1/vol1
    LXD_DIR="${LXD_TWO_DIR}" lxc config set storage.project_foo.backups_volume=pool1/vol1
    LXD_DIR="${LXD_ONE_DIR}" lxc config show | grep -F "storage.project_foo.images_volume: pool1/vol1"
    LXD_DIR="${LXD_TWO_DIR}" lxc config show | grep -F "storage.project_foo.images_volume: pool1/vol1"
    LXD_DIR="${LXD_ONE_DIR}" lxc config show | grep -F "storage.project_foo.backups_volume: pool1/vol1"
    LXD_DIR="${LXD_TWO_DIR}" lxc config show | grep -F "storage.project_foo.backups_volume: pool1/vol1"
    ! LXD_DIR="${LXD_ONE_DIR}" lxc storage volume delete pool1 vol1 --target=node1 || false
    LXD_DIR="${LXD_ONE_DIR}" lxc config unset storage.project_foo.images_volume
    LXD_DIR="${LXD_TWO_DIR}" lxc config unset storage.project_foo.images_volume
    LXD_DIR="${LXD_ONE_DIR}" lxc config unset storage.project_foo.backups_volume
    LXD_DIR="${LXD_TWO_DIR}" lxc config unset storage.project_foo.backups_volume
