...
test_clustering_roles() {
  ...
  LXD_DIR="${LXD_ONE_DIR}" lxc cluster list -f json | jq --exit-status '[.[] | select(.server_name == "node4" or .server_name == "node5") | select(any(.roles[]; test("database-(leader|voter|standby)")))] | .[0].server_name // empty'
  ...
  LXD_DIR="${LXD_ONE_DIR}" lxc cluster list -f json | jq --exit-status '[.[] | select(any(.roles[]; . == "control-plane"))] | length == 2'
  ...
  LXD_DIR="${LXD_ONE_DIR}" lxc cluster list -f json | jq --exit-status '[.[] | select(any(.roles[]; contains("database-voter") or contains("database-leader")))] | length == 3'
