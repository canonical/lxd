...
test_clustering_storage() {
  ...
  if [ "${poolDriver}" = "dir" ]; then
    ...
    LXD_DIR="${LXD_ONE_DIR}" lxc config show | grep -F "storage.project.foo.images_volume: pool1/vol1"
    LXD_DIR="${LXD_TWO_DIR}" lxc config show | grep -F "storage.project.foo.images_volume: pool1/vol1"
    LXD_DIR="${LXD_ONE_DIR}" lxc config show | grep -F "storage.project.foo.backups_volume: pool1/vol1"
    LXD_DIR="${LXD_TWO_DIR}" lxc config show | grep -F "storage.project.foo.backups_volume: pool1/vol1"
