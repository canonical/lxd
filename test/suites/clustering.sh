...
test_clustering_link() {
  ...
  [ "$(lxc cluster link list --format csv | grep -cF "$(cat "${TEST_DIR}/lxd_one_address")")" = 1 ]
  ...
  [ "$(LXD_DIR="${LXD_ONE_DIR}" lxc cluster link list --format csv | grep -cF "$(cat "${TEST_DIR}/lxd_two_address")")" = 1 ]
