...
test_clustering_openfga() {
  ...
  # Set up candid authentication using node1. It is not possible to test OpenFGA with TLS authentication, as this
  # will revert to the TLS authorization driver.
  identity_endpoint="http://$(cat "${TEST_DIR}/rbac.addr")/auth"
  LXD_DIR="${LXD_ONE_DIR}" lxc config set candid.api.url "${identity_endpoint}"

  key="$(curl -s "${identity_endpoint}/discharge/info" | jq -r .PublicKey)"
  LXD_DIR="${LXD_ONE_DIR}" lxc config set candid.api.key "${key}"
...
pass1
EOF
  ) | lxc remote add candid-openfga "https://10.1.1.101:8443" --auth-type candid --accept-certificate
  ...
  LXD_DIR="${LXD_ONE_DIR}" lxc config set candid-openfga: openfga.api.url "$(fga_address)"
  LXD_DIR="${LXD_ONE_DIR}" lxc config set candid-openfga: openfga.api.token "$(fga_token)"
  LXD_DIR="${LXD_ONE_DIR}" lxc config set candid-openfga: openfga.store.id "${OPENFGA_STORE_ID}"
  sleep 1
...
pass1
EOF
  ) | lxc remote add node2 "https://10.1.1.102:8443" --auth-type candid --accept-certificate
  ! lxc_remote info node2: | grep -Fq 'core.https_address' || false
  ...
  lxc_remote info node2: | grep -Fq 'core.https_address'
