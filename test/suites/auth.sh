...
events_filtering() {
  ...
  # Monitor as fine-grained identity with can_view_lifecycle_events in the default project.
  lxc auth group permission add test-group project default can_view_lifecycle_events
  lxc remote switch tls
  lxc monitor --all-projects --format json > "${monfile}" &
  monitor_pid=$!
  sleep 0.1
  ...
  # has can_view_lifecycle_events, but is not the same caller that started the operation.
  jq -s -e 'length == 1 and .[0].type == "lifecycle" and .[0].metadata.action == "profile-created"' "${monfile}"
  lxc profile delete p1
  rm "${monfile}"
  lxc auth group permission remove test-group project default can_view_lifecycle_events
