...
func NewNetworkZoneNode(name string, data api.NetworkZone, rank uint) *NetworkZoneNode {
	...
	return &NetworkZoneNode{
		baseNode: baseNode{
			data:    data,
			rank:    rank,
			id:      composeHashes(hash(name), hash(data)),
...
func NewNetworkZoneRecordNode(zoneName string, name string, data api.NetworkZoneRecord, rank uint) *NetworkZoneRecordNode {
	return &NetworkZoneRecordNode{
		baseNode: baseNode{
			data:    data,
			rank:    rank,
			id:      composeHashes(hash(zoneName), hash(name), hash(data)),
...
func NewNetworkPeerNode(project string, networkName string, peerName string, data api.NetworkPeer, rank uint) *NetworkPeerNode {
	// We don't need this field to represent the inner data in the graph.
	// The 'used by' relationships are already represented and exploitable in the graph topology.
	data.UsedBy = nil
	return &NetworkPeerNode{
		baseNode: baseNode{
			data:    data,
			rank:    rank,
			id:      composeHashes(hash(project), hash(networkName), hash(peerName), hash(data)),
...
func NewNetworkACLNode(project string, aclName string, data api.NetworkACL, rank uint) *NetworkACLNode {
	data.UsedBy = nil
	return &NetworkACLNode{
		baseNode: baseNode{
			data:    data,
			rank:    rank,
			id:      composeHashes(hash(project), hash(aclName), hash(data)),
...
func NewNetworkForwardNode(project string, networkName string, data api.NetworkForward, rank uint) *NetworkForwardNode {
	id := composeHashes(hash(project), hash(networkName), hash(data))
...
func NewNetworkLBNode(project string, networkName string, data api.NetworkLoadBalancer, rank uint) *NetworkLBNode {
	id := composeHashes(hash(project), hash(networkName), hash(data))
...
func NewNetworkNode(project string, name string, data api.Network, rank uint) *NetworkNode {
	// We don't need this field to represent the inner data in the graph.
	// The 'used by' relationships are already represented and exploitable in the graph topology.
	data.UsedBy = nil
	return &NetworkNode{
		baseNode: baseNode{
			data:    data,
			rank:    rank,
			id:      composeHashes(hash(project), hash(name), hash(data)),
