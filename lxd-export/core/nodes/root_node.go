...
func (r RootNode) Diff(n any) (diff.Changelog, error) {
	...
	// If the IDs are the same, the nodes are the same (because ID is an FNV hash of the data).
	if r.id == other.id {
		return nil, nil
	}
...
func NewRootNode(client lxd.InstanceServer, rank uint) (*RootNode, error) {
...
func NewRootNodeFromData(data RootData, rank uint) *RootNode {
	return &RootNode{
		baseNode: baseNode{
			data:    data,
			rank:    rank,
