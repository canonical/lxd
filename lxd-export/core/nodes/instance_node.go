...
func NewDeviceNode(project string, location string, deviceType string, deviceName string, device map[string]string, rank uint) *DeviceNode {
	return &DeviceNode{
		baseNode: baseNode{
			data:    device,
			rank:    rank,
			id:      composeHashes(hash(project), hash(location), hash(deviceType), hash(deviceName), hash(device)),
...
func NewProfileNode(project string, name string, profile api.Profile, rank uint) *ProfileNode {
	return &ProfileNode{
		baseNode: baseNode{
			data:    profile,
			rank:    rank,
			id:      composeHashes(hash(project), hash(name), hash(profile)),
...
func NewInstanceNode(project string, name string, instance api.Instance, rank uint) *InstanceNode {
	return &InstanceNode{
		baseNode: baseNode{
			data:    instance,
			rank:    rank,
			id:      composeHashes(hash(project), hash(name), hash(instance)),
