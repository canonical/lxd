...
type Node interface {
	...
	// Rank is used to specify at which 'level' a node is in a DAG. During the import process, when the graph edit distance
	// is computed, 'edits' between nodes at the same rank can be executed concurrently.
	Rank() uint
	Data() any
