...
func NewEdit(desc string, rank int, f func() error) Edit {
	return Edit{
		Desc: desc,
		Rank: rank,
		Fn:   f,
...
func (p *Plan) String() string {
	...
	for _, edit := range p.Steps {
		if edit.Rank != localRank {
			out += "<EXECUTION BARRIER>\n"
		}

		localRank = edit.Rank
		out += fmt.Sprintf("- (rank: %d) %s\n", edit.Rank, edit.Desc)
	}
...
func (p *Planner) rootNodeEdits(d diff.Changelog, tRoot *nodes.RootNode) (edits []Edit, diags Diagnostics, err error) {
	...
	edits = append(edits, NewEdit("Update global server config", 0, func() error {
	...
	for serverName, serverPut := range localServerPuts {
		edits = append(edits, NewEdit(fmt.Sprintf("Update server %q config", serverName), 0, func() error {
	...
	for _, group := range clusterGroupsToAdd {
		edits = append(edits, NewEdit(fmt.Sprintf("Add cluster group %q", group.Name), 0, func() error {
	...
	for _, group := range clusterGroupsToDelete {
		edits = append(edits, NewEdit(fmt.Sprintf("Delete cluster group %q", group), 0, func() error {
	...
	for name, group := range clusterGroupsToUpdate {
		edits = append(edits, NewEdit(fmt.Sprintf("Update cluster group %q", name), 0, func() error {
	...
	for name, newName := range clusterGroupsToRename {
		edits = append(edits, NewEdit(fmt.Sprintf("Rename cluster group %q to %q", name, newName), 1, func() error {
	...
	for _, change := range clusterMemberChanges {
		idxClusterMem, err := strconv.Atoi(change.Path[2])
		if err != nil {
			return nil, nil, err
		}
	...
	for serverName, memberPut := range clusterMemberPuts {
		edits = append(edits, NewEdit(fmt.Sprintf("Update cluster member %q", serverName), 2, func() error {
	...
	for serverName, newName := range clusterMemberRename {
		edits = append(edits, NewEdit(fmt.Sprintf("Rename cluster member %q to %q", serverName, newName), 3, func() error {
...
func (p *Planner) compareRootNodes(plan *Plan) error {
	...
	if d != nil {
		...
		rootSteps, diags, err := p.rootNodeEdits(d, &tRoot)
		if err != nil {
			return err
		}
...
func (p *Planner) GeneratePlan() (*Plan, error) {
	...
	err := p.compareRootNodes(plan)
	if err != nil {
		return nil, err
	}

	// TODO: Then compare:
	// source project set to target project set
	// source storage pool set to target storage pool set
	// ...
