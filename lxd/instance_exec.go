...
func instanceExecPost(d *Daemon, r *http.Request) response.Response {
	...
	if post.WaitForWS {
		...
		op, err := operations.CreateUserOperation(s, projectName, requestor, operations.OperationClassWebsocket, operationtype.CommandExec, resources, ws.Metadata(), ws.Do, nil, ws.Connect)
		if err != nil {
			return response.InternalError(err)
		}
	...
	op, err := operations.CreateUserOperation(s, projectName, requestor, operations.OperationClassTask, operationtype.CommandExec, resources, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
