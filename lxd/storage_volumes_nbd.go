...
// storagePoolVolumeTypeNBDConnect opens the NBD connection to the volume on the local member and returns the
// conflict reference of the session, which is empty when an existing session is reused.
func storagePoolVolumeTypeNBDConnect(s *state.State, details storageVolumeDetails, effectiveProjectName string, writable bool, reuse bool) (net.Conn, func(), string, error) {
	if details.volumeType == cluster.StoragePoolVolumeTypeVM {
...
// the operations API, and the 101 response names the operation in its Location header. The operation takes the
// conflict reference of the session, so that no other cluster member can export the same volume at the same time.
func storagePoolVolumeTypeNBDServe(s *state.State, r *http.Request, details storageVolumeDetails, effectiveProjectName string, opType operationtype.Type, writable bool, reuse bool) response.Response {
	conn, cleanup, conflictReference, err := storagePoolVolumeTypeNBDConnect(s, details, effectiveProjectName, writable, reuse)
	if err != nil {
