...
func imageCreateInPool(s *state.State, info *api.Image, storagePool string, project string) error {
	if storagePool == "" {
	...
	err = pool.EnsureImage(info.Fingerprint, nil, project)
	if err != nil {
		return err
	}
	...
	for _, node := range nodes {
		...
		err = func() error {
			...
			if err != nil {
				...
				val := resp.Config["storage.project_"+newImage.Project+".images_volume"]
				if val == nil {
...
func imageDelete(d *Daemon, r *http.Request) response.Response {
	...
	do := func(op *operations.Operation) error {
		...
		err = imageDeleteFromDisk(s, projectName, details.image.Fingerprint)
		if err != nil {
			return err
		}
...
func imageDeleteFromDisk(s *state.State, imageVolume string, fingerprint string) error {
	// Remove main image file.
	fname := filepath.Join(s.ImagesStoragePath(imageVolume), fingerprint)
	if shared.PathExists(fname) {
		err := os.Remove(fname)
