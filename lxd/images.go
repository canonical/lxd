...
func getImgPostInfo(d *Daemon, r *http.Request, builddir string, project string, post *os.File, metadata map[string]any) (*api.Image, error) {
	...
	if exists {
		...
		// Create the database entry
		err = d.db.Cluster.CreateImage(project, info.Fingerprint, info.Filename, info.Size, info.Public, info.AutoUpdate, info.Architecture, info.CreatedAt, info.ExpiresAt, info.Properties, info.Type)
		if err != nil {
			return nil, err
		}
	}
...
func imageValidSecret(d *Daemon, r *http.Request, projectName string, fingerprint string, secret string) (*api.Operation, error) {
	ops, err := operationsGetByType(d.State(), r, projectName, operationtype.ImageToken)
	if err != nil {
	...
	for _, op := range ops {
		...
		if opSecret == secret {
			...
			err = operationCancel(d.State(), r, projectName, op)
			if err != nil {
