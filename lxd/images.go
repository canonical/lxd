...
func imagesPost(d *Daemon, r *http.Request) Response {
	...
	run := func(op *operation) error {
		...
		for _, alias := range req.Aliases {
			_, _, err := db.ImageAliasGet(d.nodeDB, alias.Name, true)
			if err == nil {
				return fmt.Errorf("Alias already exists: %s", alias.Name)
			}

			id, _, err := db.ImageGet(d.nodeDB, info.Fingerprint, false, false)
			if err != nil {
				return err
			}

			err = db.ImageAliasAdd(d.nodeDB, alias.Name, id, alias.Description)
			if err != nil {
				return err
			}
		}
...
func autoUpdateImage(d *Daemon, op *operation, id int, info *api.Image) error {
	...
	poolIDs, err := db.ImageGetPools(d.nodeDB, fingerprint)
	if err != nil {
	...
	poolNames, err := db.ImageGetPoolNamesFromIDs(d.nodeDB, poolIDs)
	if err != nil {
		logger.Error("Error getting image pools", log.Ctx{"err": err, "fp": fingerprint})
		return err
	}
...
func pruneExpiredImages(d *Daemon) {
	...
	for _, fp := range images {
		// Get the IDs of all storage pools on which a storage volume
		// for the requested image currently exists.
		poolIDs, err := db.ImageGetPools(d.nodeDB, fp)
		if err != nil {
			continue
		}
		...
		imgID, _, err := db.ImageGet(d.nodeDB, fp, false, false)
		if err != nil {
			logger.Debugf("Error retrieving image info %s: %s", fp, err)
		}

		// Remove the database entry for the image.
		if err = db.ImageDelete(d.nodeDB, imgID); err != nil {
...
func imageDelete(d *Daemon, r *http.Request) Response {
	...
	deleteFromAllPools := func() error {
		...
		imgID, imgInfo, err := db.ImageGet(d.nodeDB, fingerprint, false, false)
		if err != nil {
			return err
		}

		poolIDs, err := db.ImageGetPools(d.nodeDB, imgInfo.Fingerprint)
		if err != nil {
			return err
		}

		pools, err := db.ImageGetPoolNamesFromIDs(d.nodeDB, poolIDs)
		if err != nil {
			return err
		}
		...
		// Remove the database entry for the image.
		return db.ImageDelete(d.nodeDB, imgID)
	}
...
func imagePatch(d *Daemon, r *http.Request) Response {
	...
	err = db.ImageUpdate(d.nodeDB, id, info.Filename, info.Size, info.Public, info.AutoUpdate, info.Architecture, info.CreatedAt, info.ExpiresAt, info.Properties)
	if err != nil {
		return SmartError(err)
	}
...
func aliasesGet(d *Daemon, r *http.Request) Response {
	...
	for _, res := range results {
		...
		if !recursion {
			...
			_, alias, err := db.ImageAliasGet(d.nodeDB, name, d.checkTrustedClient(r) == nil)
			if err != nil {
				continue
			}
...
func aliasPut(d *Daemon, r *http.Request) Response {
	// Get current value
	name := mux.Vars(r)["name"]
	id, alias, err := db.ImageAliasGet(d.nodeDB, name, true)
	if err != nil {
		return SmartError(err)
	}
...
func aliasPatch(d *Daemon, r *http.Request) Response {
	...
	imageId, _, err := db.ImageGet(d.nodeDB, alias.Target, false, false)
	if err != nil {
		return SmartError(err)
	}

	err = db.ImageAliasUpdate(d.nodeDB, id, imageId, alias.Description)
	if err != nil {
		return SmartError(err)
	}
...
func imageRefresh(d *Daemon, r *http.Request) Response {
	fingerprint := mux.Vars(r)["fingerprint"]
	imageId, imageInfo, err := db.ImageGet(d.nodeDB, fingerprint, false, false)
	if err != nil {
		return SmartError(err)
	}
