...
func imgPostInstanceInfo(s *state.State, r *http.Request, req api.ImagesPost, op *operations.Operation, builddir string, budget int64) (*api.Image, error) {
	...
	err = s.DB.Cluster.CreateImage(c.Project().Name, info.Fingerprint, info.Filename, info.Size, info.Public, info.AutoUpdate, info.Architecture, info.CreatedAt, info.ExpiresAt, info.Properties, info.Type)
	if err != nil {
		return nil, err
	}
...
func getImgPostInfo(s *state.State, r *http.Request, builddir string, project string, post *os.File, metadata map[string]any) (*api.Image, error) {
	...
	if exists {
		...
		// Create the database entry
		err = s.DB.Cluster.CreateImage(project, info.Fingerprint, info.Filename, info.Size, info.Public, info.AutoUpdate, info.Architecture, info.CreatedAt, info.ExpiresAt, info.Properties, info.Type)
		if err != nil {
			return nil, err
		}
	}
...
func imageGet(d *Daemon, r *http.Request) response.Response {
	...
	op, err := imageValidSecret(d.State(), r, projectName, info.Fingerprint, secret)
	if err != nil {
		return response.SmartError(err)
	}
...
func imageExport(d *Daemon, r *http.Request) response.Response {
	...
	if r.RemoteAddr == "@devlxd" {
		...
		op, err := imageValidSecret(d.State(), r, projectName, imgInfo.Fingerprint, secret)
		if err != nil {
			return response.SmartError(err)
		}
...
func imageRefresh(d *Daemon, r *http.Request) response.Response {
	...
	run := func(op *operations.Operation) error {
		...
		newImage, err := autoUpdateImage(d.shutdownCtx, s, op, imageID, imageInfo, projectName, true)
		if err != nil {
		...
		if newImage != nil {
			if len(nodes) > 1 {
				err := distributeImage(d.shutdownCtx, s, nodes, fingerprint, newImage)
				if err != nil {
...
func autoSyncImagesTask(d *Daemon) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		opRun := func(op *operations.Operation) error {
			return autoSyncImages(ctx, d.State())
		}
