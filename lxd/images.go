...
func imagesPost(d *Daemon, r *http.Request) response.Response {
	...
	imageOp, err := operations.CreateUserOperation(s, dbProject.Name, requestor, operations.OperationClassTask, operationtype.ImageDownload, nil, metadata, run, nil, nil)
	if err != nil {
		cleanup(builddir, post)
		return response.InternalError(err)
	}
...
func autoUpdateImagesTask(stateFunc func() *state.State) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		op, err := operations.CreateServerOperation(s, "", operations.OperationClassTask, operationtype.ImagesUpdate, nil, nil, opRun, nil, nil)
		if err != nil {
...
func pruneExpiredImagesTask(stateFunc func() *state.State) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		op, err := operations.CreateServerOperation(s, "", operations.OperationClassTask, operationtype.ImagesExpire, nil, nil, opRun, nil, nil)
		if err != nil {
...
func pruneLeftoverImages(s *state.State) {
	...
	op, err := operations.CreateServerOperation(s, "", operations.OperationClassTask, operationtype.ImagesPruneLeftover, nil, nil, opRun, nil, nil)
	if err != nil {
...
func doImageDelete(ctx context.Context, s *state.State, fingerprint string, imageID int, projectName string) (*operations.Operation, error) {
	...
	op, err := operations.CreateUserOperation(s, projectName, requestor, operations.OperationClassTask, operationtype.ImageDelete, resources, nil, do, nil, nil)
	if err != nil {
		return nil, err
	}
...
func imageExportPost(d *Daemon, r *http.Request) response.Response {
	...
	op, err := operations.CreateUserOperation(s, projectName, requestor, operations.OperationClassTask, operationtype.ImageDownload, nil, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
...
func imageRefresh(d *Daemon, r *http.Request) response.Response {
	...
	op, err := operations.CreateUserOperation(s, projectName, requestor, operations.OperationClassTask, operationtype.ImageRefresh, nil, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
...
func autoSyncImagesTask(stateFunc func() *state.State) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		op, err := operations.CreateServerOperation(s, "", operations.OperationClassTask, operationtype.ImagesSynchronize, nil, nil, opRun, nil, nil)
		if err != nil {
...
func createTokenResponse(s *state.State, r *http.Request, projectName string, fingerprint string, metadata shared.Jmap) response.Response {
	...
	op, err := operations.CreateUserOperation(s, projectName, requestor, operations.OperationClassToken, operationtype.ImageToken, resources, meta, nil, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
