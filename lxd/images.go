...
// imgPostRemoteInfo processes and updates remote image information in the database.
...
// imgPostURLInfo processes and updates remote image information in the database.
...
// getImgPostInfo processes the received image data and updates the image information in the database.
...
// doImagesGet retrieves image information based on filters, allowing recursive retrieval if needed.
...
// imagesGet handles image retrieval with filtering, considering project permissions and recursion requests.
...
// autoUpdateImagesTask schedules and executes a task to automatically update images in the background.
...
// autoUpdateImages automates image updates in the cluster, distributing updates to other members and removing old images from the database.
...
// Updates a single image. The operation can be nil, if no progress tracking is needed.
...
// pruneExpiredImagesTask schedules regular pruning of expired images with an initial synchronous pruning during startup.
...
// pruneLeftoverImages cleans up leftover image files from the images directory
...
// pruneExpiredImages prunes expired cached images and their records from the database, and deletes their corresponding files and storage volumes.
...
// imageDeleteFromDisk deletes an image file from the local images directory.
