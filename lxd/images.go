...
func imgPostInstanceInfo(d *Daemon, r *http.Request, req api.ImagesPost, op *operations.Operation, builddir string, budget int64) (*api.Image, error) {
	...
	_, _, err = d.db.Cluster.GetImage(info.Fingerprint, dbCluster.ImageFilter{Project: []string{projectName}})
...
func imgPostRemoteInfo(d *Daemon, r *http.Request, req api.ImagesPost, op *operations.Operation, project string, budget int64) (*api.Image, error) {
	...
	id, info, err := d.db.Cluster.GetImage(info.Fingerprint, dbCluster.ImageFilter{Project: []string{project}})
	if err != nil {
		return nil, err
	}
...
func imagesPost(d *Daemon, r *http.Request) response.Response {
	...
	run := func(op *operations.Operation) error {
		...
		for _, alias := range req.Aliases {
			...
			id, _, err := d.db.Cluster.GetImage(info.Fingerprint, dbCluster.ImageFilter{Project: []string{projectName}})
			if err != nil {
...
func autoUpdateImages(ctx context.Context, d *Daemon) error {
	...
	err := d.db.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		autoUpdate := true
		images, err := dbCluster.GetImages(ctx, tx.Tx(), dbCluster.ImageFilter{AutoUpdate: []bool{autoUpdate}})
		if err != nil {
			return err
		}
	...
	for fingerprint, images := range imageMap {
		...
		for _, image := range images {
			filter := dbCluster.ImageFilter{Project: []string{image.Project}}
			if image.Public {
				filter.Public = []bool{image.Public}
			}
...
func autoUpdateImage(ctx context.Context, d *Daemon, op *operations.Operation, id int, info *api.Image, projectName string, manual bool) (*api.Image, error) {
	...
	for _, poolName := range poolNames {
		...
		newID, _, err := d.db.Cluster.GetImage(hash, dbCluster.ImageFilter{Project: []string{projectName}})
		if err != nil {
...
func pruneExpiredImagesInProject(ctx context.Context, d *Daemon, project api.Project, op *operations.Operation) error {
	...
	for _, fingerprint := range images {
		...
		imgID, _, err := d.db.Cluster.GetImage(fingerprint, dbCluster.ImageFilter{Project: []string{project.Name}})
		if err != nil {
...
func imageDelete(d *Daemon, r *http.Request) response.Response {
	...
	imgID, imgInfo, err := d.db.Cluster.GetImage(fingerprint, dbCluster.ImageFilter{Project: []string{projectName}})
	if err != nil {
		return response.SmartError(err)
	}
...
func doImageGet(cluster *db.Cluster, project, fingerprint string, public bool) (*api.Image, response.Response) {
	filter := dbCluster.ImageFilter{Project: []string{project}}
	if public {
		filter.Public = []bool{public}
	}
...
func imagePut(d *Daemon, r *http.Request) response.Response {
	...
	id, info, err := d.db.Cluster.GetImage(fingerprint, dbCluster.ImageFilter{Project: []string{projectName}})
	if err != nil {
		return response.SmartError(err)
	}
...
func imageAliasesPost(d *Daemon, r *http.Request) response.Response {
	...
	id, _, err := d.db.Cluster.GetImage(req.Target, dbCluster.ImageFilter{Project: []string{projectName}})
	if err != nil {
		return response.SmartError(err)
	}
...
func imageAliasPut(d *Daemon, r *http.Request) response.Response {
	...
	imageID, _, err := d.db.Cluster.GetImage(req.Target, dbCluster.ImageFilter{Project: []string{projectName}})
	if err != nil {
		return response.SmartError(err)
	}
...
func imageAliasPatch(d *Daemon, r *http.Request) response.Response {
	...
	imageID, _, err := d.db.Cluster.GetImage(alias.Target, dbCluster.ImageFilter{Project: []string{projectName}})
	if err != nil {
		return response.SmartError(err)
	}
...
func imageExport(d *Daemon, r *http.Request) response.Response {
	...
	if r.RemoteAddr == "@devlxd" {
		...
		_, imgInfo, err = d.db.Cluster.GetImage(fingerprint, dbCluster.ImageFilter{Project: []string{projectName}})
		if err != nil {
			return response.SmartError(err)
		}
...
func imageExportPost(d *Daemon, r *http.Request) response.Response {
	...
	_, _, err = d.db.Cluster.GetImage(fingerprint, dbCluster.ImageFilter{Project: []string{projectName}})
	if err != nil {
		return response.SmartError(err)
	}
...
func imageRefresh(d *Daemon, r *http.Request) response.Response {
	...
	imageID, imageInfo, err := d.db.Cluster.GetImage(fingerprint, dbCluster.ImageFilter{Project: []string{projectName}})
	if err != nil {
		return response.SmartError(err)
	}
...
func imageSyncBetweenNodes(d *Daemon, r *http.Request, project string, fingerprint string) error {
	...
	// Get the image.
	_, image, err := d.db.Cluster.GetImage(fingerprint, dbCluster.ImageFilter{Project: []string{project}})
	if err != nil {
