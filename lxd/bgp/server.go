...
// Initializes the BGP server and starts its associated routines.
func (s *Server) setup() {
	if s.bgp != nil {
		return
	}
...
// Initializes and starts the BGP server with the specified configuration.
func (s *Server) start(address string, asn uint32, routerID net.IP) error {
...
// Stops the BGP server and removes all associated peers.
func (s *Server) stop() error {
	// Skip if no instance.
	if s.bgp == nil {
		return nil
