...
func (b *InstanceBackup) Rename(newName string) error {
	backupsPath := b.state.BackupsStoragePath(b.instance.Project().Config["storage.backups_volume"])
...
func (b *InstanceBackup) Delete() error {
	backupsPathBase := b.state.BackupsStoragePath(b.instance.Project().Config["storage.backups_volume"])
