...
func instanceRebuildPost(d *Daemon, r *http.Request) response.Response {
	...
	run := func(op *operations.Operation) error {
		...
		if req.Source.Server != "" {
			sourceImage, err = ensureDownloadedImageFitWithinBudget(context.TODO(), s, op, *targetProject, sourceImageRef, req.Source, inst.Type().String())
			if err != nil {
				return err
			}
		}
		...
		return instanceRebuildFromImage(context.TODO(), s, inst, sourceImage, op)
	}
