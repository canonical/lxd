...
func instanceBackupExportGet(d *Daemon, r *http.Request) response.Response {
	...
	ent := response.FileResponseEntry{
		Path: filepath.Join(d.State().BackupsStoragePath(backup.Instance().Project().Config["storage.backups_volume"]), "instances", project.Instance(projectName, backup.Name())),
	}
