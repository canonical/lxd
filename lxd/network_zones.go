...
var networkZonesCmd = APIEndpoint{
	...
	Post: APIEndpointAction{Handler: networkZonesPost, AccessHandler: allowPermission(auth.ObjectTypeProject, auth.RelationNetworkZoneManager)},
}

var networkZoneCmd = APIEndpoint{
	Path: "network-zones/{zone}",

	Delete: APIEndpointAction{Handler: networkZoneDelete, AccessHandler: allowPermission(auth.ObjectTypeNetworkZone, auth.RelationManager)},
	Get:    APIEndpointAction{Handler: networkZoneGet, AccessHandler: allowPermission(auth.ObjectTypeNetworkZone, auth.RelationViewer)},
	Put:    APIEndpointAction{Handler: networkZonePut, AccessHandler: allowPermission(auth.ObjectTypeNetworkZone, auth.RelationManager)},
	Patch:  APIEndpointAction{Handler: networkZonePut, AccessHandler: allowPermission(auth.ObjectTypeNetworkZone, auth.RelationManager)},
}
...
func networkZonesGet(d *Daemon, r *http.Request) response.Response {
	...
	userHasPermission, err := s.Authorizer.GetPermissionChecker(r, auth.RelationViewer, auth.ObjectTypeNetworkZone)
	if err != nil {
		return response.InternalError(err)
	}
	...
	for _, zoneName := range zoneNames {
		if !userHasPermission(auth.NetworkZoneObject(projectName, zoneName)) {
			continue
		}
