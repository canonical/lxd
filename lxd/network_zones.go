...
func addNetworkZoneDetailsToRequestContext(d *Daemon, r *http.Request) error {
	...
	if err != nil && api.StatusErrorCheck(err, http.StatusNotFound) {
		return api.StatusErrorf(http.StatusNotFound, http.StatusText(http.StatusNotFound))
	} else if err != nil {
...
func networkZoneAccessHandler(entitlement auth.Entitlement) func(d *Daemon, r *http.Request) response.Response {
	return func(d *Daemon, r *http.Request) response.Response {
		err := addNetworkZoneDetailsToRequestContext(d, r)
		if err != nil {
		...
		err = d.State().Authorizer.CheckPermission(r.Context(), entity.NetworkZoneURL(details.requestProject.Name, details.zoneName), entitlement)
		if err != nil {
			return response.SmartError(err)
		}
