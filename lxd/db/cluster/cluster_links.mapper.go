...
func CreateClusterLink(ctx context.Context, tx *sql.Tx, object ClusterLink) (int64, error) {
	...
	if err != nil {
		if query.IsConflictErr(err) {
			return -1, api.NewStatusError(http.StatusConflict, "This \"cluster_links\" entry already exists")
		}
...
func UpdateClusterLink(ctx context.Context, tx *sql.Tx, name string, object ClusterLink) error {
	...
	result, err := stmt.ExecContext(ctx, object.IdentityID, object.Name, object.Description, object.Type, id)
	if err != nil {
		if query.IsConflictErr(err) {
			return api.NewStatusError(http.StatusConflict, "A \"cluster_links\" entry already exists with these properties")
		}
...
func RenameClusterLink(ctx context.Context, tx *sql.Tx, name string, to string) error {
	...
	if err != nil {
		if query.IsConflictErr(err) {
			return api.NewStatusError(http.StatusConflict, "A \"cluster_links\" entry already exists with this name")
		}
