...
SELECT cluster_links.id, cluster_links.identity_id, cluster_links.name, cluster_links.addresses, coalesce(cluster_links.description, ''), cluster_links.type
  FROM cluster_links
...
INSERT INTO cluster_links (identity_id, name, addresses, description, type)
  VALUES (?, ?, ?, ?, ?)
`)
...
UPDATE cluster_links
  SET identity_id = ?, name = ?, addresses = ?, description = ?, type = ?
 WHERE id = ?
`)
...
func clusterLinkColumns() string {
	return "clusters_links.id, clusters_links.identity_id, clusters_links.name, clusters_links.addresses, coalesce(clusters_links.description, ''), clusters_links.type"
}
...
func getClusterLinks(ctx context.Context, stmt *sql.Stmt, args ...any) ([]ClusterLink, error) {
	...
	dest := func(scan func(dest ...any) error) error {
		c := ClusterLink{}
		err := scan(&c.ID, &c.IdentityID, &c.Name, &c.Addresses, &c.Description, &c.Type)
		if err != nil {
			return err
		}
...
func UpdateClusterLink(ctx context.Context, tx *sql.Tx, name string, object ClusterLink) error {
	...
	result, err := stmt.Exec(object.IdentityID, object.Name, object.Addresses, object.Description, object.Type, id)
	if err != nil {
