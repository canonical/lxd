...
SELECT cluster_links.id, cluster_links.identity_id, cluster_links.name, cluster_links.addresses, coalesce(cluster_links.description, '')
  FROM cluster_links
...
INSERT INTO cluster_links (identity_id, name, addresses, description)
  VALUES (?, ?, ?, ?)
`)
...
UPDATE cluster_links
  SET identity_id = ?, name = ?, addresses = ?, description = ?
 WHERE id = ?
`)
...
func clusterLinkColumns() string {
	return "clusters_links.id, clusters_links.identity_id, clusters_links.name, clusters_links.addresses, coalesce(clusters_links.description, '')"
}
...
func getClusterLinks(ctx context.Context, stmt *sql.Stmt, args ...any) ([]ClusterLink, error) {
	...
	dest := func(scan func(dest ...any) error) error {
		c := ClusterLink{}
		err := scan(&c.ID, &c.IdentityID, &c.Name, &c.Addresses, &c.Description)
		if err != nil {
			return err
		}
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to fetch from \"clusters_links\" table: %w", err)
	}
...
func GetClusterLinks(ctx context.Context, tx *sql.Tx, filters ...ClusterLinkFilter) ([]ClusterLink, error) {
	...
	for i, filter := range filters {
		if filter.Name != nil && filter.ID == nil {
			...
			return nil, fmt.Errorf("Cannot filter on empty ClusterLinkFilter")
		} else {
...
// generator: cluster_link GetMany
func GetClusterLinkConfig(ctx context.Context, tx *sql.Tx, clusterLinkID int, filters ...ConfigFilter) (map[string]string, error) {
	clusterLinkConfig, err := GetConfig(ctx, tx, "cluster_link", filters...)
	if err != nil {
		return nil, err
	}

	config, ok := clusterLinkConfig[clusterLinkID]
	if !ok {
...
func GetClusterLink(ctx context.Context, tx *sql.Tx, name string) (*ClusterLink, error) {
	...
	default:
		return nil, fmt.Errorf("More than one \"clusters_links\" entry matches")
	}
}
...
func GetClusterLinkID(ctx context.Context, tx *sql.Tx, name string) (int64, error) {
	...
	if err != nil {
		return -1, fmt.Errorf("Failed to get \"clusters_links\" ID: %w", err)
	}
...
func CreateClusterLink(ctx context.Context, tx *sql.Tx, object ClusterLink) (int64, error) {
	...
	if exists {
		return -1, api.StatusErrorf(http.StatusConflict, "This \"clusters_links\" entry already exists")
	}
	...
	if err != nil {
		return -1, fmt.Errorf("Failed to create \"clusters_links\" entry: %w", err)
	}

	id, err := result.LastInsertId()
	if err != nil {
		return -1, fmt.Errorf("Failed to fetch \"clusters_links\" entry ID: %w", err)
	}
...
func CreateClusterLinkConfig(ctx context.Context, tx *sql.Tx, clusterLinkID int64, config map[string]string) error {
	...
	for key, value := range config {
		...
		err := CreateConfig(ctx, tx, "cluster_link", insert)
		if err != nil {
			return fmt.Errorf("Insert Config failed for ClusterLink: %w", err)
		}
...
func DeleteClusterLink(ctx context.Context, tx *sql.Tx, name string) error {
	...
	if err != nil {
		return fmt.Errorf("Delete \"clusters_links\": %w", err)
	}
...
func UpdateClusterLink(ctx context.Context, tx *sql.Tx, name string, object ClusterLink) error {
	...
	result, err := stmt.Exec(object.IdentityID, object.Name, object.Addresses, object.Description, id)
	if err != nil {
		return fmt.Errorf("Update \"clusters_links\" entry failed: %w", err)
	}
...
func UpdateClusterLinkConfig(ctx context.Context, tx *sql.Tx, clusterLinkID int64, config map[string]string) error {
	err := UpdateConfig(ctx, tx, "cluster_link", int(clusterLinkID), config)
	if err != nil {
		return fmt.Errorf("Replace Config for ClusterLink failed: %w", err)
	}
