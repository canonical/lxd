...
func UpdateOperationNodeID(ctx context.Context, tx *sql.Tx, opReference string, newNodeID int64) error {
	stmt := `UPDATE operations SET node_id = ? WHERE reference = ?`

	result, err := tx.ExecContext(ctx, stmt, newNodeID, opReference)
	if err != nil {
...
func UpdateOperationStatus(ctx context.Context, tx *sql.Tx, opReference string, newStatus api.StatusCode) error {
	stmt := `UPDATE operations SET status = ? WHERE reference = ?`

	result, err := tx.ExecContext(ctx, stmt, newStatus, opReference)
	if err != nil {
