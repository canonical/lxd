...
WHERE projects_config.key = 'restricted.cluster.groups' AND projects_config.value LIKE '%` + groupName + `%'`
	...
	err = query.SelectObjects(ctx, stmt, func(scan func(dest ...any) error) error {
		...
		projects[projectName] = configValue
		return nil
	})
	if err != nil {
		return nil, err
	...
	for name, value := range projects {
		if shared.ValueInSlice(groupName, shared.SplitNTrimSpace(value, ",", -1, false)) {
			projectURLs = append(projectURLs, entity.ProjectURL(name).String())
		}
	}
