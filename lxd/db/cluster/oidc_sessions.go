...
func (s OIDCSession) ToAPI() (*api.OIDCSession, error) {
	sessionUUID, err := uuid.Parse(s.UUID)
	if err != nil {
		return nil, err
	}

	sec, nsec := sessionUUID.Time().UnixTime()
...
`
	...
	var session OIDCSession
	row := tx.QueryRowContext(ctx, q, sessionID.String())
