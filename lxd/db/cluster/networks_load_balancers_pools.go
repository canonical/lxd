...
// NetworksLoadBalancerPoolsConfigStore returns a [query.EntityConfigStore] for network load balancer pools.
func NetworksLoadBalancerPoolsConfigStore() *query.EntityConfigStore {
	return &query.EntityConfigStore{
		EntityTable:               "networks_load_balancer_pools",
		ConfigTable:               "networks_load_balancer_pools_config",
		ConfigTableEntityIDColumn: "network_load_balancer_pool_id",
	}
}
...
// GetNetworksLoadBalancersByPool returns the names of all load balancers in the given network used by pools,
...
func GetNetworksLoadBalancersByPool(ctx context.Context, tx *sql.Tx, networkID int64, poolName *string) (map[string][]string, error) {
	...
	b.WriteString(`
		...
		CROSS JOIN json_each(networks_load_balancers.ports)
			...
			AND json_type(value, "$.target_pool") IS NOT NULL
	`)
...
func CreateNetworksLoadBalancerPoolConfig(ctx context.Context, tx *sql.Tx, poolID int64, config map[string]string) error {
	return NetworksLoadBalancerPoolsConfigStore().Set(ctx, tx, poolID, config)
}
...
func GetNetworksLoadBalancerPoolConfig(ctx context.Context, tx *sql.Tx, networkID int64, poolID *int64) (map[int64]map[string]string, error) {
	store := NetworksLoadBalancerPoolsConfigStore()

	if poolID != nil {
		return store.GetByEntityIDs(ctx, tx, *poolID)
	}

	return store.Select(ctx, tx, "WHERE networks_load_balancer_pools.network_id = ?", networkID)
}
