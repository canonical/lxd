...
func (o OIDCMetadata) Equals(cmp OIDCMetadata) bool {
	return o.Subject == cmp.Subject && slicesHaveMatchingElements(o.IdentityProviderGroups, cmp.IdentityProviderGroups)
}

func slicesHaveMatchingElements[S ~[]E, E comparable](s1, s2 S) bool {
	if len(s1) != len(s2) {
		return false
	}

	idxs := make(map[int]struct{}, len(s1))
	for i := range s1 {
		idx := slices.Index(s2, s1[i])
		if idx == -1 {
			return false
		}
