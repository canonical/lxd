...
func (d Xtables) networkSetupACLFilteringChains(networkName string) error {
	...
	for _, ipVersion := range []uint{4, 6} {
		...
		// Unlike OVN networks, we add the rules first before the ACL candidate rules, we can't indentify
		// "INPUT" and "OUTPUT" chain traffic once we have jumped into the ACL chain. At this point it
		// becomes indistinguishable from FORWARD traffic. So unlike OVN an ACL rule cannot be used to
		// block baseline service traffic.
...
func (d Xtables) aclRuleSubjectToACLMatch(direction string, ipVersion uint, subjectCriteria ...string) ([]string, error) {
	...
	for _, subjectCriterion := range subjectCriteria {
		...
		if err == nil {
			...
			return nil, fmt.Errorf("Unsupported subject %q", subjectCriterion)
		}
	}
