...
// checkStandbyReplicaProjectWriteGuard returns a Forbidden error when the project is a standby
// replica and the requestor is not the expected cluster link identity. It is a no-op for cluster
// notifications and for projects that are not in standby mode.
func checkStandbyReplicaProjectWriteGuard(ctx context.Context, tx *db.ClusterTx, targetProject *api.Project, clusterNotification bool, requestor *request.Requestor) error {
