...
func (d *common) Update(ctx context.Context, config *api.NetworkACLPut, clientType request.ClientType) error {
	...
	if len(aclOVNNets) > 0 && clientType == request.ClientTypeNormal {
		ovnnb, err := ovn.NewOVN(d.state.GlobalConfig.NetworkOVNNorthboundConnection(), d.state.GlobalConfig.NetworkOVSConnection(), d.state.GlobalConfig.NetworkOVNSSL)
		if err != nil {
		...
		cleanup, err := OVNEnsureACLs(ctx, d.state, d.logger, ovnnb, d.projectName, aclNameIDs, aclOVNNets, []string{d.info.Name}, true)
		if err != nil {
		...
		err = OVNPortGroupDeleteIfUnused(ctx, d.state, d.logger, ovnnb, d.projectName, nil, "", d.info.Name)
		if err != nil {
