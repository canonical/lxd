...
func (n *ovn) startUplinkPortBridgeNative(uplinkNet Network, bridgeDevice string) error {
	...
	if !InterfaceExists(vars.uplinkEnd) && !InterfaceExists(vars.ovsEnd) {
		...
		revert.Add(func() error { return veth.Delete() })
	}
...
func (n *ovn) setup(update bool) error {
	...
	if !update {
		revert.Add(func() error { return client.ChassisGroupDelete(n.getChassisGroupName()) })
	}
	...
	if !update {
		revert.Add(func() error { return client.LogicalRouterDelete(n.getRouterName()) })
	}
	...
	if len(extRouterIPs) > 0 {
		...
		if !update {
			revert.Add(func() error { return client.LogicalSwitchDelete(n.getExtSwitchName()) })
		}
		...
		if !update {
			revert.Add(func() error { return client.LogicalRouterPortDelete(n.getRouterExtPortName()) })
		}
		...
		if !update {
			revert.Add(func() error { return client.LogicalSwitchPortDelete(n.getExtSwitchRouterPortName()) })
		}
		...
		if !update {
			revert.Add(func() error { return client.LogicalSwitchPortDelete(n.getExtSwitchProviderPortName()) })
		}
	...
	if !update {
		revert.Add(func() error { return client.LogicalSwitchDelete(n.getIntSwitchName()) })
	}
	...
	if update {
		...
		revert.Add(func() error { return client.AddressSetDelete(acl.OVNIntSwitchPortGroupAddressSetPrefix(n.ID())) })
	}
	...
	if !update {
		revert.Add(func() error { return client.LogicalRouterPortDelete(n.getRouterIntPortName()) })
	}
	...
	if !update {
		revert.Add(func() error { return client.LogicalSwitchPortDelete(n.getIntSwitchRouterPortName()) })
	}
...
func (n *ovn) Update(newNetwork api.NetworkPut, targetNode string, clientType request.ClientType) error {
	...
	revert.Add(func() error {
		...
		if err != nil {
			logger.Warn("Failed resetting changes to nodes and database", logger.Ctx{"err": err})
		}
		...
		if clientType == request.ClientTypeNormal {
			err = n.setup(true)
			if err != nil {
				logger.Warn("Failed resetting logical network changes", logger.Ctx{"err": err})
			}
		}
...
func (n *ovn) InstanceDevicePortSetup(opts *OVNInstanceNICSetupOpts, securityACLsRemove []string) (openvswitch.OVNSwitchPort, error) {
	...
	revert.Add(func() error { return client.LogicalSwitchPortDelete(instancePortName) })
	...
	revert.Add(func() error { return client.LogicalSwitchPortDeleteDNS(n.getIntSwitchName(), dnsUUID) })
	...
	if shared.StringInSlice(opts.UplinkConfig["ovn.ingress_mode"], []string{"l2proxy", ""}) {
		for _, k := range []string{"ipv4.nat", "ipv6.nat"} {
			...
			revert.Add(func() error { return client.LogicalRouterDNATSNATDelete(n.getRouterName(), ip) })
		}
	}
	...
	if len(routes) > 0 {
		...
		revert.Add(func() error { return client.LogicalRouterRouteDelete(n.getRouterName(), routePrefixes...) })
		...
		revert.Add(func() error {
			return client.AddressSetRemove(acl.OVNIntSwitchPortGroupAddressSetPrefix(n.ID()), routePrefixes...)
		})
		...
		err = n.forPeers(func(targetOVNNet *ovn) error {
			...
			revert.Add(func() error { return client.LogicalRouterRouteDelete(targetRouterName, routePrefixes...) })
...
func (n *ovn) ForwardCreate(forward api.NetworkForwardsPost, clientType request.ClientType) error {
	...
	if clientType == request.ClientTypeNormal {
		...
		revert.Add(func() error {
			...
			if err != nil {
				logger.Warn("Failed deleting load balancer during revert", logger.Ctx{"err": err})
			}
...
func (n *ovn) ForwardUpdate(listenAddress string, req api.NetworkForwardPut, clientType request.ClientType) error {
	...
	if clientType == request.ClientTypeNormal {
		...
		revert.Add(func() error {
			...
			err := client.LoadBalancerApply(n.getLoadBalancerName(curForward.ListenAddress), []openvswitch.OVNRouter{n.getRouterName()}, []openvswitch.OVNSwitch{n.getIntSwitchName()}, vips...)
			if err != nil {
				logger.Warn("Failed applying loadbalancer during revert", logger.Ctx{"err": err})
			}
...
func (n *ovn) PeerCreate(peer api.NetworkPeersPost) error {
	...
	revert.Add(func() error {
		return n.state.DB.Cluster.DeleteNetworkPeer(n.ID(), peerID)
	})
