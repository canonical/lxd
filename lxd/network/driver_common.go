...
func (n *common) init(state *state.State, id int64, netInfo *api.Network, netNodes map[int64]db.NetworkNode) {
	n.logger = logging.AddContext(logger.Log, log.Ctx{"driver": netInfo.Type, "network": netInfo.Name})
	n.id = id
	n.name = netInfo.Name
