...
func (n *common) validateRoutes(config map[string]string) error {
	...
	checkListenAddressesOVN := func(project, network string, ips []string) error {
		for _, ip := range ips {
	...
	// Check that IP routes are provided for already existing OVN forwards.
	for project, networks := range forwards {
	...
	// Check that IP routes are provided for already existing OVN load balancers.
	for project, networks := range loadBalancers {
		for network, ips := range networks {
			err = checkListenAddressesOVN(project, network, ips)
			if err != nil {
				return err
