...
func usedByInstanceDevices(s *state.State, networkProjectName string, networkName string, usageFunc func(inst db.InstanceArgs, instProject api.Project, nicName string, nicConfig map[string]string) error) error {
	return s.Cluster.InstanceList(nil, func(inst db.InstanceArgs, p api.Project, profiles []api.Profile) error {
		...
		for devName, devConfig := range devices {
			if isInUseByDevice(networkName, devConfig) {
				err := usageFunc(inst, p, devName, devConfig)
				if err != nil {
					return err
				}
			}
...
func UsedBy(s *state.State, networkProjectName string, networkID int64, networkName string, firstOnly bool) ([]string, error) {
	...
	for i, profile := range profiles {
		inUse, err := usedByProfileDevices(s, profile, dbProfiles[i].Project, networkProjectName, networkName)
		if err != nil {
			return nil, err
		}

		if inUse {
			usedBy = append(usedBy, api.NewURL().Path(version.APIVersion, "profiles", profile.Name).Project(dbProfiles[i].Project).String())
	...
	err = usedByInstanceDevices(s, networkProjectName, networkName, func(inst db.InstanceArgs, instProject api.Project, nicName string, nicConfig map[string]string) error {
		// Minimal api.Instance to build the URL.
		apiInst := api.Instance{Name: inst.Name}
		usedBy = append(usedBy, apiInst.URL(version.APIVersion, instProject.Name).String())
...
func usedByProfileDevices(s *state.State, profile api.Profile, projectName string, networkProjectName string, networkName string) (bool, error) {
	// Get the translated network project name from the profiles's project.
	profileNetworkProjectName, _, err := project.NetworkProject(s.Cluster, projectName)
	if err != nil {
		return false, err
	}
