...
// rewrites the instance with the source's config before any data has moved, so a refresh that then fails would
// otherwise leave the source's config, profiles and description on an instance whose disks were never refreshed.
