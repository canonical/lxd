...
func instanceImageTransfer(ctx context.Context, s *state.State, projectName string, imagesVolume string, hash string, nodeAddress string) error {
	...
	err = imageImportFromNode(s.ImagesStoragePath(imagesVolume), client, hash)
	if err != nil {
		return err
	}
