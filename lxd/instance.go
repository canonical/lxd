...
// instanceRefreshRestoreHook returns a hook that puts an existing instance back the way it is now. A refresh
// rewrites the instance with the source's config before any data has moved, so a refresh refused before the
// transfer starts would otherwise leave the source's config, profiles and description on an instance whose
// disks were never touched. The hook is only for that window: once data moves the record stays as sent.
...
func pruneExpiredAndAutoCreateInstanceSnapshots(ctx context.Context, s *state.State) error {
	...
	if err != nil {
		return fmt.Errorf("Failed getting instance snapshot schedule info: %w", err)
	}
	...
	if len(expiredSnapshotInstances) > 0 {
		...
		if err != nil {
			return fmt.Errorf("Failed creating instance snapshots expiry operation: %w", err)
		}

		err = op.Wait(ctx)
		if err != nil {
			return fmt.Errorf("Failed pruning instance snapshots: %w", err)
		}
	...
	if len(instances) > 0 {
		...
		op, err := operations.ScheduleServerOperation(s, args)
		if err != nil {
			return fmt.Errorf("Failed creating scheduled instance snapshot operation: %w", err)
		}
