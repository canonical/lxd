...
func instanceImageTransfer(ctx context.Context, s *state.State, localProject string, remoteProject string, hash string, nodeAddress string) error {
	...
	client = client.UseProject(remoteProject)

	err = imageImportFromNode(s.ImagesStoragePath(localProject), client, hash)
	if err != nil {
		return err
	}
