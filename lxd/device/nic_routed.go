...
func (d *nicRouted) Start() (*deviceConfig.RunConfig, error) {
	...
	err = d.state.Firewall.InstanceSetupRPFilter(context.TODO(), d.inst.Project().Name, d.inst.Name(), d.name, saveData["host_name"])
	if err != nil {
...
func (d *nicRouted) postStop() error {
	...
	err := d.state.Firewall.InstanceClearRPFilter(context.TODO(), d.inst.Project().Name, d.inst.Name(), d.name)
	if err != nil {
		errs = append(errs, err)
	}
