...
// specDevToLXDDev builds a list of unix-char devices to be created from a CDI spec.
func specDevToLXDDev(configDevices *ConfigDevices, d specs.DeviceNode) error {
	if d.Path == "" {
	...
	nativeDev := map[string]string{"type": "unix-char",
		"source": hostPath,
		"path":   d.Path,
	...
	if d.UID != nil {
		nativeDev["uid"] = strconv.FormatUint(uint64(*d.UID), 10)
	}

	if d.GID != nil {
		nativeDev["gid"] = strconv.FormatUint(uint64(*d.GID), 10)
	}

	configDevices.UnixCharDevs = append(configDevices.UnixCharDevs, nativeDev)
	return nil
}

// specMountToLXDDev builds a list of disk mounts to be created from a CDI spec.
func specMountToLXDDev(configDevices *ConfigDevices, cdiID ID, mounts []*specs.Mount) ([]SymlinkEntry, error) {
...
func applyContainerEdits(edits specs.ContainerEdits, configDevices *ConfigDevices, hooks *Hooks) error {
	for _, d := range edits.DeviceNodes {
		...
		err := specDevToLXDDev(configDevices, *d)
		if err != nil {
			return err
		}
	}
...
func GenerateFromCDI(isCore bool, inst instance.Instance, cdiID ID) (*ConfigDevices, *Hooks, error) {
	...
	indirectSymlinks, err := specMountToLXDDev(configDevices, cdiID, mounts)
	if err != nil {
		return nil, nil, err
	}
...
// ReloadConfigDevicesFromDisk reads the paths to the CDI configuration devices
// file from the disk. This is useful in order to cache the CDI configuration
// devices file so that wee don't have to re-generate a CDI spec whhen stopping
// the container.
