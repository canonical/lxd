...
// PostMigrate is run after an instance is migrated to another cluster member.
func (d *nicBridged) PostMigrate() error {
	// Populate device config with volatile fields (hwaddr) if needed.
	networkVethFillFromVolatile(d.config, d.volatileGet())

	// Remove device (removing dnsmaq lease and config). This is required to reset leases post-migration.
	err := d.Remove()
	if err != nil {
		return err
	}
