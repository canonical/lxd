...
// networkTapMultiQueueSupported reports whether the TAP device for the given instance should be
// created with the multi-queue flag. libkrun MicroVMs open the TAP themselves and attach a single
// queue without IFF_MULTI_QUEUE, so a multi-queue TAP would fail to activate. QEMU VMs open the
// TAP with IFF_MULTI_QUEUE and use multi-queue.
func networkTapMultiQueueSupported(inst instance.Instance) bool {
