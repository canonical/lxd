...
func (d *gpuPhysical) startCDIDevices(configDevices cdi.ConfigDevices, runConf *deviceConfig.RunConfig) error {
	...
	for _, conf := range configDevices.UnixCharDevs {
		...
		_, major, minor, err := unixDeviceAttributes(conf["source"])
		if err != nil {
			return err
		}
		...
		err = unixDeviceSetupCharNum(d.state, d.inst.DevicesPath(), cdi.CDIUnixPrefix, d.name, conf, major, minor, conf["path"], false, runConf)
		if err != nil {
			return err
		}
	}
...
func (d *gpuPhysical) Stop() (*deviceConfig.RunConfig, error) {
	...
	if d.inst.Type() == instancetype.Container {
		...
		if !cdiID.Empty() {
			configDevices, _, err := cdi.GenerateFromCDI(d.inst, cdiID, d.logger)
			if err != nil {
				return nil, err
			}
