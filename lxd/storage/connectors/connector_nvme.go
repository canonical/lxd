...
func Discover(ctx context.Context, c Connector, serverUUID string, targetAddresses ...string) ([]nvmeDiscoveryLogRecord, error) {
	if c.Type() != TypeNVME {
		return nil, fmt.Errorf("Discover() helper can only be used with NVMe connector type")
	}
	...
	for _, targetAddr := range targetAddresses {
		stdout, err := shared.RunCommandContext(ctx, "nvme", "discover", "--transport", "tcp", "--traddr", targetAddr, "--hostnqn", hostNQN, "--hostid", serverUUID, "--output-format", "json")
		if err != nil {
