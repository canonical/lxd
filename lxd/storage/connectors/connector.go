...
type session struct {
	...
	// hostAddresses maps a target address to the local host addresses that already have
	// an established path to it. It is populated only for NVMe/FC where every local HBA
	// that is zoned to a target port forms a path of its own.
