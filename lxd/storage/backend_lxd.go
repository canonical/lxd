...
func (b *lxdBackend) CreateInstance(inst instance.Instance, op *operations.Operation) error {
	...
	err = VolumeDBCreate(b, inst.Project().Name, inst.Name(), vol, "", false, inst.CreationDate(), time.Time{}, true, false)
	if err != nil {
		return err
	}
...
func (b *lxdBackend) CreateInstanceFromBackup(srcBackup backup.Info, srcData io.ReadSeeker, op *operations.Operation) (func(instance.Instance) error, revert.Hook, error) {
	...
	postHook := func(inst instance.Instance) error {
		...
		// Fill volume config with pool defaults before writing it to the database.
		...
		for i, backupFileSnap := range srcBackup.Snapshots {
			...
			// Validate config and create database entry for new storage volume.
			// Strip unsupported config keys (in case the export was made from a different type of storage pool).
			err = VolumeDBCreate(b, inst.Project().Name, newSnapshotName, snapVol, volumeSnapDescription, true, volumeSnapCreationDate, volumeSnapExpiryDate, true, true)
			if err != nil {
				return err
			}
...
func (b *lxdBackend) CreateInstanceFromCopy(inst instance.Instance, src instance.Instance, snapshots bool, allowInconsistent bool, op *operations.Operation) error {
	...
	if b.Name() == srcPool.Name() {
		...
		// Validate config and create database entry for new storage volume.
		err = VolumeDBCreate(b, inst.Project().Name, inst.Name(), vol, "", false, inst.CreationDate(), time.Time{}, false, true)
		if err != nil {
			return err
		}
		...
		for i, snapName := range snapshotNames {
			...
			// Validate config and create database entry for new storage volume.
			err = VolumeDBCreate(b, inst.Project().Name, newSnapshotName, vol, srcConfig.VolumeSnapshots[i].Description, true, srcConfig.VolumeSnapshots[i].CreatedAt, volumeSnapExpiryDate, false, true)
			if err != nil {
				return err
			}
...
func (b *lxdBackend) RefreshCustomVolume(projectName string, srcProjectName string, volName string, desc string, config map[string]string, srcPoolName, srcVolName string, snapshots bool, op *operations.Operation) error {
	...
	if srcPool == b {
		...
		for _, srcSnap := range srcConfig.VolumeSnapshots {
			...
			err = VolumeDBCreate(b, projectName, newSnapshotName, targetSnapVol, srcSnap.Description, true, srcSnap.CreatedAt, snapExpiryDate, false, true)
			if err != nil {
				return err
			}
...
func (b *lxdBackend) RefreshInstance(inst instance.Instance, src instance.Instance, srcSnapshots []instance.Instance, allowInconsistent bool, op *operations.Operation) error {
	...
	if b.Name() == srcPool.Name() {
		...
		for i := range srcConfig.VolumeSnapshots {
			...
			// Validate config and create database entry for new storage volume.
			err = VolumeDBCreate(b, inst.Project().Name, newSnapshotName, snapVol, srcConfig.VolumeSnapshots[i].Description, true, srcConfig.VolumeSnapshots[i].CreatedAt, volumeSnapExpiryDate, false, true)
			if err != nil {
				return err
			}
...
func (b *lxdBackend) CreateInstanceFromMigration(inst instance.Instance, conn io.ReadWriteCloser, args migration.VolumeTargetArgs, op *operations.Operation) error {
	...
	if !isRemoteClusterMove {
		for i, snapName := range args.Snapshots {
			...
			// Validate config and create database entry for new storage volume.
			// Strip unsupported config keys (in case the export was made from a different type of storage pool).
			err = VolumeDBCreate(b, inst.Project().Name, newSnapshotName, snapVol, snapDescription, true, snapCreationDate, snapExpiryDate, true, true)
			if err != nil {
				return err
			}
...
func (b *lxdBackend) CreateInstanceFromConversion(inst instance.Instance, conn io.ReadWriteCloser, args migration.VolumeTargetArgs, op *operations.Operation) error {
	...
	// Validate config and create database entry for new storage volume if not refreshing.
	// Strip unsupported config keys (in case the export was made from a different type of storage pool).
	err = VolumeDBCreate(b, inst.Project().Name, inst.Name(), vol, args.Description, false, inst.CreationDate(), time.Time{}, false, true)
	if err != nil {
		return err
	}
...
func (b *lxdBackend) CreateInstanceSnapshot(inst instance.Instance, src instance.Instance, op *operations.Operation) error {
	...
	// Validate config and create database entry for new storage volume.
	err = VolumeDBCreate(b, inst.Project().Name, inst.Name(), vol, srcDBVol.Description, true, inst.CreationDate(), time.Time{}, false, true)
	if err != nil {
		return err
	}
...
func (b *lxdBackend) EnsureImage(fingerprint string, op *operations.Operation) error {
	...
	// Validate config and create database entry for new storage volume.
	err = VolumeDBCreate(b, api.ProjectDefaultName, image.Fingerprint, imgVol, "", false, time.Now().UTC(), time.Time{}, false, false)
	if err != nil {
		return err
	}
...
func (b *lxdBackend) CreateCustomVolume(projectName string, volName string, desc string, config map[string]string, contentType drivers.ContentType, op *operations.Operation) error {
	...
	// Validate config and create database entry for new storage volume.
	err = VolumeDBCreate(b, projectName, volName, vol, desc, false, time.Now().UTC(), time.Time{}, false, false)
	if err != nil {
		return err
	}
...
func (b *lxdBackend) CreateCustomVolumeFromCopy(projectName string, srcProjectName string, volName string, desc string, config map[string]string, srcPoolName, srcVolName string, snapshots bool, op *operations.Operation) error {
	...
	if srcPool == b {
		...
		// Validate config and create database entry for new storage volume.
		err = VolumeDBCreate(b, projectName, volName, vol, desc, false, time.Now().UTC(), time.Time{}, false, true)
		if err != nil {
			return err
		}
		...
		for i, snapName := range snapshotNames {
			...
			// Validate config and create database entry for new storage volume.
			err = VolumeDBCreate(b, projectName, newSnapshotName, snapVol, srcConfig.VolumeSnapshots[i].Description, true, srcConfig.VolumeSnapshots[i].CreatedAt, volumeSnapExpiryDate, false, true)
			if err != nil {
				return err
			}
...
func (b *lxdBackend) CreateCustomVolumeFromMigration(projectName string, conn io.ReadWriteCloser, args migration.VolumeTargetArgs, op *operations.Operation) error {
	...
	if len(args.Snapshots) > 0 {
		...
		for _, snapName := range args.Snapshots {
			...
			// Validate config and create database entry for new storage volume.
			// Strip unsupported config keys (in case the export was made from a different type of storage pool).
			err = VolumeDBCreate(b, projectName, newSnapshotName, snapVol, snapDescription, true, snapCreationDate, snapExpiryDate, true, true)
			if err != nil {
				return err
			}
...
func (b *lxdBackend) ImportCustomVolume(projectName string, poolVol *backupConfig.Config, op *operations.Operation) (revert.Hook, error) {
	...
	err := VolumeDBCreate(b, projectName, poolVol.Volume.Name, vol, poolVol.Volume.Description, false, poolVol.Volume.CreatedAt, time.Time{}, false, true)
	if err != nil {
		return nil, err
	}
	...
	for _, poolVolSnap := range poolVol.VolumeSnapshots {
		...
		// Validate config and create database entry for restored storage volume.
		err = VolumeDBCreate(b, projectName, fullSnapName, snapVol, poolVolSnap.Description, true, poolVolSnap.CreatedAt, time.Time{}, false, true)
		if err != nil {
			return nil, err
		}
...
func (b *lxdBackend) ImportInstance(inst instance.Instance, poolVol *backupConfig.Config, op *operations.Operation) (revert.Hook, error) {
	...
	if poolVol != nil {
		...
		err = VolumeDBCreate(b, inst.Project().Name, inst.Name(), vol, "", false, creationDate, time.Time{}, false, true)
		if err != nil {
			return nil, err
		}
		...
		if len(snapshots) > 0 && len(poolVol.VolumeSnapshots) > 0 {
			...
			for _, poolVolSnap := range poolVol.VolumeSnapshots {
				...
				// Validate config and create database entry for recovered storage volume.
				err = VolumeDBCreate(b, inst.Project().Name, fullSnapName, snapVol, poolVolSnap.Description, true, poolVolSnap.CreatedAt, time.Time{}, false, true)
				if err != nil {
					return nil, err
				}
			...
			for _, i := range snapshots {
				...
				// Validate config and create database entry for new storage volume.
				err = VolumeDBCreate(b, inst.Project().Name, fullSnapName, snapVol, "", true, time.Time{}, time.Time{}, false, true)
				if err != nil {
					return nil, err
				}
...
func (b *lxdBackend) CreateCustomVolumeFromISO(projectName string, volName string, srcData io.ReadSeeker, size int64, op *operations.Operation) error {
	...
	// Validate config and create database entry for new storage volume.
	err = VolumeDBCreate(b, projectName, volName, vol, "", false, time.Now(), time.Time{}, true, true)
	if err != nil {
...
func (b *lxdBackend) CreateCustomVolumeFromBackup(srcBackup backup.Info, srcData io.ReadSeeker, op *operations.Operation) error {
	...
	// Validate config and create database entry for new storage volume.
	// Strip unsupported config keys (in case the export was made from a different type of storage pool).
	err = VolumeDBCreate(b, srcBackup.Project, srcBackup.Name, vol, srcBackup.Config.Volume.Description, false, srcBackup.Config.Volume.CreatedAt, time.Time{}, true, true)
	if err != nil {
		return err
	}
	...
	for _, s := range srcBackup.Config.VolumeSnapshots {
		...
		// Validate config and create database entry for new storage volume.
		// Strip unsupported config keys (in case the export was made from a different type of storage pool).
		err = VolumeDBCreate(b, srcBackup.Project, fullSnapName, snapVol, snapshot.Description, true, snapshot.CreatedAt, *snapshot.ExpiresAt, true, true)
		if err != nil {
			return err
		}
