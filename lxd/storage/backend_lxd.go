...
func (b *lxdBackend) imageFiller(fingerprint string, op *operations.Operation, project string) func(vol drivers.Volume, rootBlockPath string, allowUnsafeResize bool) (int64, error) {
	return func(vol drivers.Volume, rootBlockPath string, allowUnsafeResize bool) (int64, error) {
		...
		imageFile := filepath.Join(b.state.ImagesStoragePath(project), fingerprint)
		return ImageUnpack(b.state, project, imageFile, vol, rootBlockPath, allowUnsafeResize, tracker)
	}
}
...
func (b *lxdBackend) EnsureImage(fingerprint string, op *operations.Operation, project string) error {
	...
	volFiller := drivers.VolumeFiller{
		...
		Fill:        b.imageFiller(image.Fingerprint, op, project),
	}
