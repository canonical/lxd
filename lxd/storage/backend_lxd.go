...
func (b *lxdBackend) EnsureImage(fingerprint string, op *operations.Operation) error {
	...
	err = VolumeDBCreate(b.state, b, project.Default, fingerprint, "", drivers.VolumeTypeImage, false, volConfig, time.Time{})
	if err != nil {
		return err
	}
...
func (b *lxdBackend) updateVolumeDescriptionOnly(project string, volName string, dbVolType int, newDesc string, newConfig map[string]string, op *operations.Operation) error {
	...
	// Validate config.
	vol := b.GetVolume(drivers.VolumeType(curVol.Type), drivers.ContentTypeFS, volName, newConfig)
...
func (b *lxdBackend) CreateCustomVolume(projectName string, volName string, desc string, config map[string]string, op *operations.Operation) error {
	...
	// Validate config.
	vol := b.GetVolume(drivers.VolumeTypeCustom, drivers.ContentTypeFS, volStorageName, config)
...
func (b *lxdBackend) CreateCustomVolumeFromCopy(projectName string, srcProjectName string, volName string, desc string, config map[string]string, srcPoolName, srcVolName string, srcVolOnly bool, op *operations.Operation) error {
	...
	if srcPool == b {
		...
		srcVol := b.GetVolume(drivers.VolumeTypeCustom, drivers.ContentTypeFS, srcVolStorageName, srcVolRow.Config)
		...
		if len(snapshotNames) > 0 {
			for _, snapName := range snapshotNames {
				...
				err = VolumeDBCreate(b.state, b, projectName, newSnapshotName, desc, vol.Type(), true, vol.Config(), time.Time{})
				if err != nil {
					return err
				}
...
func (b *lxdBackend) MigrateCustomVolume(projectName string, conn io.ReadWriteCloser, args *migration.VolumeSourceArgs, op *operations.Operation) error {
	...
	vol := b.GetVolume(drivers.VolumeTypeCustom, drivers.ContentTypeFS, volStorageName, nil)
	err := b.driver.MigrateVolume(vol, conn, args, op)
	if err != nil {
		return err
	}
...
func (b *lxdBackend) CreateCustomVolumeFromMigration(projectName string, conn io.ReadWriteCloser, args migration.VolumeTargetArgs, op *operations.Operation) error {
	...
	vol := b.GetVolume(drivers.VolumeTypeCustom, drivers.ContentTypeFS, volStorageName, args.Config)
	err = b.driver.ValidateVolume(vol, true)
	if err != nil {
		return err
	}
	...
	if len(args.Snapshots) > 0 {
		for _, snapName := range args.Snapshots {
			...
			// Create database entry for new storage volume snapshot.
			err = VolumeDBCreate(b.state, b, projectName, newSnapshotName, args.Description, vol.Type(), true, vol.Config(), time.Time{})
			if err != nil {
				return err
			}
...
func (b *lxdBackend) UpdateCustomVolume(projectName string, volName string, newDesc string, newConfig map[string]string, op *operations.Operation) error {
	...
	// Validate config.
	newVol := b.GetVolume(drivers.VolumeTypeCustom, drivers.ContentTypeFS, volStorageName, newConfig)
	err := b.driver.ValidateVolume(newVol, false)
	if err != nil {
		return err
	}
	...
	if len(changedConfig) != 0 {
		...
		curVol := b.GetVolume(drivers.VolumeTypeCustom, drivers.ContentTypeFS, volStorageName, curVol.Config)
		if !userOnly {
			err = b.driver.UpdateVolume(curVol, changedConfig)
			if err != nil {
				return err
...
func (b *lxdBackend) ImportCustomVolume(projectName string, poolVol backup.Config, op *operations.Operation) error {
	...
	err := VolumeDBCreate(b.state, b, projectName, poolVol.Volume.Name, poolVol.Volume.Description, drivers.VolumeTypeCustom, false, poolVol.Volume.Config, time.Time{})
	if err != nil {
	...
	for _, poolVolSnap := range poolVol.VolumeSnapshots {
		...
		err = VolumeDBCreate(b.state, b, projectName, fullSnapName, poolVolSnap.Description, drivers.VolumeTypeCustom, true, poolVolSnap.Config, time.Time{})
		if err != nil {
			return err
		}
...
func (b *lxdBackend) CreateCustomVolumeSnapshot(projectName, volName string, newSnapshotName string, newExpiryDate time.Time, op *operations.Operation) error {
	...
	// Create database entry for new storage volume snapshot.
	err = VolumeDBCreate(b.state, b, projectName, fullSnapshotName, parentVol.Description, drivers.VolumeTypeCustom, true, parentVol.Config, newExpiryDate)
	if err != nil {
		return err
	}
	...
	vol := b.GetVolume(drivers.VolumeTypeCustom, drivers.ContentTypeFS, volStorageName, parentVol.Config)
...
func (b *lxdBackend) RestoreCustomVolume(projectName, volName string, snapshotName string, op *operations.Operation) error {
	...
	// Get the volume name on storage.
	volStorageName := project.StorageVolume(projectName, volName)
	vol := b.GetVolume(drivers.VolumeTypeCustom, drivers.ContentTypeFS, volStorageName, dbVol.Config)
