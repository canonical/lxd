...
func (b *lxdBackend) CreateInstanceFromBackup(srcBackup backup.Info, srcData io.ReadSeeker, op *operations.Operation) (func(instance.Instance) error, revert.Hook, error) {
	...
	var rootVol *backupConfig.VolumeConfig
	...
	postHook := func(inst instance.Instance) error {
		...
		err = b.UpdateInstanceBackupFile(inst, false, api.BackupVersion20, op)
		if err != nil {
...
func (b *lxdBackend) RefreshCustomVolume(projectName string, srcProjectName string, volName string, desc string, config map[string]string, srcPoolName, srcVolName string, snapshots bool, op *operations.Operation) error {
	...
	if desc == "" {
		desc = rootVol.Description
	}

	contentDBType, err := VolumeContentTypeNameToContentType(rootVol.ContentType)
	if err != nil {
		return err
	}
	...
	sourceSnapshots := make([]drivers.Volume, 0, len(rootVol.Snapshots))
	for _, sourceSnap := range rootVol.Snapshots {
		snapshotName := drivers.GetSnapshotVolumeName(rootVol.Name, sourceSnap.Name)
	...
	dbVol, err := VolumeDBGet(b, projectName, volName, drivers.VolumeType(rootVol.Type))
	if err != nil {
		return err
	}
	...
	if snapshots {
		// Compare snapshots.
		sourceSnapshotComparable := make([]ComparableSnapshot, 0, len(rootVol.Snapshots))
		for _, sourceSnap := range rootVol.Snapshots {
		...
		rootVol.Snapshots = make([]*api.StorageVolumeSnapshot, 0, len(syncSourceSnapshotIndexes))
		for _, syncSourceSnapIndex := range syncSourceSnapshotIndexes {
			...
			rootVol.Snapshots = append(rootVol.Snapshots, allSnapshots[syncSourceSnapIndex])
		}
	}
	...
	// Get the src volume name on storage.
	srcVolStorageName := project.StorageVolume(srcProjectName, rootVol.Name)
	srcVol := srcPool.GetVolume(drivers.VolumeTypeCustom, contentType, srcVolStorageName, rootVol.Config)

	if srcPool == b {
		l.Debug("RefreshCustomVolume same-pool mode detected")

		// Only refresh the snapshots that the target needs.
		srcSnapVols := make([]string, 0, len(rootVol.Snapshots))
		for _, srcSnap := range rootVol.Snapshots {
...
func (b *lxdBackend) BackupInstance(inst instance.Instance, tarWriter *instancewriter.InstanceTarWriter, optimized bool, snapshots bool, version api.BackupVersion, op *operations.Operation) error {
...
func (b *lxdBackend) RenameInstanceSnapshot(inst instance.Instance, newName string, op *operations.Operation) error {
	...
	// Ensure the backup file reflects current config.
	// Always use the latest backup file format.
	err = b.UpdateInstanceBackupFile(inst, true, api.BackupVersion20, op)
	if err != nil {
		return err
	}
...
func (b *lxdBackend) CreateCustomVolumeFromCopy(projectName string, srcProjectName string, volName string, desc string, config map[string]string, srcPoolName, srcVolName string, snapshots bool, op *operations.Operation) error {
	...
	if srcPool == b {
		...
		for i, snapName := range snapshotNames {
			...
			// Create a new snapshot volume with its own config and UUID.
			snapVol := b.GetNewVolume(vol.Type(), contentType, newSnapshotName, rootVol.Snapshots[i].Config)

			// Validate config and create database entry for new storage volume.
			err = VolumeDBCreate(b, projectName, newSnapshotName, rootVol.Snapshots[i].Description, vol.Type(), true, snapVol.Config(), rootVol.Snapshots[i].CreatedAt, volumeSnapExpiryDate, vol.ContentType(), false, true)
			if err != nil {
				return err
			}
...
func (b *lxdBackend) MigrateCustomVolume(projectName string, conn io.ReadWriteCloser, args *migration.VolumeSourceArgs, op *operations.Operation) error {
	...
	vol := b.GetVolume(drivers.VolumeTypeCustom, contentType, volStorageName, rootVol.Config)
...
func (b *lxdBackend) ImportCustomVolume(projectName string, poolVol *backupConfig.Config, op *operations.Operation) (revert.Hook, error) {
	...
	l := b.logger.AddContext(logger.Ctx{"project": projectName, "volName": rootVol.Name})
	...
	// Get the volume name on storage.
	volStorageName := project.StorageVolume(projectName, rootVol.Name)

	// Copy volume config from backup file if present (so VolumeDBCreate can safely modify the copy if needed).
	vol := b.GetNewVolume(drivers.VolumeTypeCustom, drivers.ContentType(rootVol.ContentType), volStorageName, rootVol.Config)

	// Validate config and create database entry for restored storage volume.
	err = VolumeDBCreate(b, projectName, rootVol.Name, rootVol.Description, drivers.VolumeTypeCustom, false, vol.Config(), rootVol.CreatedAt, time.Time{}, drivers.ContentType(rootVol.ContentType), false, true)
	if err != nil {
		return nil, err
	}

	revert.Add(func() { _ = VolumeDBDelete(b, projectName, rootVol.Name, drivers.VolumeTypeCustom) })

	// Create the storage volume snapshot DB records.
	for _, poolVolSnap := range rootVol.Snapshots {
		fullSnapName := drivers.GetSnapshotVolumeName(rootVol.Name, poolVolSnap.Name)
...
func (b *lxdBackend) GenerateCustomVolumeBackupConfig(projectName string, volName string, snapshots bool, op *operations.Operation) (*backupConfig.Config, error) {
	...
	volConfig := &backupConfig.VolumeConfig{
	...
	return &backupConfig.Config{
		Version: api.BackupVersion20,
		Volumes: []*backupConfig.VolumeConfig{volConfig},
	}, nil
}
...
func (b *lxdBackend) GenerateInstanceBackupConfig(inst instance.Instance, snapshots bool, op *operations.Operation) (*backupConfig.Config, error) {
	...
	volumeConfig := &backupConfig.VolumeConfig{
	...
	config.Volumes = []*backupConfig.VolumeConfig{volumeConfig}
	return config, nil
}

// UpdateInstanceBackupFile writes the instance's config to the backup.yaml file on the storage device.
func (b *lxdBackend) UpdateInstanceBackupFile(inst instance.Instance, snapshots bool, version api.BackupVersion, op *operations.Operation) error {
	l := b.logger.AddContext(logger.Ctx{"project": inst.Project().Name, "instance": inst.Name()})
...
func (b *lxdBackend) detectUnknownInstanceVolume(vol *drivers.Volume, projectVols map[string][]*backupConfig.Config, op *operations.Operation) error {
	...
	instVolDBType, err := VolumeTypeNameToDBType(rootVol.Type)
	if err != nil {
...
func (b *lxdBackend) detectUnknownCustomVolume(vol *drivers.Volume, projectVols map[string][]*backupConfig.Config, op *operations.Operation) error {
	...
	customVol := &backupConfig.VolumeConfig{
	...
	backupConf := &backupConfig.Config{
		Volumes: []*backupConfig.VolumeConfig{customVol},
	}
...
func (b *lxdBackend) CreateCustomVolumeFromBackup(srcBackup backup.Info, srcData io.ReadSeeker, op *operations.Operation) error {
	...
	if len(srcBackup.Snapshots) != len(rootVol.Snapshots) {
	...
	// Validate config and create database entry for new storage volume.
	// Strip unsupported config keys (in case the export was made from a different type of storage pool).
	err = VolumeDBCreate(b, srcBackup.Project, srcBackup.Name, rootVol.Description, vol.Type(), false, vol.Config(), rootVol.CreatedAt, time.Time{}, vol.ContentType(), true, true)
	if err != nil {
		return err
	}
	...
	for _, s := range rootVol.Snapshots {
		...
		snapVol := b.GetNewVolume(drivers.VolumeTypeCustom, drivers.ContentType(rootVol.ContentType), snapVolStorageName, snapshot.Config)
