...
func (b *lxdBackend) CreateInstanceFromBackup(srcBackup backup.Info, srcData io.ReadSeeker, op *operations.Operation) (func(instance.Instance) error, revert.Hook, error) {
	...
	_, err := instancetype.ValidName(srcBackup.Name, false)
	if err != nil {
		return nil, nil, err
	}

	_, err = instancetype.ValidName(srcBackup.Config.Instance.Name, false)
	if err != nil {
		return nil, nil, err
	}
	...
	for _, snapName := range srcBackup.Snapshots {
		...
		_, err = instancetype.ValidName(snapInstName, true)
		if err != nil {
			return nil, nil, err
		}
	}
...
func (b *lxdBackend) RenameInstance(inst instance.Instance, newName string, op *operations.Operation) error {
	...
	// Quick checks.
	_, err := instancetype.ValidName(newName, false)
	if err != nil {
		return err
	}
