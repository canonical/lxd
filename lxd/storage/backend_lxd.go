...
func (b *lxdBackend) CreateInstance(inst instance.Instance, op *operations.Operation) error {
	...
	vol := b.GetVolume(volType, contentType, inst.Name(), nil)

	err = vol.FillConfig(instanceDiskVolumeEffectiveFields...)
	if err != nil {
		return err
	}

	// Validate config and create database entry for new storage volume.
	err = VolumeDBCreate(b, inst.Project().Name, inst.Name(), "", volType, false, vol.Config(), inst.CreationDate(), time.Time{}, contentType, false)
	if err != nil {
		return err
	}
	...
	vol = b.GetVolume(volType, contentType, volStorageName, vol.Config())
