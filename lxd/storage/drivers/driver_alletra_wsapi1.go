...
func (p *alletraClient) request(method string, url url.URL, reqBody map[string]any, reqHeaders map[string]string, respBody any, respHeaders map[string]string) error {
	...
	scheme, host, found := strings.Cut(p.driver.config["alletra.wsapi.url"], "://")
	if !found {
		return fmt.Errorf("Invalid HPE Storage WSAPI URL: %q", p.driver.config["alletra.wsapi.url"])
	}
	...
	client := &http.Client{
		Transport: &http.Transport{
			TLSClientConfig: &tls.Config{
				InsecureSkipVerify: shared.IsFalse(p.driver.config["alletra.wsapi.verify"]),
			},
		},
	}
	...
	if assert {
		...
		if resp.StatusCode == http.StatusForbidden && hpeErr.Code == 6 { // 6 = INV_SESS_KEY
...
func (p *alletraClient) getSessionKeysCacheKey() string {
	return p.driver.config["alletra.wsapi.url"] + p.driver.config["alletra.wsapi.user.name"] + p.driver.config["alletra.wsapi.user.password"]
}
...
func (p *alletraClient) login() error {
	...
	body := map[string]any{
		"user":        p.driver.config["alletra.wsapi.user.name"],
		"password":    p.driver.config["alletra.wsapi.user.password"],
		"sessionType": 1, // REGULAR
	}
...
func (p *alletraClient) getCurrentHost() (*hpeHost, error) {
...
func (p *alletraClient) createHost(hostName string, qns []string) error {
	req := map[string]any{
	...
	default:
		return fmt.Errorf("Unsupported HPE Alletra Storage mode %q", connector.Type())
	}
...
func (p *alletraClient) deleteVolumeSnapshot(poolName string, volName string, snapshotName string) error {
	err := p._deleteVolume(poolName, snapshotName)
	if err != nil {
