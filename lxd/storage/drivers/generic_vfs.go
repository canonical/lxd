...
func mountVolume(d Driver, vol Volume, getDevicePath func(Volume, bool) (string, func() error, error), op *operations.Operation) error {
...
func unmountVolume(d Driver, vol Volume, keepBlockDev bool, getDevicePath func(Volume, bool) (string, func() error, error), op *operations.Operation) (bool, error) {
	...
	if vol.contentType == ContentTypeFS && filesystem.IsMountPoint(mountPath) {
		...
		// Attempt to unmap.
		if !keepBlockDev {
			_, deactivate, err := getDevicePath(vol, false)
			if err != nil {
				return false, err
			}
