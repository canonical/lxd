...
func (d *zfs) restoreVolume(vol Volume, snapVol Volume, migration bool, op *operations.Operation) error {
	...
	if len(snapshots) > 0 {
		...
		for _, snapName := range snapshots {
			...
			if err != nil {
				return fmt.Errorf("Failed to check clones for snapshot %q: %w", snapName, err)
			}

			if len(clones) > 0 {
				return fmt.Errorf("Snapshot %q has %d dependent clone(s), cannot delete", snapName, len(clones))
			}
		}
