...
// and its error messages contain any of the provided substrings.
...
func isPureErrorNotFound(err error) bool {
	return isPureErrorOf(err, http.StatusBadRequest, "does not exist", "No such volume or snapshot")
}
...
func (p *pureClient) request(method string, path string, reqBody io.Reader, reqHeaders map[string]string, respBody any, respHeaders map[string]string) error {
	...
	if len(urlParts) != 2 {
		return fmt.Errorf("Invalid PureStorage gateway URL: %q", p.driver.config["pure.gateway"])
	}

	// Construct the request URL.
	url := api.NewURL().Scheme(urlParts[0]).Host(urlParts[1]).Path(path)
	...
	if path != "/api/api_version" {
		...
		url = url.Path("api", p.driver.apiVersion, path)
	}
...
// requestAuthenticated issues an authenticated HTTP request against the PureStorage gateway.
...
// getAPIVersion returns the list of API versions that are supported by the PureStorage.
func (p *pureClient) getAPIVersions() ([]string, error) {
	...
	if len(resp.APIVersions) == 0 {
		return nil, fmt.Errorf("PureStorage does not support any API versions")
	}
...
func (p *pureClient) getTarget() (*pureTarget, error) {
	...
	targetAddr := p.driver.config["pure.array.address"]

	// Append service port to the address if missing.
	// This way we can use exact matching in the filter to find the target.
	if !strings.Contains(targetAddr, ":") {
		...
		targetAddr = fmt.Sprintf("%s:%s", targetAddr, port)
	}

	err := p.requestAuthenticated(http.MethodGet, fmt.Sprintf("/ports?filter=portal='%s'", targetAddr), nil, &resp)
	if err != nil {
		return nil, fmt.Errorf("Failed to retrieve PureStorage targets for address %q: %w", targetAddr, err)
	}

	if len(resp.Items) == 0 {
		return nil, api.StatusErrorf(http.StatusNotFound, "No PureStorage target found on address %q", targetAddr)
	}
...
func (d *pure) connect() (revert.Hook, error) {
	...
	case pureModeISCSI:
		...
		_, _, err = shared.RunCommandSplit(d.state.ShutdownCtx, nil, nil, "iscsiadm", "--mode", "node", "--targetname", targetIQN, "--portal", targetAddr, "--login")
		if err != nil {
