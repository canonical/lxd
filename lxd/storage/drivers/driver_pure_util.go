...
func (p *pureClient) getTarget() (*pureTarget, error) {
	...
	if !strings.Contains(targetAddr, ":") {
		...
		if p.driver.config["pure.mode"] == pureModeISCSI {
...
func (d *pure) connect() (revert.Hook, error) {
	...
	case pureModeISCSI:
		...
		_, stderr, err := shared.RunCommandSplit(d.state.ShutdownCtx, nil, nil, "iscsiadm", "--mode", "node", "--targetname", targetIQN, "--portal", targetAddr, "--login")
		if err != nil {
			return nil, fmt.Errorf("Failed to connect to PureStorage array %q via iSCSI: %s\n%v", targetAddr, stderr, err)
		}
	...
	case pureModeNVMe:
		...
		_, stderr, err := shared.RunCommandSplit(d.state.ShutdownCtx, nil, nil, "nvme", "connect", "--transport", "tcp", "--traddr", targetAddr, "--nqn", targetNQN, "--hostnqn", hostNQN, "--hostid", serverUUID)
		if err != nil {
			return nil, fmt.Errorf("Failed to connect to PureStorage array %q via NVMe: %s\n%w", targetAddr, stderr, err)
		}
	}
...
func (d *pure) getMappedDevPath(vol Volume, mapVolume bool) (string, revert.Hook, error) {
	...
	case pureModeNVMe:
		...
		// to lower case, as on host the device ID is completely lower case.
