...
// existed.
//
// The LUN is required by the SCSI/FC connector, which uses it to scope the SCSI bus rescan. It is
// a SCSI concept, so it is not meaningful for the NVMe modes, which address namespaces by NSID;
// for those the returned value is unused.
...
func (d *pure) mapVolume(vol Volume) (cleanup revert.Hook, err error) {
	...
	// Ensure the volume is connected to the host, obtaining the assigned LUN. Only SCSI/FC
	// passes it to the connector, to scope the SCSI bus rescan. iSCSI addresses the volume
	// by target and IQN, and the NVMe modes have no LUN at all - they address namespaces by
	// NSID - so the value is unused outside SCSI/FC.
