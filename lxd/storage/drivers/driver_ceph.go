...
func (d *ceph) Validate(config map[string]string) error {
	...
	for k := range config {
		...
		// to a different peer.
		//
		// Setting it requires `ceph.rbd.clone_copy=false`, set before any container is created on
		// the pool, because Ceph cannot mirror a volume that is a clone of its image.
		// ---
		//  type: string
