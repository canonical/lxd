...
func Test_ceph_Validate_replicatorPoolKey(t *testing.T) {
	tests := []struct {
		...
		{"Replicator key naming a peer site", map[string]string{"ceph.replicator.dr": "site-b", "ceph.rbd.clone_copy": "false"}, false},
		{"Replicator key of a project whose name holds a full stop", map[string]string{"ceph.replicator.dr.eu": "site-b", "ceph.rbd.clone_copy": "false"}, false},
		...
		{"Replicator key while container volumes are clones", map[string]string{"ceph.replicator.dr": "site-b"}, true},
		{"Replicator key while clone copy is explicitly on", map[string]string{"ceph.replicator.dr": "site-b", "ceph.rbd.clone_copy": "true"}, true},
