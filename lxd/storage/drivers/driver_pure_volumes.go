...
func (d *pure) MountVolume(vol Volume, op *operations.Operation) error {
	...
	if vol.contentType == ContentTypeFS {
		...
		if !filesystem.IsMountPoint(mountPath) {
			...
			d.logger.Debug("Mounted PureStorage volume", logger.Ctx{"volName": vol.name, "dev": volDevPath, "path": mountPath, "options": mountOptions})
		}
...
func (d *pure) RenameVolume(vol Volume, newVolName string, op *operations.Operation) error {
	// Renaming a volume won't change an actual name of the PureStorage volume.
	return nil
}
...
func (d *pure) RenameVolumeSnapshot(snapVol Volume, newSnapshotName string, op *operations.Operation) error {
	// Renaming a volume snapshot won't change an actual name of the PureStorage volume snapshot.
	return nil
}
