...
func (d *powerstore) EnsureImage(imgVol Volume, filler *VolumeFiller, progressReporter ioprogress.ProgressReporter) error {
...
// ensureHost returns the ID of the host that is configured with the local initiator
// qualified names, registering any that are missing from it. If no such host exists,
// it creates a new host using the server name with the mode appended as the host name.
func (d *powerstore) ensureHost() (hostID string, cleanup revert.Hook, err error) {
	...
	if err != nil {
		...
		// The host is identified by a match on any one of its initiators.
		// Register the ones it is missing, otherwise the array never presents its volumes
		// to those ports and their paths stay unused.
		missingQNs, err := host.MissingQualifiedNames(connector.Type(), qns)
		if err != nil {
			return "", nil, err
		}

		if len(missingQNs) > 0 {
			d.logger.Info("Registering additional initiators with PowerStore host", logger.Ctx{"host": host.Name, "initiators": missingQNs})

			err = client.AddHostInitiators(hostID, connector.Type(), missingQNs)
			if err != nil {
				return "", nil, err
			}
		}
