...
func (d *alletra) CreateVolumeFromCopy(vol VolumeCopy, srcVol VolumeCopy, allowInconsistent bool, op *operations.Operation) error {
	...
	if !srcVol.IsSnapshot() {
		for _, snapshot := range vol.Snapshots {
			...
			err = d.client().CreateVolumePhysicalCopy(poolName, srcSnapshotName, volName)
			if err != nil {
	...
	err = d.client().CreateVolumePhysicalCopy(poolName, srcVolName, volName)
	if err != nil {
		return err
	}
...
func (d *alletra) refreshVolume(vol VolumeCopy, srcVol VolumeCopy, refreshSnapshots []string, allowInconsistent bool, op *operations.Operation) (revert.Hook, error) {
	...
	revert.Add(func() {
		...
		err := d.client().RestoreVolumeSnapshot(vol.pool, volName, reverterSnapshotName)
		if err != nil {
			d.logger.Error("RestoreVolumeSnapshot API call failed on error path", logger.Ctx{"err": err})
		}
	...
	if !srcVol.IsSnapshot() && len(refreshSnapshots) > 0 {
		...
		for _, snapshot := range vol.Snapshots {
			...
			revert.Add(func() {
				...
				if err != nil {
					d.logger.Error("DeleteVolumeSnapshot failed on error path", logger.Ctx{"err": err})
				}
			})
	...
	if filesystem.IsMountPoint(sourcePath) {
		...
		if err == nil {
			defer func() {
				err := unfreezeFS()
				if err != nil {
					d.logger.Error("unfreezeFS failed on error path", logger.Ctx{"err": err})
				}
			}()
		}
	}
...
func (d *alletra) RestoreVolume(vol Volume, snapVol Volume, op *operations.Operation) error {
	...
	if ourUnmount {
		defer func() {
			...
			if err != nil {
				d.logger.Error("MountVolume failed on error path", logger.Ctx{"err": err})
			}
		}()
	}
