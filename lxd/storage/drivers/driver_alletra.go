...
type alletra struct {
	...
	// Holds connection targets.
	targets []connectors.Target
...
func (d *alletra) load() error {
	...
	connnectorTypes := make([]connectors.ConnectorType, len(alletraSupportedConnectors))
	for i := range alletraSupportedConnectors {
		connnectorTypes[i] = connectors.ConnectorType(alletraSupportedConnectors[i])
	}
...
func (d *alletra) connector() (connectors.Connector, error) {
	if d.storageConnector == nil {
		connector, err := connectors.NewConnector(connectors.ConnectorType(d.config["alletra.mode"]), d.state.OS.ServerUUID)
		if err != nil {
			return nil, err
		}
...
func (d *alletra) FillConfig() error {
	...
	if d.config["alletra.mode"] == "" {
		d.config["alletra.mode"] = string(connectors.TypeNVME)
	}
...
// discoverTargets discovers targets matching the given addresses.
func (d *alletra) discoverTargets(targetAddresses ...string) ([]connectors.Target, error) {
	...
	targets = connectors.MatchTargetAddresses(connector.Type(), targets, targetAddresses...)
	if len(targets) == 0 {
		return nil, errors.New("Could not discover target QN")
	}
...
// getTargets discovers the targetQN and target's IP addresses list.
func (d *alletra) getTargets() (targets []connectors.Target, err error) {
	// First check if target addresses are configured, otherwise, use the discovered ones.
	targetAddrs := shared.SplitNTrimSpace(d.config["alletra.target"], ",", -1, true)
	...
	if len(targetAddrs) == 0 {
		targetAddrs, err = d.client().GetTargetAddrs(connector.Type())
		if err != nil {
			return nil, err
		}
	}
	...
	if len(targetAddrs) == 0 {
		return nil, fmt.Errorf("No usable target found for mode %q", connector.Type())
	}

	// Discover the targetQN from any of the addresses.
	targets, err = d.discoverTargets(targetAddrs...)
	if err != nil {
		return nil, err
	}
