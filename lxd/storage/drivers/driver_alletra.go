...
func (d *alletra) client() *alletraClient {
	if d.httpClient == nil {
		d.httpClient = newAlletraClient(d)
	}
...
func (d *alletra) Validate(config map[string]string) error {
	rules := map[string]func(value string) error{
		// lxdmeta:generate(entities=storage-alletra; group=pool-conf; key=alletra.wsapi.url)
		//
		// ---
		//  type: string
		//  shortdesc: Address of the HPE Alletra Storage UI/WSAPI
		"alletra.wsapi.url": validate.Optional(validate.IsRequestURL),
		...
		// lxdmeta:generate(entities=storage-alletra; group=pool-conf; key=alletra.wsapi.user.name)
		//
		// ---
		//  type: string
		//  shortdesc: HPE Alletra storage admin username
		"alletra.wsapi.user.name": validate.IsAny,
		// lxdmeta:generate(entities=storage-alletra; group=pool-conf; key=alletra.wsapi.user.password)
		//
		// ---
		//  type: string
		//  shortdesc: HPE Alletra storage admin password
		"alletra.wsapi.user.password": validate.IsAny,
		// lxdmeta:generate(entities=storage-alletra; group=pool-conf; key=alletra.wsapi.cpg)
		//
		// ---
		//  type: string
		//  shortdesc: HPE Alletra CPG name
		"alletra.wsapi.cpg": validate.IsAny,
...
func (d *alletra) Create() error {
	...
	if d.config["alletra.wsapi.url"] == "" {
		return errors.New("The alletra.wsapi.url cannot be empty")
	}

	if d.config["alletra.wsapi.user.name"] == "" {
		return errors.New("The alletra.wsapi.user.name cannot be empty")
	}

	if d.config["alletra.wsapi.user.password"] == "" {
		return errors.New("The alletra.wsapi.user.password cannot be empty")
	}
