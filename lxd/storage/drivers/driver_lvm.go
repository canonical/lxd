...
func (d *lvm) Create() error {
	...
	if vgExists {
		...
		if !pvExists {
			...
			_, err := shared.TryRunCommand(context.TODO(), tryCommandOpts, "pvcreate", pvName)
			if err != nil {
				return err
			}

			revert.Add(func() { _, _ = shared.TryRunCommand(context.TODO(), tryCommandOpts, "pvremove", pvName) })
		}

		// Create volume group.
		_, err := shared.TryRunCommand(context.TODO(), tryCommandOpts, "vgcreate", d.config["lvm.vg_name"], pvName)
		if err != nil {
			return err
		}
		...
		revert.Add(func() {
			_, _ = shared.TryRunCommand(context.TODO(), tryCommandOpts, "vgremove", d.config["lvm.vg_name"])
		})
	}
	...
	_, err = shared.TryRunCommand(context.TODO(), tryCommandOpts, "vgchange", "--addtag", lvmVgPoolMarker, d.config["lvm.vg_name"])
	if err != nil {
		return err
	}
...
func (d *lvm) Delete(op *operations.Operation) error {
	...
	if vgExists && shared.IsFalseOrEmpty(d.config["lvm.vg.force_reuse"]) {
		...
		if removeVg {
			_, err := shared.TryRunCommand(context.TODO(), tryCommandOpts, "vgremove", "-f", d.config["lvm.vg_name"])
			if err != nil {
			...
			if slices.Contains(vgTags, lvmVgPoolMarker) {
				_, err = shared.TryRunCommand(context.TODO(), tryCommandOpts, "vgchange", "--deltag", lvmVgPoolMarker, d.config["lvm.vg_name"])
				if err != nil {
	...
	if removeVg && loopDevPath != "" {
		_, err := shared.TryRunCommand(context.TODO(), tryCommandOpts, "pvremove", "-f", loopDevPath)
		if err != nil {
...
func (d *lvm) Update(changedConfig map[string]string) error {
	...
	if changedConfig["lvm.vg_name"] != "" {
		_, err := shared.TryRunCommand(context.TODO(), tryCommandOpts, "vgrename", d.config["lvm.vg_name"], changedConfig["lvm.vg_name"])
		if err != nil {
	...
	if changedConfig["lvm.thinpool_name"] != "" {
		_, err := shared.TryRunCommand(context.TODO(), tryCommandOpts, "lvrename", d.config["lvm.vg_name"], d.thinpoolName(), changedConfig["lvm.thinpool_name"])
		if err != nil {
