...
func (d *zfs) deleteDatasetRecursive(dataset string) error {
	...
	_, err = shared.TryRunCommand(context.TODO(), tryCommandOpts, "zfs", "destroy", "-r", dataset)
	if err != nil {
		return err
	}
	...
	if origin != "" && origin != "-" {
		...
		if strings.Contains(originToDelete, "@") && strings.HasPrefix(originToDelete, d.config["zfs.pool_name"]+"/deleted/") {
			...
			_, err = shared.TryRunCommand(context.TODO(), tryCommandOpts, "zfs", "destroy", originToDelete)
			if err != nil {
				return err
			}
