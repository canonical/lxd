...
func validatePoolCommonRules() map[string]func(string) error {
	rules := map[string]func(string) error{
		...
		// lxdmeta:generate(entities=storage-dir,storage-lvm,storage-btrfs,storage-zfs,storage-ceph,storage-cephfs; group=pool-conf; key=source.reuse)
		// Set this option to `true` to reuse an existing `source`
		// which might already contain volumes.
		// ---
		//  type: bool
		//  defaultdesc: `false`
		//  shortdesc: Whether to reuse an existing `source`
		//  scope: local
