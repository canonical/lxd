...
var ConfigSchema = config.Schema{
	...
	// lxdmeta:generate(entities=server; group=core; key=oidc.session.expiry)
	...
	"oidc.session.expiry": {Default: "1w", Validator: func(s string) error {
		...
		// TODO: This should be less than the auth secret expiry.
		if exp.Sub(now) > 31*24*time.Hour {
			return errors.New("OIDC session expiry cannot be set to greater than one month")
		}
