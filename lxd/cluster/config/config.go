...
// AuthSecretExpiry returns the number of seconds that the auth secret is considered valid after creation.
func (c *Config) AuthSecretExpiry() (int64, error) {
	exp, err := shared.GetExpiry(time.Unix(0, 0), c.m.GetString("core.auth_secret_expiry"))
	if err != nil {
		return -1, err
	}
...
var ConfigSchema = config.Schema{
	...
	"core.auth_secret_expiry": {Default: "1m", Validator: func(s string) error {
		exp, err := shared.GetExpiry(time.Unix(0, 0), s)
		if err != nil {
			return err
		}

		if time.Duration(exp.Unix())*time.Second < 24*time.Hour {
