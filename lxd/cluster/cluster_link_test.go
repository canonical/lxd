...
type testLXDServer struct {
	...
	// started is closed when the server first begins handling a request.
	started chan struct{}
...
func newTestLXDServer(t *testing.T, block <-chan struct{}) *testLXDServer {
	...
	server := &testLXDServer{started: make(chan struct{}), canceled: make(chan struct{})}

	var startedOnce sync.Once
	var canceledOnce sync.Once
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		startedOnce.Do(func() { close(server.started) })

		if block != nil {
			select {
			case <-block:
			case <-r.Context().Done():
				canceledOnce.Do(func() { close(server.canceled) })
				return
			}
		}
...
func TestGetNextMemberClient(t *testing.T) {
	...
	t.Run("cleanup aborts pending attempts and disconnects handed out clients", func(t *testing.T) {
		...
		// Wait until the pending attempt has actually reached the server and is blocking in its
		// handler. Otherwise cleanup could cancel it mid-connect, before the handler runs, and its
		// canceled channel would never be closed.
		<-pending.started
