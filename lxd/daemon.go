...
func (d *Daemon) nodeRefreshTask(heartbeatData *cluster.APIHeartbeat, isLeader bool, unavailableMembers []string) {
	...
	if isLeader && unavailableMembers != nil && len(heartbeatData.Members) > 1 {
		...
		// Precompute control-plane status
		controlPlaneCount := 0
		...
		controlPlaneActive := int64(controlPlaneCount) >= maxVoters
		...
		for _, node := range heartbeatData.Members {
			...
			if node.Online {
				...
				// Check if a non-control-plane node currently has a raft role other than spare.
		...
		if needsRebalance || hasNodesNotPartOfRaft {
			...
			if needsRebalance {
				...
				err := rebalanceMemberRoles(context.Background(), d.State(), d.gateway, unavailableMembers, controlPlaneActive, memberRoles)
