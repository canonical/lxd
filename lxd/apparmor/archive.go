...
func archiveProfile(s *state.State, outputPath string, allowedCommandPaths []string) (string, error) {
	...
	err = s.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		for _, project := range projects {
			projectImagesVolume, err := dbCluster.GetProjectConfigValue(ctx, tx.Tx(), project.Name, "storage.images_volume")
			if err != nil {
			...
			projectBackupsVolume, err := dbCluster.GetProjectConfigValue(ctx, tx.Tx(), project.Name, "storage.backups_volume")
			if err != nil {
				return fmt.Errorf("Failed to fetch project config: %w", err)
			}

			projectBackupsVolumes[projectBackupsVolume] = true
		}
