...
func doInstancesGet(s *state.State, r *http.Request) (any, error) {
	...
	userHasPermission, err := s.Authorizer.GetPermissionChecker(r, auth.RelationViewer, auth.ObjectTypeInstance)
	if err != nil {
		return nil, err
	}
	...
	for address, instances := range memberAddressInstances {
		...
		for _, inst := range instances {
			if !userHasPermission(auth.InstanceObject(inst.Project, inst.Name)) {
				continue
			}
