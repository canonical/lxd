...
func hashAndBlockKeys(s secret.Secret, salt []byte) ([]byte, []byte, error) {
	hash, block := cookieEncryptionKeys()
	err := extractKeys(s, salt, []key{hash, block}...)
	if err != nil {
		return nil, nil, err
	}

	return hash.value, block.value, nil
}
