...
func NewVerifier(ctx context.Context, issuer string, clientID string, clientSecret string, scopes []string, audience string, groupsClaim string, clusterUUID string, secretsFunc func(ctx context.Context) (cluster.AuthSecrets, error), httpClientFunc func() (*http.Client, error), sessionHandler SessionHandler) (*Verifier, error) {
	verifier := &Verifier{
		issuer:         issuer,
		clientID:       clientID,
	...
	// Ensure configuration is valid with dummy host.
	err := verifier.ensureConfig(ctx, "localhost")
	if err != nil {
