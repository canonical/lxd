...
func (o *Verifier) secureCookieFromSession(ctx context.Context, sessionID uuid.UUID) (*securecookie.SecureCookie, error) {
	...
	for i := 0; i < len(secrets); i++ {
		...
		if secrets[i].CreatedAt > sessionStartedAtSeconds {
			continue
		}
	}
...
func NewVerifier(issuer string, clientID string, clientSecret string, scopes []string, audience string, secretsFunc func(ctx context.Context) (cluster.Secrets, error), identityCache *identity.Cache, httpClientFunc func() (*http.Client, error), options *Opts) (*Verifier, error) {
	opts := &Opts{}
