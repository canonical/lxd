...
func (o *Verifier) verifySession(r *http.Request, w http.ResponseWriter, sessionToken string) (*AuthenticationResult, error) {
	...
	secret, startNewSession, err := o.getSecretFromUsedAtTime(r.Context(), issuedAt.Unix())
	if err != nil {
...
func (o *Verifier) startSession(r *http.Request, w http.ResponseWriter, res AuthenticationResult, tokens *oidc.Tokens[*oidc.IDTokenClaims], expiryOverride *time.Time) error {
	...
	http.SetCookie(w, &http.Cookie{
		...
		Value:    token,
		// The cookie has to have an expiry for the persistent cookie jar (CLI) to save it to disk.
		// This sets the cookie to expire one week after the session expires. This allows sessions to be refreshed.
		Expires: expiry.Add(time.Hour * 24 * 7),
	})

	return nil
}

// endSession deletes the session cookie. It does not delete the session from the database.
...
// given unix time in seconds. A boolean is returned to indicate if the returned secret is not the most recent, this
// prompts the Verifier to refresh the session token, so that it doesn't become unverifiable.
func (o *Verifier) getSecretFromUsedAtTime(ctx context.Context, usedAtTimeUnixSeconds int64) (*cluster.AuthSecret, bool, error) {
