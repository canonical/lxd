...
// LXDClaims represents the claims of a LXD bearer token.
type LXDClaims struct {
	jwt.RegisteredClaims

	// ServerFingerprint is the fingerprint of the network certificate of the server.
...
// GetLXDBearerToken generates and signs a token for use with the main LXD API. For claims it has:
// - Subject (sub): Identity identifier (UUID)
// - Issuer (iss): "lxd:{cluster_uuid}"
...
func GetLXDBearerToken(secret []byte, identityIdentifier string, clusterUUID string, expiresAt time.Time, serverCertFingerprint string) (string, error) {
