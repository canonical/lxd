...
func (e *embeddedOpenFGA) GetViewableProjects(ctx context.Context, permissions []api.Permission) ([]string, error) {
	// Use a different context from the given context.
	// Why? Because the OpenFGA datastore will attempt to load a cache that is optimised for permission checking an individual request.
	// In this case we are asking the datastore to enumerate projects based on contextual tuples. The cache is not useful for this, so we don't
	// want to trigger loading it.
	...
	for _, permission := range permissions {
		req.ContextualTuples.TupleKeys = append(req.ContextualTuples.TupleKeys, &openfgav1.TupleKey{
			User:     groupObject + "#member", // Members of the dummy group have permission, not the group itself.
