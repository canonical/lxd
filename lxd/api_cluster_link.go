...
func clusterLinkCreateActive(s *state.State, r *http.Request, req api.ClusterLinksPost, clusterLinkType dbCluster.ClusterLinkType, networkCert *shared.CertInfo, notify identityNotificationFunc, requestor *api.EventLifecycleRequestor, trustToken *api.CertificateAddToken) response.Response {
	...
	for {
		...
		targetClient, err := cluster.ConnectClusterLink(r.Context(), s, req.Name)
		if err != nil {
			logger.Warn("Failed connecting to cluster link after link creation", logger.Ctx{"err": err, "clusterLinkName": req.Name})
		} else if targetClient != nil {
			err = cluster.RefreshClusterLinkVolatileAddresses(r.Context(), s, req.Name, targetClient)
...
func clusterLinkActivate(s *state.State, r *http.Request, req api.ClusterLinksPost, notify identityNotificationFunc, requestor *api.EventLifecycleRequestor, trustToken *api.CertificateAddToken, addresses []string) response.Response {
	...
	targetClient, err := cluster.ConnectClusterLink(r.Context(), s, clusterLinkName)
	if err != nil {
		logger.Warn("Failed connecting to cluster link after link activation", logger.Ctx{"err": err, "clusterLinkName": clusterLinkName})
	} else if targetClient != nil {
		err = cluster.RefreshClusterLinkVolatileAddresses(r.Context(), s, clusterLinkName, targetClient)
		targetClient.Disconnect()
		if err != nil {
...
func autoRefreshClusterLinkVolatileAddresses(ctx context.Context, s *state.State) error {
	...
	for _, name := range names {
		targetClient, err := cluster.ConnectClusterLink(ctx, s, name)
		if err != nil {
			logger.Warn("Failed connecting to cluster link", logger.Ctx{"err": err, "clusterLinkName": name})
			continue
		}

		if targetClient == nil {
			// Pending or otherwise incomplete cluster link with no addresses to refresh.
			continue
		}

		err = cluster.RefreshClusterLinkVolatileAddresses(ctx, s, name, targetClient)
		targetClient.Disconnect()
		if err != nil {
