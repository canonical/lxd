...
func imageRegistriesGet(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		allImageRegistries, err := dbCluster.GetImageRegistries(ctx, tx.Tx())
		if err != nil {
			return fmt.Errorf("Failed loading image registries: %w", err)
		}

		imageRegistries := make([]dbCluster.ImageRegistryRow, 0, len(allImageRegistries))
		...
		if recursion > 0 {
			...
			for _, registry := range imageRegistries {
				apiImageRegistry, err := registry.ToAPI(ctx, tx.Tx())
				if err != nil {
					return err
				}
...
func imageRegistryGet(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		apiImageRegistry, err = dbImageRegistry.ToAPI(ctx, tx.Tx())
		return err
	})
	if err != nil {
		return response.SmartError(err)
...
func imageRegistriesPost(d *Daemon, r *http.Request) response.Response {
	...
	registryPublic := shared.IsTrue(req.Config["public"])
...
func imageRegistryPost(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		if imageRegistry.Builtin {
			return api.StatusErrorf(http.StatusBadRequest, "Built-in image registry cannot be renamed")
		}
...
func updateImageRegistry(s *state.State, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		// Fetch its config and convert to API representation.
		existingRegistry, err = dbImageRegistry.ToAPI(ctx, tx.Tx())
		return err
	})
	if err != nil {
		return response.SmartError(err)
	...
	// Flag to track whether the image registry record itself needs an update.
	...
	registryPublic := shared.IsTrue(existingRegistry.Config["public"])
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		// Update the image registry record only if the description needs an update.
		// Protocol is immutable and name can only be changed using the renaming endpoint.
		if descriptionChanged {
			err = dbCluster.UpdateImageRegistry(ctx, tx.Tx(), existingRegistry.Name, dbCluster.ImageRegistryRow{
...
func imageRegistryDelete(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		if imageRegistry.Builtin {
			return api.StatusErrorf(http.StatusBadRequest, "Built-in image registry cannot be deleted")
		}
...
func imageRegistryValidate(registry api.ImageRegistry) error {
	...
	registryPublic := shared.IsTrue(registry.Config["public"])
	...
	case api.ImageRegistryProtocolLXD:
		...
		if !registryPublic && registryCluster == "" {
...
func imageRegistryValidateConfig(config map[string]string) error {
	imageRegistryConfigKeys := map[string]func(value string) error{
		...
		//  type: string
		//  required: yes
		//  shortdesc: Source URL for the image registry
		"url": validate.Required(validate.IsRequestURL),
		// lxdmeta:generate(entities=image-registry; group=image-registry-conf; key=public)
		//
		// ---
		//  type: bool
		//  required: no
		//  defaultdesc: `true`
		//  shortdesc: Whether the image registry can be accessed without authentication
		"public": validate.Optional(validate.IsBool),
