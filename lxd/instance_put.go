...
func instancePut(d *Daemon, r *http.Request) response.Response {
	...
	if configRaw.Restore == "" {
		if configRaw.DisksMode != "" {
			return response.BadRequest(errors.New(`No snapshot provided on "restore" and "disks" was provided`))
		}

		// "excluded_disks" do not apply if not preforming a restore.
		if len(configRaw.ExcludeDisks) > 0 {
			return response.BadRequest(errors.New(`No snapshot provided on "restore" and "excluded_disks" was provided`))
		}
...
func instanceSnapRestore(s *state.State, projectName string, name string, req api.InstancePut) error {
	...
	// Get which disks should be included apart from the root disk.
	disks, err := instance.ParseSnapshotDisks(source, req.DisksMode, req.ExcludeDisks)
	if err != nil {
