...
func instanceSnapRestore(s *state.State, projectName string, name string, req api.InstancePut) error {
	...
	var diskVolumes []*api.StorageVolume
	if req.RestoreDiskVolumesMode == api.DiskVolumesModeAllExclusive {
		diskVolumes, err = instance.ResolveAttachedVolumeSnapshotsForRestore(s, source, req.RestoreDiskVolumesMode)
		if err != nil {
			return err
		}
	}
	...
	err = inst.Restore(source, req.Stateful, diskVolumes)
	if err != nil {
		return err
	}
