...
const (
	// ProtocolCluster is set as the RequestorDetails.Protocol when the request is authenticated via mTLS and the peer
	// certificate is present in the trust store as type certificate.TypeServer.
	ProtocolCluster string = "cluster"

	// ProtocolUnix is set as the RequestorDetails.Protocol when the request is made over the unix socket.
	ProtocolUnix string = "unix"

	// ProtocolPKI is set as the RequestorDetails.Protocol when a `server.ca` file exists in LXD_DIR, the peer
	...
	// RequestorDetails.Protocol is set to api.AuthenticationMethodTLS.
	//
	...
	// the identity should be set as the RequestorDetails.Identity.
