...
func profilesGet(d *Daemon, r *http.Request) response.Response {
	...
	err = d.cluster.Transaction(func(tx *db.ClusterTx) error {
		...
		for i, profile := range profiles {
			...
			apiProfiles[i], err = profile.ToAPI(tx)
			if err != nil {
				return err
			}

			apiProfiles[i].UsedBy = project.FilterUsedBy(r, usedBy)
		}
...
func profileUsedBy(tx *db.ClusterTx, profile db.Profile) ([]string, error) {
	...
	for i, inst := range dbInsts {
		...
		usedBy[i] = apiInst.URL(version.APIVersion, profile.Project).String()
	}
...
func profileGet(d *Daemon, r *http.Request) response.Response {
	...
	err = d.cluster.Transaction(func(tx *db.ClusterTx) error {
		...
		resp, err = profile.ToAPI(tx)
		if err != nil {
			return err
		}

		resp.UsedBy = project.FilterUsedBy(r, usedBy)
