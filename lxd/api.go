...
func restServer(d *Daemon) *http.Server {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		if isBrowserClient(r) {
			...
			if isUIMagicLink {
				requestorArgs, err := bearer.Authenticate(token, subject, d.identityCache)
				if err == nil {
					http.SetCookie(w, &http.Cookie{
						...
						// Set max age. Otherwise, the cookie will be deleted when the
						// browser is closed.
						MaxAge: int(time.Until(*requestorArgs.TokenExpiresAt).Seconds()),
					})
