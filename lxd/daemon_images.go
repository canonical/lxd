...
func ImageDownload(reqContext context.Context, s *state.State, op *operations.Operation, args *ImageDownloadArgs) (*api.Image, error) {
	var err error
	...
	if err == nil {
		...
		if nodeAddress != "" {
			...
			err = instanceImageTransfer(reqContext, s, args.ProjectName, imgInfo.Fingerprint, nodeAddress)
			if err != nil {
		...
		if err == nil {
			...
			if nodeAddress != "" {
				// The image is available from another node, let's try to import it.
				err = instanceImageTransfer(reqContext, s, args.ProjectName, info.Fingerprint, nodeAddress)
				if err != nil {
	...
	} else if request.IsRequestContext(reqContext) {
