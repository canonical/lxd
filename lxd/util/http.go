...
func EtagCheck(r *http.Request, data any) error {
	return EtagCheckString(r.Header.Get("If-Match"), data)
}

// EtagCheckString validates the hash of the current state with the hash
// provided by the client.
func EtagCheckString(etag string, data any) error {
	if etag == "" {
		return nil
	}
	...
	if hash != etag {
		return api.StatusErrorf(http.StatusPreconditionFailed, "ETag doesn't match: %s vs %s. The configuration has been modified since this change began. Please retrieve the updated configuration before proceeding.", hash, etag)
	}
