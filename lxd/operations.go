...
func autoRemoveOrphanedOperationsTask(stateFunc func() *state.State) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		op, err := operations.CreateServerOperation(s, "", operations.OperationClassTask, operationtype.RemoveOrphanedOperations, nil, nil, opRun, nil, nil)
		if err != nil {
...
func operationWaitHandler(d *Daemon, r *http.Request) response.Response {
	...
	var onConnect operations.ConnectFunc
	...
	op, err := operations.CreateUserOperation(d.State(), request.QueryParam(r, "project"), requestor, req.OpClass, req.OpType, resources, nil, run, nil, onConnect)
	if err != nil {
		return response.InternalError(err)
	}
