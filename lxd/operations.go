...
func operationCancel(reqContext context.Context, s *state.State, projectName string, op *api.Operation) error {
	...
	if localOp != nil {
		...
		s.Events.SendLifecycle(projectName, lifecycle.OperationCancelled.Event(localOp, request.CreateRequestor(reqContext), nil))
	...
	client, err := cluster.Connect(reqContext, memberAddress, s.Endpoints.NetworkCert(), s.ServerCert(), true)
	if err != nil {
...
func operationsGetByType(reqContext context.Context, s *state.State, projectName string, opType operationtype.Type) ([]*api.Operation, error) {
	...
	for memberAddress := range memberOps {
		...
		// Connect to the remote server. Use notify=true to only get local operations on remote member.
		client, err := cluster.Connect(reqContext, memberAddress, networkCert, serverCert, true)
		if err != nil {
