...
func operationDelete(d *Daemon, r *http.Request) response.Response {
	...
	// Don't forward to ourselves, or forward request for finalized operation.
...
func operationsGet(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		dbOps, err := dbCluster.GetOperationsWithAddress(ctx, tx.Tx(), filter...)
		if err != nil {
		...
		for _, dbOp := range dbOps {
			if operations.OperationClass(dbOp.Class) == operations.OperationClassDurable {
				...
				if err != nil {
					return fmt.Errorf("Failed loading durable operation ID %q: %w", dbOp.Reference, err)
				}
				...
				if err != nil {
					return fmt.Errorf("Failed converting durable operation ID %q to API representation: %w", dbOp.Reference, err)
				}
				...
				for _, memberInMembers := range members {
					if dbOp.NodeID != nil && memberInMembers.ID == *dbOp.NodeID {
...
func syncDurableOperationsTask(stateFunc func() *state.State) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		for _, dbOp := range dbOps {
			...
			operations.CreateDurableOperation(s, dbOp)
		}
