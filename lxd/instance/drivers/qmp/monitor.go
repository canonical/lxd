...
func (m *Monitor) start() error {
	...
	go func() {
		...
		// We use this variable to record if a normal shutdown event has been seen, and that way when the
		// monitor eventually disconnects we do not trigger another shutdon event. In this way if the
		// monitor disconnects unexpectedly then we can trigger a shutdown event so devices are cleaned up.
		var shutdownEventSeen bool
		...
		for {
			...
			case e, more := <-chEvents:
				...
				if !more {
					// If disconnection happened unexpectedly then send shutdown event so that
					// VM devices can be cleaned up on the host.
					if !shutdownEventSeen && m.eventHandler != nil {
