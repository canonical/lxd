...
func (d *lxc) initLXC(config bool) (*liblxc.Container, error) {
	...
	err = lxcSetConfigItem(cc, "lxc.hook.pre-start", fmt.Sprintf("/proc/%d/exe callhook %s %s %s start", os.Getpid(), shared.VarPath(""), shared.SingleQuote(d.Project().Name), shared.SingleQuote(d.Name())))
	if err != nil {
		return nil, err
	}
	...
	err = lxcSetConfigItem(cc, "lxc.hook.stop", fmt.Sprintf("%s callhook %s %s %s stopns", lxdStopHookPath, shared.VarPath(""), shared.SingleQuote(d.Project().Name), shared.SingleQuote(d.Name())))
	if err != nil {
		return nil, err
	}

	// Call the onstop hook on stop.
	err = lxcSetConfigItem(cc, "lxc.hook.post-stop", fmt.Sprintf("%s callhook %s %s %s stop", lxdStopHookPath, shared.VarPath(""), shared.SingleQuote(d.Project().Name), shared.SingleQuote(d.Name())))
	if err != nil {
		return nil, err
	}
	...
	if shared.IsTrue(d.expandedConfig["security.delegate_bpf"]) {
		err = lxcSetConfigItem(cc, "lxc.hook.start-host", d.state.OS.ExecPath+" callhook "+shared.VarPath("")+" "+shared.SingleQuote(d.Project().Name)+" "+shared.SingleQuote(d.Name())+" starthost")
		if err != nil {
			return nil, err
		}
	}
...
func (d *lxc) startCommon() (revert.Hook, string, []func() error, error) {
	...
	if len(cdiConfigFiles) > 0 {
		err = lxcSetConfigItem(cc, "lxc.hook.mount", d.state.OS.ExecPath+" callhook "+shared.VarPath("")+" "+shared.SingleQuote(d.Project().Name)+" "+shared.SingleQuote(d.Name())+" startmountns --devicesRootFolder "+d.DevicesPath()+" "+strings.Join(cdiConfigFiles, " "))
		if err != nil {
