...
func (s *storageZfs) doCrossPoolContainerCopy(target container, source container, containerOnly bool) error {
	...
	defer s.ContainerUmount(target, shared.VarPath("containers", target.Name()))
...
func (s *storageZfs) doContainerBackupLoadVanilla(info backupInfo, data io.ReadSeeker, tarArgs []string) error {
	...
	if err != nil {
		s.doContainerDelete("default", info.Name)
		return err
	}

	_, err = s.doContainerMount("default", info.Name, info.Privileged)
	if err != nil {
		return err
	}
