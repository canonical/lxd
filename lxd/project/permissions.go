...
func FilterUsedBy(reqContext context.Context, authorizer auth.Authorizer, entries []string) []string {
	...
	for entityType, urls := range urlsByEntityType {
		...
		if len(urls) == 1 {
			err := authorizer.CheckPermission(reqContext, urls[0], auth.EntitlementCanView)
			if err != nil {
				continue
			}
		...
		canViewEntity, err := authorizer.GetPermissionChecker(reqContext, auth.EntitlementCanView, entityType)
		if err != nil {
