...
func CheckClusterTargetRestriction(reqContext context.Context, authorizer auth.Authorizer, project *api.Project, targetFlag string) error {
	if projectHasRestriction(project, "restricted.cluster.target", "block") && targetFlag != "" {
		...
		err := authorizer.CheckPermission(reqContext, entity.ServerURL(), auth.EntitlementCanOverrideClusterTargetRestriction)
...
func CheckTarget(ctx context.Context, reqContext context.Context, authorizer auth.Authorizer, tx *db.ClusterTx, p *api.Project, target string, allMembers []db.NodeInfo) (*db.NodeInfo, string, error) {
	targetMemberName, targetGroupName := shared.TargetDetect(target)

	// Check manual cluster member targeting restrictions.
	err := CheckClusterTargetRestriction(reqContext, authorizer, p, target)
	if err != nil {
		return nil, "", err
	}
