...
func doCustomVolumeRefresh(s *state.State, r *http.Request, requestProjectName string, projectName string, poolName string, req *api.StorageVolumesPost) response.Response {
	...
	op, err := operations.CreateUserOperation(s, requestProjectName, requestor, operations.OperationClassTask, operationtype.VolumeCopy, nil, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
...
func doVolumeMigration(s *state.State, r *http.Request, requestProjectName string, projectName string, poolName string, req *api.StorageVolumesPost) response.Response {
	...
	if push {
		op, err = operations.CreateUserOperation(s, requestProjectName, requestor, operations.OperationClassWebsocket, operationtype.VolumeCreate, resources, sink.Metadata(), run, nil, sink.Connect)
		if err != nil {
			return response.InternalError(err)
		}
	} else {
		op, err = operations.CreateUserOperation(s, requestProjectName, requestor, operations.OperationClassTask, operationtype.VolumeCopy, resources, nil, run, nil, nil)
		if err != nil {
			return response.InternalError(err)
		}
	}
...
func storagePoolVolumePost(d *Daemon, r *http.Request) response.Response {
	...
	if s.ServerClustered && target != "" && req.Source.Location != "" && req.Migration {
		...
		op, err := operations.CreateUserOperation(s, effectiveProjectName, requestor, operations.OperationClassTask, operationtype.VolumeMigrate, resources, nil, run, nil, nil)
		if err != nil {
			return response.InternalError(err)
		}
...
func storageVolumePostClusteringMigrate(ctx context.Context, s *state.State, srcPool storagePools.Pool, srcProjectName string, srcVolumeName string, newPoolName string, newProjectName string, newVolumeName string, srcMember db.NodeInfo, newMember db.NodeInfo, volumeOnly bool) (operations.RunFunc, error) {
	...
	run := func(ctx context.Context, op *operations.Operation) error {
		...
		srcOp, err := operations.CreateUserOperation(s, srcProjectName, requestor, operations.OperationClassWebsocket, operationtype.VolumeMigrate, resources, srcMigration.Metadata(), run, cancel, srcMigration.Connect)
		if err != nil {
			return err
		}
...
func storagePoolVolumeTypePostMigration(state *state.State, r *http.Request, requestProjectName string, projectName string, poolName string, volumeName string, req api.StorageVolumePost) response.Response {
	...
	if req.Target != nil {
		// Push mode.
		op, err := operations.CreateUserOperation(state, requestProjectName, requestor, operations.OperationClassTask, operationtype.VolumeMigrate, resources, nil, run, nil, nil)
		if err != nil {
			return response.InternalError(err)
		}
	...
	// Pull mode.
	op, err := operations.CreateUserOperation(state, requestProjectName, requestor, operations.OperationClassWebsocket, operationtype.VolumeMigrate, resources, ws.Metadata(), run, nil, ws.Connect)
	if err != nil {
		return response.InternalError(err)
	}
...
func storagePoolVolumeTypePostRename(s *state.State, r *http.Request, poolName string, projectName string, vol *api.StorageVolume, req api.StorageVolumePost) response.Response {
	...
	op, err := operations.CreateUserOperation(s, request.ProjectParam(r), requestor, operations.OperationClassTask, operationtype.VolumeMove, resources, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
...
func storagePoolVolumeTypePostMove(s *state.State, r *http.Request, poolName string, requestProjectName string, projectName string, vol *api.StorageVolume, req api.StorageVolumePost) response.Response {
	...
	op, err := operations.CreateUserOperation(s, requestProjectName, requestor, operations.OperationClassTask, operationtype.VolumeMove, nil, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
...
func storagePoolVolumePut(d *Daemon, r *http.Request) response.Response {
	...
	op, err := operations.CreateUserOperation(s, request.ProjectParam(r), requestor, operations.OperationClassTask, operationtype.VolumeUpdate, nil, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
...
func doStoragePoolVolumeDelete(ctx context.Context, s *state.State, name string, volType cluster.StoragePoolVolumeType, pool storagePools.Pool, requestProjectName string, effectiveProjectName string) (*operations.Operation, error) {
	...
	op, err := operations.CreateUserOperation(s, requestProjectName, requestor, operations.OperationClassTask, operationtype.VolumeDelete, resources, nil, run, nil, nil)
	if err != nil {
		return nil, err
	}
...
func createStoragePoolVolumeFromISO(s *state.State, r *http.Request, requestProjectName string, projectName string, data io.Reader, pool string, volName string) response.Response {
	...
	op, err := operations.CreateUserOperation(s, requestProjectName, requestor, operations.OperationClassTask, operationtype.VolumeCreate, resources, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
...
func createStoragePoolVolumeFromBackup(s *state.State, r *http.Request, requestProjectName string, projectName string, data io.Reader, pool string, volName string) response.Response {
	...
	op, err := operations.CreateUserOperation(s, requestProjectName, requestor, operations.OperationClassTask, operationtype.CustomVolumeBackupRestore, resources, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
