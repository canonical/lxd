...
func storagePoolVolumeDelete(d *Daemon, r *http.Request) response.Response {
	...
	run := func(op *operations.Operation) error {
		return doStoragePoolVolumeDelete(r.Context(), s, details.volumeName, details.volumeType, details.pool, requestProjectName, effectiveProjectName, op)
	}
	...
	op, err := operations.OperationCreate(r.Context(), s, requestProjectName, operations.OperationClassTask, operationtype.VolumeDelete, resources, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
...
func doStoragePoolVolumeDelete(ctx context.Context, s *state.State, name string, volType cluster.StoragePoolVolumeType, pool storagePools.Pool, requestProjectName string, effectiveProjectName string, op *operations.Operation) error {
