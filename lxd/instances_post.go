...
// ensureDownloadedImageFitWithinBudget handles image download, caching, and storage budget enforcement for a specific project.
...
// createFromImage creates a new instance from an image, handling image download, budget enforcement, and instance creation.
...
// createFromNone creates a new instance with no image (from "none") and handles instance creation as empty.
...
// createFromMigration handles instance creation from migration, whether it's a pull or push migration.
...
// createFromCopy creates an instance by copying an existing one with options and validations.
...
// createFromBackup creates an instance from a backup file, including pool selection, parsing backup info, and restoring to the target pool.
