...
func instancesPost(d *Daemon, r *http.Request) response.Response {
	...
	if req.Source.ImageRegistry != "" {
		err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
			...
			if !project.RegistryAllowed(projectConfig, req.Source.ImageRegistry, dbImageRegistry.Builtin) {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		case api.SourceTypeImage:
			...
			if req.Type == "" {
				...
				if imageRegistry == nil || imageRegistry.Protocol == api.ImageRegistryProtocolLXD {
					var imgType string
					if sourceImage != nil {
