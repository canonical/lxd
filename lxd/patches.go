...
func patchZfsSetContentTypeUserProperty(name string, d *Daemon) error {
	...
	err = s.DB.Cluster.Transaction(s.ShutdownCtx, func(ctx context.Context, tx *db.ClusterTx) error {
		for _, pool := range pools {
			...
			customVolumes, err := tx.GetStoragePoolVolumes(ctx, false, db.StorageVolumeFilter{Type: &volTypeCustom, PoolID: &poolID})
			if err != nil {
...
func patchStorageZfsUnsetInvalidBlockSettings(_ string, d *Daemon) error {
	...
	err = s.DB.Cluster.Transaction(s.ShutdownCtx, func(ctx context.Context, tx *db.ClusterTx) error {
		for _, pool := range pools {
			...
			// Get the pool's custom storage volumes.
			volumes, err := tx.GetStoragePoolVolumes(ctx, false, db.StorageVolumeFilter{Type: &volTypeCustom, PoolID: &poolID}, db.StorageVolumeFilter{Type: &volTypeVM, PoolID: &poolID})
			if err != nil {
				return fmt.Errorf("Failed getting custom storage volumes of pool %q: %w", pool, err)
			}
