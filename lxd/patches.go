...
func patchVMRenameUUIDKey(name string, d *Daemon) error {
	...
	return d.State().Cluster.InstanceList(nil, func(inst db.InstanceArgs, p api.Project, profiles []api.Profile) error {
		...
		return d.State().Cluster.Transaction(func(tx *db.ClusterTx) error {
			...
			for _, snap := range snaps {
				snapConfig, err := tx.GetInstanceSnapshotConfig(snap.ID)
				if err != nil {
					return err
				}
