...
func patchEventEntitlementNames(name string, d *Daemon) error {
	...
	return s.DB.Cluster.Transaction(d.shutdownCtx, func(ctx context.Context, tx *db.ClusterTx) error {
		...
		// Rename `can_view_events` on `project` to `can_view_lifecycle_events`.
		_, err := tx.Tx().ExecContext(ctx, q, auth.EntitlementCanViewLifecycleEvents, auth.EntitlementCanViewEvents, dbCluster.EntityType(entity.TypeProject))
		if err != nil {
			return err
		}
