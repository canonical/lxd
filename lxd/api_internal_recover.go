			...
			for projectName, poolVols := range poolProjectVols {
				for _, poolVol := range poolVols {
					...
					if poolVol.Instance != nil {
						...
						displayName = rootVol.Name
						displaySnapshotCount = len(rootVol.Snapshots)
					}
	...
	for _, pool := range pools {
		...
		if pool.ID() == storagePools.PoolIDTemporary {
			...
			for _, poolVol := range poolVols {
				...
				if err != nil {
					return response.SmartError(fmt.Errorf("Failed getting the root volume's pool: %w", err))
				}
			...
			if instPoolVol != nil {
				...
				poolID, err = dbStoragePoolCreateAndUpdateCache(s, rootVolPool.Name, rootVolPool.Description, rootVolPool.Driver, rootVolPool.Config)
				if err != nil {
