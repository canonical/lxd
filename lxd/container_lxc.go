...
func (c *containerLXC) fillNetworkDevice(name string, m types.Device) (types.Device, error) {
	...
	if !shared.StringInSlice(m["nictype"], []string{"physical", "infiniband", "sriov"}) && m["hwaddr"] == "" {
...
func (c *containerLXC) removeDiskDevice(name string, m types.Device) error {
	...
	if m["pool"] != "" {
		s, err := storagePoolVolumeInit(c.state, m["pool"], m["source"], storagePoolVolumeTypeCustom)
		if err != nil {
			return err
		}
