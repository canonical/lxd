...
func oidcSessionsGet(d *Daemon, r *http.Request) response.Response {
	...
	for _, session := range sessions {
		...
		if recurse {
			...
			sessionURLs = append(sessionURLs, api.NewURL().Path(version.APIVersion, "auth", "oidc-sessions", session.UUID).String())
		}
	}
...
func oidcSessionGet(d *Daemon, r *http.Request) response.Response {
	...
	session, err := dbSessionDetails.ToAPI()
	if err != nil {
		return response.SmartError(err)
	}

	return response.SyncResponse(true, session)
}
...
func oidcSessionDelete(d *Daemon, r *http.Request) response.Response {
	...
	sessionID, err := uuid.Parse(dbSessionDetails.UUID)
	if err != nil {
		return response.SmartError(err)
	}
	...
	s := d.State()
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		return dbCluster.DeleteOIDCSessionByUUID(ctx, tx.Tx(), sessionID)
	})
	if err != nil {
		return response.SmartError(err)
	}
