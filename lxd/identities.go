...
//	  "201":
//	    oneOf:
//	      - $ref: "#/responses/IdentityTokenTLS"
//	      - $ref: "#/responses/EmptySyncResponse"
//	  "400":
//	    $ref: "#/responses/BadRequest"
//	  "403":
...
func createTLSIdentity(d *Daemon, r *http.Request) response.Response {
	...
	if req.Token {
		...
		trustToken := api.IdentityTokenTLS{TrustToken: base64.StdEncoding.EncodeToString(b)}
	...
	if !trusted {
		...
		identifier, err := tlsIdentityTokenValid(r.Context(), s, *joinToken)
		if err != nil {
...
func tlsIdentityTokenValid(ctx context.Context, s *state.State, token api.CertificateAddToken) (uuid.UUID, error) {
	var id *dbCluster.Identity
...
func updateIdentityCache(d *Daemon) {
	...
	for _, id := range identities {
		...
		if cacheEntry.AuthenticationMethod == api.AuthenticationMethodTLS {
			if cacheEntry.IdentityType == api.IdentityTypeCertificateClientPending {
				continue
			}
