...
func identityBearerTokenPost(d *Daemon, r *http.Request) response.Response {
	...
	var token string
	switch id.Type {
	case api.IdentityTypeBearerTokenLXD:
		...
		token, err = encryption.GetLXDBearerToken(secret, id.Identifier, s.GlobalConfig.ClusterUUID(), expiresAt, serverCertFingerprint)
