...
func (c *cmdRecover) command() *cobra.Command {
	cmd := &cobra.Command{}
	cmd.Use = "recover"
	cmd.Short = "Recover LXD database records for instances and custom volumes"
	cmd.Long = cli.FormatSection("Description", cmd.Short+`

  Use this interactive tool to re-create a LXD database.

  You must re-create at least one storage pool before running this command. When
  you run this command, the recovery tool scans known storage pools and
  identifies resources missing from the LXD database. The tool can automatically
  re-create database records for instances and custom volumes discovered in the
  storage pools. Before doing so, the tool will prompt you to manually re-create
  other missing entities that it discovers.
