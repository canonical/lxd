...
func (c *cmdRecover) run(cmd *cobra.Command, args []string) error {
	...
	for {
		...
		if len(existingPools) == 0 {
			return errors.New(`There are no storage pools defined in the database. To recover pools that exist on the storage device already, run "lxc storage create" for each pool with its source configuration and "source.recover=true" setting.`)
		}
