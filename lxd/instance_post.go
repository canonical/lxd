...
func instancePost(d *Daemon, r *http.Request) response.Response {
	...
	if req.Migration {
		...
		if needsClusterMove && needsLocalCopy {
			// Check if source is offline with remote storage.
			...
			// Pool/config changes (without project changes) can be handled directly in cluster migration to avoid copying locally then migrating over the network.
			// Project changes require local copy first since cluster migration protocol assumes same project on source and dest.
			if targetProjectName == inst.Project().Name {
		...
		if needsLocalCopy || needsClusterMove {
			...
			run := func(op *operations.Operation) error {
				...
				if needsClusterMove {
					...
					return instancePostMigration(r.Context(), s, currentInst, req, targetMemberInfo, targetGroupName, op)
				}
...
func migrateInstance(ctx context.Context, s *state.State, inst instance.Instance, targetNode string, targetGroupName string, req api.InstancePost, targetArgs *db.InstanceArgs, op *operations.Operation) error {
	...
	if srcMember.IsOffline(s.GlobalConfig.OfflineThreshold()) && srcPool.Driver().Info().Remote {
		f, err := instancePostClusteringMigrateWithRemoteStorage(s, srcPool, inst, targetArgs.Name, newMember, targetGroupName)
		if err != nil {
			return err
		}
