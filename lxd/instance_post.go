...
func instancePost(d *Daemon, r *http.Request) response.Response {
	...
	if req.Migration {
		...
		if req.Pool != "" || req.Project != "" {
			...
			op, err := operations.CreateUserOperation(s, projectName, requestor, operations.OperationClassTask, operationtype.InstanceMigrate, resources, nil, run, nil, nil)
			if err != nil {
				return response.InternalError(err)
			}
		...
		if targetMemberInfo != nil {
			...
			run := func(ctx context.Context, op *operations.Operation) error {
				return migrateInstance(ctx, s, inst, targetMemberInfo.Name, targetGroupName, req, op)
			}
		...
		// Pull mode.
		op, err := operations.CreateUserOperation(s, projectName, requestor, operations.OperationClassWebsocket, operationtype.InstanceMigrate, resources, ws.Metadata(), run, cancel, ws.Connect)
		if err != nil {
			return response.InternalError(err)
		}
	...
	op, err := operations.CreateUserOperation(s, projectName, requestor, operations.OperationClassTask, operationtype.InstanceRename, resources, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
...
func instancePostClusteringMigrate(s *state.State, srcPool storagePools.Pool, srcInst instance.Instance, newInstName string, srcMember db.NodeInfo, newMember db.NodeInfo, targetGroupName string, stateful bool, allowInconsistent bool) (operations.RunFunc, error) {
	...
	run := func(ctx context.Context, op *operations.Operation) error {
		...
		srcOp, err := operations.CreateUserOperation(s, projectName, op.Requestor(), operations.OperationClassWebsocket, operationtype.InstanceMigrate, resources, srcMigration.Metadata(), run, cancel, srcMigration.Connect)
		if err != nil {
			return err
		}
...
func instancePostClusteringMigrateWithRemoteStorage(s *state.State, srcPool storagePools.Pool, srcInst instance.Instance, newInstName string, newMember db.NodeInfo, targetGroupName string) (operations.RunFunc, error) {
...
func migrateInstance(ctx context.Context, s *state.State, inst instance.Instance, targetNode string, targetGroupName string, req api.InstancePost, op *operations.Operation) error {
	...
	f, err := instancePostClusteringMigrate(s, srcPool, inst, req.Name, srcMember, newMember, targetGroupName, req.Live, req.AllowInconsistent)
	if err != nil {
		return err
	}
