...
// restrictions. A cross-project move is validated as an instance creation (its snapshots are
// validated too when instanceOnly is false). A same-project move applies the merged config,
// devices, and profiles to the instance and is therefore validated as an instance update.
// rootDevKey and rootDevPool describe the target instance's root disk, used to adjust snapshot
// device pools before validation (mirroring what instanceCreateAsCopy persists).
func checkTargetProjectRestrictions(s *state.State, inst instance.Instance, targetProject string, sourceProject string, targetName string, instConfig map[string]string, instDevices map[string]map[string]string, instProfiles []string, instanceOnly bool, rootDevKey string, rootDevPool string) error {
	...
	if targetProject == sourceProject {
		...
		return s.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
			return project.AllowInstanceUpdate(tx, sourceProject, inst.Name(), updateReq, inst.LocalConfig())
		})
	}
