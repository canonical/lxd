...
func clusterLinksPost(d *Daemon, r *http.Request) response.Response {
	...
	if req.Name != "" {
		...
		for _, address := range trustToken.Addresses {
			...
			_, _, err = client.RawQuery("POST", "/1.0/cluster/links", api.ClusterLinkPost{TrustToken: trustToken.String(), ClusterLinkPut: clusterLinkPut, ClusterCertificate: string(networkCert.PublicKey())}, "")
			if err != nil {
	...
	addresses := strings.Split(req.Config["volatile.addresses"], ",")
	if req.Name == "" && len(addresses) > 0 {
		...
		cert, _, err = cluster.GetClusterLinkCertificate(r.Context(), addresses, fingerprint, version.UserAgent)
		if err != nil {
			return response.BadRequest(fmt.Errorf("Failed to validate cluster certificate: %w", err))
		}
	...
	clusterLinkConfigKeys := map[string]func(value string) error{
		...
		"volatile.addresses": validate.Optional(func(value string) error {
			for entry := range strings.SplitSeq(value, ",") {
				...
				err := validate.IsHostname(entry)
				if err != nil {
					return fmt.Errorf("Invalid address %q: %w", entry, err)
				}
			}
