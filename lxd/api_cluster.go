...
func clusterLinksPost(d *Daemon, r *http.Request) response.Response {
	...
	if req.TrustToken == "" {
		// Create a pending identity and cluster link using the identity API
		return createIdentityTLSPending(r.Context(), s, api.IdentitiesTLSPost{Name: req.Name, Token: true, Groups: req.AuthGroups}, notify, api.IdentityTypeCertificateClusterLinkPending, req.Name, req.Description)
	}
	...
	cert, address, err := clusterlinks.GetClusterLinkCertificate(context.TODO(), joinToken, addresses, version.UserAgent)
	if err != nil {
	...
	args := &lxd.ConnectionArgs{
		TLSClientCert: string(serverCert.PublicKey()),
		TLSClientKey:  string(serverCert.PrivateKey()),
		TLSServerCert: string(clusterCert),
		UserAgent:     version.UserAgent,
	}

	client, err := lxd.ConnectLXD("https://"+address, args)
	if err != nil {
		return response.BadRequest(fmt.Errorf("Failed to connect to remote cluster: %w", err))
	}

	_, _, err = client.RawQuery("POST", "/1.0/identities/tls", api.IdentitiesTLSPost{Name: req.Name, Groups: req.AuthGroups, TrustToken: joinToken.String()}, "")
	if err != nil {
		return response.SmartError(fmt.Errorf("Failed to create identity on remote cluster: %w", err))
	}

	// Create an identity for the cluster link using the identity API
	identityReq := api.IdentitiesTLSPost{
		Name:   req.Name,
		Groups: req.AuthGroups,
	}
	...
	return createIdentityTLSUntrusted(context.TODO(), s, r.TLS.PeerCertificates, networkCert, identityReq, notify)
}
