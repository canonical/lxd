...
func clusterLinkStateGet(d *Daemon, r *http.Request) response.Response {
	...
	for _, address := range shared.SplitNTrimSpace(clusterLink.Config["volatile.addresses"], ",", -1, false) {
		clusterLinkMember := api.ClusterLinkMember{
			Address:    address,
			AuthStatus: api.ClusterLinkMemberAuthStatusUnreachable,
			ServerName: "UNKNOWN",
		}
		...
		if err != nil {
			...
			if resp.StatusCode == http.StatusOK {
				clusterLinkMember.AuthStatus = api.ClusterLinkMemberAuthStatusUntrusted
			}

			// Set [api.ClusterLinkMember.AuthStatus] based on the 'auth' field.
			authVal, ok := respData["auth"].(string)
			if ok && authVal == "trusted" {
				clusterLinkMember.AuthStatus = api.ClusterLinkMemberAuthStatusTrusted
			}
