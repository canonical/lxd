...
func rebalanceMemberRoles(reqContext context.Context, s *state.State, gateway *cluster.Gateway, unavailableMembers []string) error {
...
again:
	...
	err = changeMemberRole(reqContext, s, address, nodes)
	if err != nil {
		return err
	}
...
func changeMemberRole(reqContext context.Context, s *state.State, address string, nodes []db.RaftNode) error {
	...
	client, err := cluster.Connect(reqContext, address, s.Endpoints.NetworkCert(), s.ServerCert(), true)
	if err != nil {
		return err
	}
...
func internalClusterHeal(d *Daemon, r *http.Request) response.Response {
	migrateFunc := func(s *state.State, _ *http.Request, inst instance.Instance, targetMemberInfo *db.NodeInfo, live bool, startInstance bool, _ map[string]any, _ *operations.Operation) error {
		...
		dest, err := cluster.Connect(context.Background(), targetMemberInfo.Address, s.Endpoints.NetworkCert(), s.ServerCert(), true) //nolint:staticcheck
		if err != nil {
			return err
		}
