...
func updateClusterLink(s *state.State, gateway *cluster.Gateway, r *http.Request, isPatch bool) response.Response {
	name, err := url.PathUnescape(mux.Vars(r)["name"])
	if err != nil {
		return response.SmartError(err)
	}
...
func clusterLinkPatch(d *Daemon, r *http.Request) response.Response {
	return updateClusterLink(d.State(), d.gateway, r, true)
}
...
func clusterLinkPut(d *Daemon, r *http.Request) response.Response {
	return updateClusterLink(d.State(), d.gateway, r, false)
}
...
func clusterLinksPost(d *Daemon, r *http.Request) response.Response {
	...
	if req.Name != "" {
		...
		for _, address := range trustToken.Addresses {
			...
			_, _, err = client.RawQuery("POST", "/1.0/cluster/links", api.ClusterLinkPost{TrustToken: trustToken.String(), Addresses: listenAddresses, ClusterCertificate: string(networkCert.PublicKey())}, "")
			if err != nil {
	...
	if req.Name == "" && len(req.Addresses) > 0 {
		...
		if block == nil {
			return response.BadRequest(errors.New("Failed to decode certificate"))
		}
...
func clusterLinkValidateConfig(config map[string]string) error {
	...
	for k, v := range config {
		...
		// lxdmeta:generate(entities=cluster; group=link; key=user.*)
		// User keys can be used in search.
		// ---
		//  type: string
		//  shortdesc: Free form user key/value storage
