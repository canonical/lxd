...
func clusterLinkStateGet(d *Daemon, r *http.Request) response.Response {
	...
	clusterLinkState := api.ClusterLinkState{
		ClusterLinkMembers: make([]api.ClusterLinkMember, len(addresses)),
	}
	...
	for i, address := range addresses {
		...
		go func(index int, addr string) {
			...
			if err != nil {
				logger.Error("Failed to get remote certificate for cluster link member", logger.Ctx{"address": addr, "err": err})
			} else {
				...
				if err != nil {
					logger.Error("Failed to connect to cluster link member", logger.Ctx{"address": addr, "err": err})
				} else {
			...
			clusterLinkState.ClusterLinkMembers[index] = clusterLinkMember
		}(i, address)
	}
