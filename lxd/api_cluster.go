...
func clusterLinksPost(d *Daemon, r *http.Request) response.Response {
	...
	if req.Name != "" && req.TrustToken == "" {
		...
		err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
			// Check if we already have an identity with the same name.
			_, err := dbCluster.GetIdentityByNameOrIdentifier(ctx, tx.Tx(), api.AuthenticationMethodTLS, req.Name)
			if err == nil {
