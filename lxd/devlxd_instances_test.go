...
func Test_devLXDPatchInstanceDevices(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.TestName, func(t *testing.T) {
			...
			err := patchDevLXDInstanceDevices(&test.CurrentInstance, test.RequestInstance, identityID, test.AccessValidator)
			...
			if test.ExpectErr != "" {
				...
				if test.ExpectConfig != nil && !reflect.DeepEqual(test.ExpectConfig, test.CurrentInstance.Config) {
					t.Fatalf("Expected config to be:\n%+v\nFound:\n%+v", test.ExpectConfig, test.CurrentInstance.Config)
				}

				// Confirm expected devices are present in patched instance.
				if test.ExpectDevices != nil && !reflect.DeepEqual(test.ExpectDevices, test.CurrentInstance.Devices) {
					t.Fatalf("Expected devices to be:\n%+v\nFound:\n%+v", test.ExpectDevices, test.CurrentInstance.Devices)
				}
			}
		})
	}
