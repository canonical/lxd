...
func (s *storageDir) ContainerCreateFromImage(
	...
	if err := unpackImage(imagePath, container.Path(), s.storage.GetStorageType(), s.s.OS.RunningInUserNS); err != nil {
...
func (s *storageDir) MigrationSink(live bool, container container, snapshots []*migration.Snapshot, conn *websocket.Conn, srcIdmap *idmap.IdmapSet) error {
