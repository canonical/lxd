...
func devLXDStoragePoolVolumePostAccessHandler(d *Daemon, r *http.Request) response.Response {
	...
	// The effective project name is set in the request context even though it cannot differ from the request (instance) project.
	// This is because the main API handler expects this to be set, and the devlxd handler calls it directly with a constructed http.Request.
