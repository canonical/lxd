...
// ConstructOperationsFromDB constructs a list of Operation objects based on their database representation.
...
func constructOperationsFromDB(ctx context.Context, tx *sql.Tx, s *state.State, dbOps []cluster.Operation, inspectionOnly bool) ([]*Operation, error) {
	...
	for _, parent := range parents {
		...
		isDurable := operationtype.Class(parent.Row.Class) == operationtype.OperationClassDurable
		childOps, hasChildren := children[parent.Row.ID]
		requiresRunHook := !inspectionOnly && isDurable && !hasChildren
		...
		for _, child := range childOps {
			// A child operation requires a run hook if it is durable and not for inspection only.
			// Child operations are all durable if the parent is durable.
			childRequiresRunHook := !inspectionOnly && isDurable
